...
from chaste_codegen._rdf import (PYCMLMETA, OXMETA)
from chaste_codegen.model_with_conversions import set_is_metadata, get_equations_for
...
class ChasteModel(object):
    ...
    def __init__(self, model, file_name, **kwargs):
        ...
        self._stimulus_params, self._stimulus_equations = self._get_stimulus()#self._model.stimulus_params, self._model.stimulus_equations#self._get_stimulus()
        ...
        self._modifiers = sorted(self._modifiers_set, key=lambda m: self._model.get_display_name(m, OXMETA))
        self._modifiable_parameters = self._model.modifiable_parameters - self._stimulus_params
        ...
        if self._model.membrane_capacitance is None:#move...
            self._model.membrane_capacitance = 1

        self._convert_other_currents()

        self._ionic_derivs = self._get_ionic_derivs()
        self._i_ionic_lhs, self._equations_for_ionic_vars = self._get_equations_for_ionic_vars()
        ...
        self._in_interface.add(self._i_ionic_lhs)
        self._in_interface.update(self._stimulus_params)
    ...
    def get_equations_for(self, variables, recurse=True, filter_modifiable_parameters_lhs=True, optimise=True):
        return get_equations_for(self._model, variables, recurse=True, filter_modifiable_parameters_lhs=True, optimise=True)
    ...
    def _state_var_key_order(self, var):
        ...
        elif var == self._model.cytosolic_calcium_concentration_var and\
                self._model.cytosolic_calcium_concentration_var.units.dimensionality == \
                self._model.conversion_units.get_unit('millimolar').dimensionality:
    ...
    # def _add_conversion_rules(self):
     ...
     #def _get_time_variable(self):
        ...
        # self._model.set_is_metadata(time_variable, 'derived-quantity')
        # try:
    ...
    #def _annotate_if_not_statevar(self, var):
        ...
        # if var not in self._state_vars:
            # if self._model.is_constant(var):
                # self._model.set_is_metadata(var, 'modifiable-parameter')
            # else:  # not constant
                # self._model.set_is_metadata(var, 'derived-quantity')

    #def _get_membrane_voltage_var(self):
        # """ Find the membrane_voltage variable"""
        # try:
            # # Get and convert V
            # voltage = self._model.get_variable_by_ontology_term((OXMETA, 'membrane_voltage'))
        ...
        # model.annotate_if_not_statevar(model, voltage)  # If V is not state var annotate as appropriate.
        # return voltage

    #def _get_cytosolic_calcium_concentration_var(self):
        # """ Find the cytosolic_calcium_concentration variable if it exists"""
        # try:
            # calcium = self._model.get_variable_by_ontology_term((OXMETA, 'cytosolic_calcium_concentration'))
        ...
        # self._model.(calcium)  # If not state var annotate as appropriate
        # return calcium
    ...
    def _get_modifiers(self):
        ...
        if self.use_modifiers:
            modifiers = set(filter(lambda m: m not in (self._model.membrane_stimulus_current_orig, self._model.time_variable) and
                                   self._model.has_ontology_annotation(m, OXMETA), self._model.variables()))
    ...
    # def _get_modifiable_parameters(self):
        ...
        # tagged = set(self._model.get_variables_by_rdf((PYCMLMETA, 'modifiable-parameter'), 'yes', sort=False))
        # annotated = set(filter(lambda q: self._model.has_ontology_annotation(q, OXMETA), self._model.variables()))

        # return (tagged | annotated) -\
            # set(self._model.get_derived_quantities(sort=False) + [self._model.time_variable]) - self._state_vars
    ...
    #def _get_membrane_stimulus_current(self):
        ...
        # try:  # get membrane_stimulus_current
            # return self._model.get_variable_by_ontology_term((OXMETA, 'membrane_stimulus_current'))
        # except KeyError:
    ...
    def _convert_membrane_stimulus_current(self):
        ...
        if self._model.membrane_stimulus_current_orig is None:
            return None, self._model.stimulus_units
        # get derivative equations
        d_eqs = get_equations_for(self._model, self._y_derivatives, filter_modifiable_parameters_lhs=False, optimise=False)

        # get dv/dt
        deriv_eq_only = filter(lambda eq: isinstance(eq.lhs, Derivative) and
                               eq.lhs.args[0] == self._model.membrane_voltage_var, d_eqs)
        ...
        for variable in variables:
            if self._model.membrane_stimulus_current_orig != variable:
                if self._model.membrane_stimulus_current_orig.units.dimensionality == variable.units.dimensionality:
        ...
        if isinstance(voltage_rhs, Expr):
            voltage_rhs = voltage_rhs.xreplace({self._model.membrane_stimulus_current_orig: 1.0})  # stimulus = 1
        ...
        membrane_stimulus_current_converted = self._model.convert_variable(self._model.membrane_stimulus_current_orig,
                                                                           self._model.conversion_units.get_unit('uA_per_cm2'),
                                                                           DataDirectionFlow.INPUT)

        # Replace stim = ... with stim = +/-GetIntracellularAreaStimulus(t)
        GetIntracellularAreaStimulus = Function('GetIntracellularAreaStimulus', real=True)(self._model.time_variable)
        if negate_stimulus:
        ...
        set_is_metadata(self._model, membrane_stimulus_current_converted, 'derived-quantity')
        return membrane_stimulus_current_converted, \
            (self._model.membrane_stimulus_current_orig.units, membrane_stimulus_current_converted.units)
    ...
    def _convert_other_currents(self):
        ...
        all_currents = set(get_variables_transitively(self._model, (OXMETA, 'IonicCurrent')))
        currents = current_related.intersection(all_currents) - set([self._membrane_stimulus_current_converted])
        for current in currents:
            try:
                self._model.convert_variable(current, self._model.conversion_units.get_unit('uA_per_cm2'), DataDirectionFlow.OUTPUT)
            except DimensionalityError:
    ...
    #def _get_membrane_capacitance(self):
        ...
        # try:
            # capacitance = self._model.get_variable_by_ontology_term((OXMETA, 'membrane_capacitance'))
    ...
    def _get_stimulus(self):
        return self._model.stimulus_params, self._model.stimulus_equations
        # """ Store the stimulus currents in the model"""
        # stim_params_orig, stim_params = set(), set()
        # try:  # Get required stimulus parameters
            # ampl = self._model.get_variable_by_ontology_term((OXMETA, 'membrane_stimulus_current_amplitude'))
            # duration = self._model.get_variable_by_ontology_term((OXMETA, 'membrane_stimulus_current_duration'))
            # period = self._model.get_variable_by_ontology_term((OXMETA, 'membrane_stimulus_current_period'))
        ...
        # try:  # Get optional stimulus parameter
            # offset = self._model.get_variable_by_ontology_term((OXMETA, 'membrane_stimulus_current_offset'))
        ...
        # try:  # Get optional stimulus parameter
            # end = self._model.get_variable_by_ontology_term((OXMETA, 'membrane_stimulus_current_end'))
            # if end is not None:
    ...
    def _get_ionic_derivs(self):
        """ Getting the derivatives that define V (self._model.membrane_voltage_var)"""
        # use the RHS of the ODE defining V
        return set(filter(lambda x: x.args[0] == self._model.membrane_voltage_var, self._model.get_derivatives(sort=False)))
    ...
    def _get_equations_for_ionic_vars(self):
        ...
        for unit in self._stimulus_units:
            ...
            while len(equations_for_ionic_vars) == 0 and old_equations != equations:
                ...
                equations = get_equations_for(self._model, equations, recurse=False, filter_modifiable_parameters_lhs=False,
                                                   optimise=False)
                equations_for_ionic_vars = [eq for eq in equations
                                            if (eq.lhs != self._model.membrane_stimulus_current_orig
        ...
        i_ionic_lhs = self._model.add_variable(name='_i_ionic', units=self._model.conversion_units.get_unit('uA_per_cm2'))
        ...
        for var in reversed(equations_for_ionic_vars):
            factor = self._model.units.get_conversion_factor(from_unit=self._model.units.evaluate_units(var.lhs),
                                                             to_unit=self._model.conversion_units.get_unit('uA_per_cm2'))
    ...
    def _get_extended_equations_for_ionic_vars(self):
        ...
        # Update equations to include i_ionic_eq & defining eqs, set stimulus current to 0
        return [eq if eq.lhs != self._model.membrane_stimulus_current_orig else Eq(eq.lhs, 0.0)
                for eq in get_equations_for(self._model, [eq.lhs for eq in self._equations_for_ionic_vars])
    ...
    def _get_derivative_equations(self):
        ...
        return [eq for eq in get_equations_for(self._model, self._y_derivatives)
    ...
    def _get_derivative_eqs_excl_voltage(self):
        ...
        eqs = set()
        deriv_and_eqs = set(filter(lambda deriv: deriv.args[0] != self._model.membrane_voltage_var, self._y_derivatives))
        num_derivatives = -1
    ...
    def _get_derivative_eqs_voltage(self):
        ...
        eqs = set()
        derivatives = set(filter(lambda deriv: deriv.args[0] == self._model.membrane_voltage_var, self._y_derivatives))
        num_derivatives = -1
