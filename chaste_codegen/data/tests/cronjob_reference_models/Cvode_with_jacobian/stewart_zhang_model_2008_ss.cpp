...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellstewart_zhang_model_2008_ssFromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x150 = var_x140 / pow(var_x141, 3);
        ...
        const double var_x195 = pow((1 + 0.037037037037037035 * var_chaste_interface__membrane__V), 2);
        const double var_x196 = exp(-3.2400000000000002 * var_x195);
        const double var_x197 = exp(2.8571428571428572 + var_x99);
        const double var_x198 = 1.0 + var_x197;
        const double var_x199 = var_x189 / pow(var_x190, 2);
        const double var_x200 = 1102.5 * var_x196;
        const double var_x201 = 1 / (20.0 + var_x200 + 200.0 * var_x194 + 180.0 * var_x191);
        const double var_x202 = exp(2.5 + var_x96);
        const double var_x203 = 1.0 + var_x202;
        const double var_x204 = 1 / var_x203;
        const double var_x205 = exp(-3.0375000000000001 * var_x195);
        const double var_x206 = exp(5.0 + var_x99);
        const double var_x207 = 1.0 + var_x206;
        const double var_x208 = 562.0 * var_x205;
        const double var_x209 = 1 / (var_x208 + 80.0 * var_x191 + 31.0 * var_x204);
        const double var_x210 = pow(var_chaste_interface__calcium_dynamics__Ca_ss, 2);
        const double var_x211 = 1 / (1.0 + 399.99999999999994 * var_x210);
        const double var_x212 = 1 / (2.0 + 80.0 * var_x211);
        const double var_x213 = var_chaste_interface__calcium_dynamics__Ca_ss / pow((0.0025000000000000005 + var_x210), 2);
        const double var_x214 = exp(-8.0 + var_x126);
        const double var_x215 = 1.0 + var_x214;
        const double var_x216 = 1 / var_x215;
        const double var_x217 = exp(-1.953125 * pow((1 + 0.040000000000000001 * var_chaste_interface__membrane__V), 2));
        const double var_x218 = exp(2.0769230769230771 + var_x180);
        const double var_x219 = 1.0 + var_x218;
        const double var_x220 = 85.0 * var_x217;
        const double var_x221 = 1 / (42.0 + var_x220 + 5.0 * var_x216);
        const double var_x222 = exp(1.5384615384615385 + var_x181);
        const double var_x223 = 1.0 + var_x222;
        const double var_x224 = exp(-0.88888888888888884 * pow((1 + 0.025000000000000001 * var_chaste_interface__membrane__V), 2));
        ...
        const double var_x233 = var_x210 * var_x232;
        ...
        const double var_x241 = var_x210 * var_x235 * var_x238 * var_x240;
        ...
        const double var_x248 = 0.010081409587705739 * var_x41;
        const double var_x249 = var_x248 * var_x39;
        ...
        const double var_x254 = var_x248 * var_x252;
        const double var_x255 = var_x254 * var_x61;
        const double var_x256 = var_x254 * var_x63;
        const double var_x257 = 0.30611192392099484 * var_x236;
        const double var_x258 = -0.11249999999999999 + 0.067500000000000004 * var_x231;
        const double var_x259 = 0.015262253697454368 * var_x25;
        const double var_x260 = 0.00028664102046376645 * var_x21 - 1.0730088665360983e-5 * var_chaste_interface__membrane__V;
        const double var_x261 = 0.00025554140647880875 * var_x21 - 9.5659091108185493e-6 * var_chaste_interface__membrane__V;
        ...
        IJth(rJacobian, 10, 0) = 8.2270247479188195e-7 * (-var_chaste_interface__L_type_Ca_current_f_gate__f + 1.0 / var_x198) * (18.0 * var_x199 - var_x200 * (-0.23999999999999999 - 0.0088888888888888889 * var_chaste_interface__membrane__V) - 20.0 * var_x192 / pow(var_x193, 2)) / pow((0.018140589569160998 + var_x196 + 0.16326530612244897 * var_x191 + 0.18140589569160998 * var_x194), 2) - 0.14285714285714285 * var_x197 * var_x201 / pow(var_x198, 2);
        IJth(rJacobian, 11, 0) = 3.1661199832828864e-6 * (0.33000000000000002 - var_chaste_interface__L_type_Ca_current_f2_gate__f2 + 0.67000000000000004 / var_x207) * (8.0 * var_x199 - var_x208 * (-0.22500000000000001 - 0.0083333333333333332 * var_chaste_interface__membrane__V) - 3.1000000000000001 * var_x202 / pow(var_x203, 2)) / pow((var_x205 + 0.055160142348754451 * var_x204 + 0.14234875444839859 * var_x191), 2) - 0.095714285714285724 * var_x206 * var_x209 / pow(var_x207, 2);
        IJth(rJacobian, 13, 0) = 0.00013840830449826991 * (-var_chaste_interface__transient_outward_current_s_gate__s + 1.0 / var_x219) * (-var_x220 * (-0.15625 - 0.0062500000000000003 * var_chaste_interface__membrane__V) + 1.0 * var_x214 / pow(var_x215, 2)) / pow((0.49411764705882355 + var_x217 + 0.058823529411764705 * var_x216), 2) - 0.076923076923076927 * var_x218 * var_x221 / pow(var_x219, 2);
        IJth(rJacobian, 14, 0) = 0.076923076923076927 * var_x222 * var_x225 / pow(var_x223, 2) - 0.09569377990430622 * var_x224 * (-0.044444444444444446 - 0.0011111111111111111 * var_chaste_interface__membrane__V) * (-var_chaste_interface__transient_outward_current_r_gate__r + 1.0 / var_x223) / pow((0.69856459330143539 + var_x224), 2);
        IJth(rJacobian, 16, 0) = var_x253 * (-var_x249 + 0.00075477277169111218 * var_x45 - 0.00018869319292277804 * var_x46);
        IJth(rJacobian, 18, 0) = -3.389679425876563e-8 - 1.7024840244711204e-6 * var_chaste_interface__hyperpolarization_activated_current_y_gate__y - 3.0388663876087418e-8 * var_x18 - 7.3941785176126768e-11 * var_x30 - 0.00080594011556369446 * var_x35 - var_x259 * var_chaste_interface__fast_sodium_current_h_gate__h;
        ...
        IJth(rJacobian, 15, 1) = var_x229 * (0.00036000000000000002 + 7.96875e-10 * var_x86);
        IJth(rJacobian, 16, 1) = 1.1399999999999999 * var_x252;
        ...
        IJth(rJacobian, 19, 3) = var_x260 * var_chaste_interface__rapid_time_dependent_potassium_current_Xr2_gate__Xr2;
        ...
        IJth(rJacobian, 19, 4) = var_x260 * var_chaste_interface__rapid_time_dependent_potassium_current_Xr1_gate__Xr1;
        ...
        IJth(rJacobian, 18, 7) = -var_x259 * var_x57;
        ...
        IJth(rJacobian, 16, 9) = -var_x255 * var_chaste_interface__L_type_Ca_current_f_gate__f;
        IJth(rJacobian, 0, 10) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x62 * var_chaste_interface__L_type_Ca_current_d_gate__d);
        IJth(rJacobian, 10, 10) = -var_x201;
        IJth(rJacobian, 16, 10) = -var_x255 * var_chaste_interface__L_type_Ca_current_d_gate__d;
        ...
        IJth(rJacobian, 16, 11) = -var_x256 * var_chaste_interface__L_type_Ca_current_fCass_gate__fCass;
        ...
        IJth(rJacobian, 16, 12) = -var_x256 * var_chaste_interface__L_type_Ca_current_f2_gate__f2;
        IJth(rJacobian, 0, 13) = mSetVoltageDerivativeToZero ? 0.0 : (var_x65 * var_chaste_interface__transient_outward_current_r_gate__r);
        IJth(rJacobian, 13, 13) = -var_x221;
        IJth(rJacobian, 19, 13) = var_x261 * var_chaste_interface__transient_outward_current_r_gate__r;
        ...
        IJth(rJacobian, 19, 14) = var_x261 * var_chaste_interface__transient_outward_current_s_gate__s;
        IJth(rJacobian, 1, 15) = 2.40087783467447e-5 * var_x82;
        IJth(rJacobian, 15, 15) = var_x229 * (-0.00036000000000000002 - var_x237 + 0.021759999999999998 * var_x244 - 0.0032639999999999995 * var_x241) + 0.66666666666666674 * (0.00036000000000000002 * var_chaste_interface__calcium_dynamics__Ca_i + 0.0063749999999999996 * var_x87 - 0.00036000000000000002 * var_chaste_interface__calcium_dynamics__Ca_SR - var_x237 * var_x239) / (pow(var_x226, 3) * pow((0.33333333333333337 + var_x227), 2));
        IJth(rJacobian, 16, 15) = var_x253 * (var_x257 + 0.065303877103145572 * var_x241 - 0.43535918068763713 * var_x244);
        IJth(rJacobian, 17, 15) = 0.059999999999999998 * var_x242 * var_chaste_interface__calcium_dynamics__Ca_ss;
        IJth(rJacobian, 0, 16) = mSetVoltageDerivativeToZero ? 0.0 : (-0.14375050524031124 * var_x66);
        IJth(rJacobian, 1, 16) = 0.0038 * var_x82;
        IJth(rJacobian, 12, 16) = -0.0030000000000000001 * var_x212 * var_x213 + 6.2500000000000001e-5 * var_x213 * (0.40000000000000002 - var_chaste_interface__L_type_Ca_current_fCass_gate__fCass + 0.59999999999999998 * var_x211) / pow((0.025000000000000001 + var_x211), 2);
        IJth(rJacobian, 15, 16) = var_x229 * (var_x237 + 0.032639999999999995 * var_x245 - 0.030599999999999995 * var_x246);
        IJth(rJacobian, 16, 16) = var_x253 * (-1.1399999999999999 - var_x257 + 0.61222384784198969 * var_x246 - 0.65303877103145569 * var_x245 - 0.0025203523969264349 * var_x66) + 0.00020000000000000001 * (1.1399999999999999 * var_chaste_interface__calcium_dynamics__Ca_i - 1.1399999999999999 * var_chaste_interface__calcium_dynamics__Ca_ss + var_x239 * var_x257 - var_x249 * var_x43) / (pow(var_x250, 3) * pow(var_x251, 2));
        IJth(rJacobian, 17, 16) = var_x258 * var_chaste_interface__calcium_dynamics__R_prime;
        IJth(rJacobian, 15, 17) = -0.015299999999999998 * var_x228 * var_x247;
        IJth(rJacobian, 16, 17) = 0.30611192392099484 * var_x247 * var_x252;
        IJth(rJacobian, 17, 17) = -0.0050000000000000001 + var_x258 * var_chaste_interface__calcium_dynamics__Ca_ss;
