...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellearm_noble_model_1990FromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time_converted, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x110 = exp(-7.5 + var_x103);
        const double var_x111 = exp(-0.80000000000000004 - 0.20000000000000001 * var_chaste_interface__membrane__V);
        const double var_x112 = exp(-0.058823529411764705 * var_chaste_interface__membrane__V);
        const double var_x113 = exp(-1.25 + var_x108);
        ...
        const double var_x117 = 1.0 - var_chaste_interface__L_type_calcium_current_d_gate__d;
        ...
        const double var_x123 = exp(1.8999999999999999 + var_x102);
        ...
        const double var_x128 = 120.0 * var_x21;
        const double var_x129 = 0.0030000000000000001 * var_x126 * (var_x26 + var_x128 * var_x24) / pow(var_x127, 2);
        ...
        const double var_x136 = var_x135 * (-1.0 - 119.0 * var_x24);
        const double var_x137 = 1.0 - var_chaste_interface__calcium_release__ActFrac - var_chaste_interface__calcium_release__ProdFrac;
        ...
        const double var_x141 = pow((var_x140 * var_chaste_interface__intracellular_calcium_concentration__Ca_i), 2.0);
        const double var_x142 = 1.0 * var_x141;
        const double var_x143 = var_x139 * var_x142 * var_x54 * (var_x140 - var_chaste_interface__intracellular_calcium_concentration__Ca_i / pow(var_x139, 2));
        const double var_x144 = var_x143 * var_chaste_interface__calcium_release__ActFrac;
        const double var_x145 = -0.59999999999999998 * var_x138;
        const double var_x146 = 0.5 * var_x141;
        const double var_x147 = 0.00042079708847221582 * var_x6;
        const double var_x148 = 1.5752072268398093e-8 * var_x40;
        const double var_x149 = 8.4159417694443166e-6 * var_chaste_interface__transient_outward_current_s_gate__s;
        const double var_x150 = 3.1504144536796185e-9 * var_x42;
        const double var_x151 = -var_x96;
        ...
        IJth(rJacobian, 3, 0) = -0.0025000000000000001 * var_x109 * (1.0 - var_chaste_interface__fast_sodium_current_h_gate__h) - 0.00062500000000000001 * var_x110 * var_chaste_interface__fast_sodium_current_h_gate__h / pow((0.0031250000000000002 + var_x110), 2);
        IJth(rJacobian, 4, 0) = 0.066600000000000006 * var_x111 / pow((1.0 + var_x111), 2);
        IJth(rJacobian, 5, 0) = -1.9411764705882353e-6 * var_x112 * (1.0 - var_chaste_interface__transient_outward_current_s_gate__s) - 0.0041250000000000002 * var_x113 * var_chaste_interface__transient_outward_current_s_gate__s / pow(var_x114, 2);
        IJth(rJacobian, 6, 0) = ((var_x116) ? (0) : (var_x117 * var_x122 - var_x125 * var_chaste_interface__L_type_calcium_current_d_gate__d + 0.0012000000000000001 * var_x115 * var_x123 * var_chaste_interface__L_type_calcium_current_d_gate__d / pow(var_x124, 2) - 0.0074999999999999997 * var_x115 * var_x117 * var_x120 / pow(var_x121, 2)));
        IJth(rJacobian, 7, 0) = ((var_x131) ? (var_x129) : (var_x129 - var_x134 * var_chaste_interface__L_type_calcium_current_f_Ca_gate__f_Ca + 0.0015625000000000001 * var_x130 * var_x132 * var_chaste_interface__L_type_calcium_current_f_Ca_gate__f_Ca / pow(var_x133, 2)));
        IJth(rJacobian, 8, 0) = 0.048000000000000001 * var_x137 * var_x138;
        IJth(rJacobian, 10, 0) = -1.009913012333318e-7 + 5.8966135352721076e-10 * var_x51 - 2.5247825308332952e-7 * var_x20 - 8.2552589493809504e-8 * var_x49 - var_x147 * var_chaste_interface__fast_sodium_current_h_gate__h - var_x148 * var_x38;
        IJth(rJacobian, 11, 0) = -1.4307101008055338e-6 + 3.0604026121459151e-7 * var_x10 + 1.1793227070544215e-10 * var_x53 - 4.0877431451586679e-6 * var_x5 - 4.7172908282176859e-10 * var_x49 - var_x149 * var_chaste_interface__transient_outward_current_r_gate__r - var_x150 * var_x38;
        ...
        IJth(rJacobian, 7, 1) = var_x135 * (var_x61 + var_x128 * var_x23 - var_x128 * var_x60);
        IJth(rJacobian, 8, 1) = -var_x144 + var_x137 * var_x143;
        IJth(rJacobian, 9, 1) = var_x144;
        IJth(rJacobian, 10, 1) = 0.69280032646065626 * var_x58 + 1.7420999462749737e-9 * var_x57 + 9.2011025176140482e-10 * var_x56 - var_x148 * var_x64;
        IJth(rJacobian, 11, 1) = -var_x150 * var_x64;
        ...
        IJth(rJacobian, 3, 3) = -2.0 / (1.0 + 320.0 * var_x110) - 0.02 * var_x109;
        IJth(rJacobian, 10, 3) = -var_x147 * var_x66;
        IJth(rJacobian, 0, 4) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x8 * var_x9);
        IJth(rJacobian, 4, 4) = -0.33300000000000002;
        IJth(rJacobian, 11, 4) = -var_x149 * var_x9;
        IJth(rJacobian, 0, 5) = mSetVoltageDerivativeToZero ? 0.0 : (-0.24999999999999997 * var_x68);
        IJth(rJacobian, 5, 5) = -0.033000000000000002 / var_x114 - 3.3000000000000003e-5 * var_x112;
        ...
        IJth(rJacobian, 6, 6) = ((var_x116) ? (-0.23999999999999999) : (-var_x115 * var_x122 - var_x115 * var_x125));
        IJth(rJacobian, 10, 6) = -var_x148 * var_x70;
        IJth(rJacobian, 11, 6) = -var_x150 * var_x70;
        ...
        IJth(rJacobian, 7, 7) = ((var_x131) ? (-0.025000000000000001 + var_x136) : (var_x136 - var_x130 * var_x134));
        IJth(rJacobian, 10, 7) = var_x148 * var_x72;
        IJth(rJacobian, 11, 7) = var_x150 * var_x72;
        IJth(rJacobian, 1, 8) = 0.10204081632653063 * var_x95;
        IJth(rJacobian, 8, 8) = -0.059999999999999998 + var_x145 - var_x142;
