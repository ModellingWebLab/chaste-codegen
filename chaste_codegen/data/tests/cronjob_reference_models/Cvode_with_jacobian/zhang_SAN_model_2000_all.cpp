...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellzhang_SAN_model_2000_allFromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time_converted, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x4 = 0.015940800000000001 * var_chaste_interface__rapid_delayed_rectifying_potassium_current_P_as_gate__P_as + 0.023911199999999997 * var_chaste_interface__rapid_delayed_rectifying_potassium_current_P_af_gate__P_af;
        const double var_x5 = 0.28968999999999995 * var_chaste_interface__L_type_Ca_channel_f_gate__f_L;
        const double var_x6 = 0.21390299999999998 * var_chaste_interface__T_type_Ca_channel_f_gate__f_T;
        const double var_x7 = 0.24524999999999994 * var_chaste_interface__four_AP_sensitive_currents_r_gate__r;
        const double var_x8 = exp(-1.5 - 0.025000000000000001 * var_chaste_interface__membrane__V);
        const double var_x9 = -46.399999999999999 + var_chaste_interface__membrane__V;
        const double var_x10 = -45.0 + var_chaste_interface__membrane__V;
        const double var_x11 = 86.631929397420336 + var_chaste_interface__membrane__V;
        const double var_x12 = var_x11 * var_chaste_interface__rapid_delayed_rectifying_potassium_current_P_i_gate__P_i;
        const double var_x13 = exp(-5.5531135531135529 - 0.18315018315018314 * var_chaste_interface__membrane__V);
        const double var_x14 = 1 / (1.0 + var_x13);
        const double var_x15 = pow(var_x14, 0.33333333333333331);
        const double var_x16 = exp(5.3505781299999997 + 0.082299999999999998 * var_chaste_interface__membrane__V);
        const double var_x17 = exp(-19.034003092000003 - 0.33566000000000001 * var_chaste_interface__membrane__V);
        const double var_x18 = 1 / (0.62739999999999996 * var_x16 + 0.83221659999999997 * var_x17);
        const double var_x19 = 1 / (4.5689999999999998e-5 + 0.00062469999999999995 * var_x18);
        const double var_x20 = exp(-12.936576000000001 - 0.34260000000000002 * var_chaste_interface__membrane__V);
        const double var_x21 = 1.0 + 0.0037320000000000001 * var_x20;
        const double var_x22 = exp(-4.8164649999999991 - 0.28149999999999997 * var_chaste_interface__membrane__V);
        const double var_x23 = var_x22 / var_x21;
        const double var_x24 = exp(10.328124999999998 + 0.15625 * var_chaste_interface__membrane__V);
        const double var_x25 = 1.0 + var_x24;
        const double var_x26 = 1.0 / var_x25;
        const double var_x27 = 1 / (0.00059770000000000005 + 3.7170000000000002e-6 * var_x23);
        const double var_x28 = 0.00015625 * var_x24 / pow(var_x25, 2);
        const double var_x29 = exp(-22.768981 - 0.66830000000000001 * var_chaste_interface__membrane__V);
        const double var_x30 = 1.0 + 7.1890000000000005e-5 * var_x29;
        const double var_x31 = exp(-11.69172 - 0.62190000000000001 * var_chaste_interface__membrane__V);
        const double var_x32 = var_x31 / var_x30;
        const double var_x33 = 1 / (0.0035560000000000001 + 3.1860000000000002e-8 * var_x32);
        const double var_x34 = exp(-3.7166666666666668 + var_x1);
        const double var_x35 = 1.0 + var_x34;
        const double var_x36 = 0.40000000000000002 * var_chaste_interface__membrane__V;
        const double var_x37 = exp(-2.0 + var_x36);
        const double var_x38 = -1.0 + var_x37;
        const double var_x39 = 1 / var_x38;
        const double var_x40 = 5.7149999999999999 * var_x39;
        const double var_x41 = exp(-14.0 - var_x36);
        const double var_x42 = -1.0 + var_x41;
        const double var_x43 = 1 / var_x42;
        const double var_x44 = 14.195 * var_x43;
        const double var_x45 = exp(-0.20799999999999999 * var_chaste_interface__membrane__V);
        const double var_x46 = -1.0 + var_x45;
        const double var_x47 = 1 / var_x46;
        const double var_x48 = 42.450000000000003 * var_x47;
        const double var_x49 = -5.0 + var_chaste_interface__membrane__V;
        const double var_x50 = 35.0 + var_chaste_interface__membrane__V;
        const double var_x51 = exp(9.0 + 0.20000000000000001 * var_chaste_interface__membrane__V);
        const double var_x52 = 1.0 + var_x51;
        const double var_x53 = 0.25 * var_chaste_interface__membrane__V;
        const double var_x54 = exp(7.0 + var_x53);
        const double var_x55 = -1.0 + var_x54;
        const double var_x56 = 1 / var_x55;
        const double var_x57 = 3.125 * var_x56;
        const double var_x58 = exp(-7.0 - var_x53);
        const double var_x59 = 1.0 + var_x58;
        const double var_x60 = 28.0 + var_chaste_interface__membrane__V;
        const double var_x61 = 1 / var_x59;
        const double var_x62 = exp(-5.4411764705882355 - 0.14705882352941177 * var_chaste_interface__membrane__V);
        const double var_x63 = 1.0 + var_x62;
        const double var_x64 = 0.033333333333333333 * var_chaste_interface__membrane__V;
        const double var_x65 = exp(0.87666666666666671 + var_x64);
        const double var_x66 = exp(-0.87666666666666671 - var_x64);
        const double var_x67 = 0.1111111111111111 * var_chaste_interface__membrane__V;
        const double var_x68 = exp(7.8888888888888893 + var_x67);
        const double var_x69 = 1.0 + var_x68;
        const double var_x70 = exp(4.6163849154746419 + 0.065019505851755519 * var_chaste_interface__membrane__V);
        const double var_x71 = exp(-0.85234093637454988 - 0.012004801920768308 * var_chaste_interface__membrane__V);
        const double var_x72 = exp(4.5320610687022898 + 0.076335877862595422 * var_chaste_interface__membrane__V);
        const double var_x73 = 1.0 + var_x72;
        const double var_x74 = exp(-3.1827900000000002 - 0.081610000000000002 * var_chaste_interface__membrane__V);
        const double var_x75 = exp(11.128867 + 0.27189999999999998 * var_chaste_interface__membrane__V);
        const double var_x76 = 1 / (0.56859999999999999 * var_x74 + 0.71740000000000004 * var_x75);
        const double var_x77 = 1 / (0.010103333333333334 + 0.065166666666666664 * var_x76);
        const double var_x78 = exp(0.5548223350253807 - 0.050761421319796954 * var_chaste_interface__membrane__V);
        const double var_x79 = 1.0 + var_x78;
        const double var_x80 = exp(2.7585732000000003 + 0.090120000000000006 * var_chaste_interface__membrane__V);
        const double var_x81 = exp(-2.8369599999999999 - 0.11899999999999999 * var_chaste_interface__membrane__V);
        const double var_x82 = 1 / (1.0369999999999999 * var_x80 + 0.36899999999999999 * var_x81);
        const double var_x83 = 1 / (0.0029775000000000001 + 0.019595000000000001 * var_x82);
        const double var_x84 = exp(-1.3396226415094339 - 0.094339622641509441 * var_chaste_interface__membrane__V);
        const double var_x85 = 1.0 + var_x84;
        const double var_x86 = 1.0 / var_x85;
        const double var_x87 = exp(-0.56603773584905659 + 0.062893081761006289 * var_chaste_interface__membrane__V);
        const double var_x88 = exp(0.40000000000000002 - 0.044444444444444446 * var_chaste_interface__membrane__V);
        const double var_x89 = 9.4339622641509443e-5 * var_x84 / pow(var_x85, 2);
        const double var_x90 = exp(-0.52941176470588236 + 0.058823529411764705 * var_chaste_interface__membrane__V);
        const double var_x91 = exp(0.41666666666666663 - 0.046296296296296294 * var_chaste_interface__membrane__V);
        const double var_x92 = exp(1.8415841584158419 + 0.099009900990099015 * var_chaste_interface__membrane__V);
        const double var_x93 = exp(-0.022222222222222223 * var_chaste_interface__membrane__V);
        const double var_x94 = exp(4.4444444444444446 - var_x67);
        const double var_x95 = 1.0 + var_x94;
        const double var_x96 = exp(3.5355294117647058 + 0.047058823529411764 * var_chaste_interface__membrane__V);
        const double var_x97 = exp(-2.9631993991738641 - 0.037551633496057078 * var_chaste_interface__membrane__V);

        // Matrix entries
        IJth(rJacobian, 0, 0) = mSetVoltageDerivativeToZero ? 0.0 : (-0.0048327079999999989 - 0.05464999999999999 * var_chaste_interface__hyperpolarisation_activated_current_y_gate__y - 0.017225000000000001 * pow(var_chaste_interface__slow_delayed_rectifying_potassium_current_xs_gate__xs, 2.0) - 0.0033227519999999996 * var_chaste_interface__four_AP_sensitive_currents_r_gate__r - 0.0023092690674785805 * exp(var_x0) - 0.00061881194542590091 * exp(-var_x0) - 0.0017381399999999998 / var_x3 - var_x4 * var_chaste_interface__rapid_delayed_rectifying_potassium_current_P_i_gate__P_i - var_x5 * var_chaste_interface__L_type_Ca_channel_d_gate__d_L - var_x6 * var_chaste_interface__T_type_Ca_channel_d_gate__d_T - var_x7 * var_chaste_interface__four_AP_sensitive_currents_q_gate__q - 0.0068989604918571288 * var_x8 / pow((1 + 0.66666666666666663 * var_x8), 2) - 0.00028968999999999995 * var_x2 * var_x9 / pow(var_x3, 2));
        IJth(rJacobian, 1, 0) = 6.105006105006105e-5 * var_x13 * var_x14 * var_x15 * var_x19 - 2.3112969460591235 * (-var_chaste_interface__sodium_current_m_gate__m + 1.0 * var_x15) * (0.27934182395599999 * var_x17 - 0.051635019999999997 * var_x16) / (pow((0.073139106771250198 + var_x18), 2) * pow((var_x17 + 0.75389027327741354 * var_x16), 2));
        IJth(rJacobian, 2, 0) = -var_x27 * var_x28 + 2799.1971566651023 * (var_x26 - var_chaste_interface__sodium_current_h_gate__h1) * (1.0463354999999999e-6 * var_x23 - 4.7524937544000007e-9 * var_x20 * var_x22 / pow(var_x21, 2)) / pow((1 + 0.0062188388823824657 * var_x23), 2);
        IJth(rJacobian, 3, 0) = -var_x28 * var_x33 + 79.081790816642851 * (var_x26 - var_chaste_interface__sodium_current_h_gate__h2) * (1.9813734000000003e-8 * var_x32 - 1.5306846118200001e-12 * var_x29 * var_x31 / pow(var_x30, 2)) / pow((1 + 8.959505061867266e-6 * var_x32), 2);
        IJth(rJacobian, 4, 0) = 0.001 * (-var_chaste_interface__L_type_Ca_channel_d_gate__d_L + 1.0 / var_x35) * (var_x40 - var_x44 - var_x48 - 8.829600000000001 * var_x45 * var_chaste_interface__membrane__V / pow(var_x46, 2) - 2.286 * var_x37 * var_x49 / pow(var_x38, 2) - 5.6780000000000008 * var_x41 * var_x50 / pow(var_x42, 2)) + 0.00016666666666666666 * var_x34 * (var_x40 * var_x49 - var_x44 * var_x50 - var_x48 * var_chaste_interface__membrane__V) / pow(var_x35, 2);
        IJth(rJacobian, 5, 0) = 0.001 * (-var_chaste_interface__L_type_Ca_channel_f_gate__f_L + 1.0 / var_x52) * (var_x57 + 6.25 * var_x58 / pow(var_x59, 2) - 0.78125 * var_x54 * var_x60 / pow(var_x55, 2)) - 0.00020000000000000001 * var_x51 * (25.0 * var_x61 + var_x57 * var_x60) / pow(var_x52, 2);
        IJth(rJacobian, 6, 0) = 0.001 * (-var_chaste_interface__T_type_Ca_channel_d_gate__d_T + 1.0 / var_x63) * (35.600000000000001 * var_x65 - 35.600000000000001 * var_x66) + 0.00014705882352941178 * var_x62 * (1068.0 * var_x65 + 1068.0 * var_x66) / pow(var_x63, 2);
        IJth(rJacobian, 7, 0) = 0.001 * (-var_chaste_interface__T_type_Ca_channel_f_gate__f_T + 1.0 / var_x69) * (0.97529258777633276 * var_x70 - 0.18367346938775511 * var_x71) - 0.0001111111111111111 * var_x68 * (15.0 * var_x70 + 15.300000000000001 * var_x71) / pow(var_x69, 2);
        IJth(rJacobian, 8, 0) = -7.6335877862595422e-5 * var_x72 * var_x77 / pow(var_x73, 2) - 0.029816162611414683 * (-var_chaste_interface__four_AP_sensitive_currents_q_gate__q + 1.0 / var_x73) * (0.046403446000000001 * var_x74 - 0.19506105999999998 * var_x75) / (pow((0.15503836317135553 + var_x76), 2) * pow((var_x75 + 0.79258433231112346 * var_x74), 2));
        IJth(rJacobian, 9, 0) = 5.0761421319796953e-5 * var_x78 * var_x83 / pow(var_x79, 2) - 0.047456665474470604 * (-var_chaste_interface__four_AP_sensitive_currents_r_gate__r + 1.0 / var_x79) * (0.043910999999999999 * var_x81 - 0.09345444 * var_x80) / (pow((0.15195202857871906 + var_x82), 2) * pow((var_x80 + 0.35583413693346194 * var_x81), 2));
        IJth(rJacobian, 10, 0) = var_x89 * (0.95999999999999996 * var_x88 + 37.200000000000003 * var_x87) + 0.001 * (var_x86 - var_chaste_interface__rapid_delayed_rectifying_potassium_current_P_af_gate__P_af) * (2.3396226415094343 * var_x87 - 0.042666666666666665 * var_x88);
        IJth(rJacobian, 11, 0) = var_x89 * (4.2000000000000002 * var_x90 + 0.14999999999999999 * var_x91) + 0.001 * (var_x86 - var_chaste_interface__rapid_delayed_rectifying_potassium_current_P_as_gate__P_as) * (0.24705882352941178 * var_x90 - 0.0069444444444444441 * var_x91);
        IJth(rJacobian, 12, 0) = -0.049504950495049507 * var_x92 / pow((1.0 + var_x92), 2);
        IJth(rJacobian, 13, 0) = 2.2222222222222223e-5 * var_x93 * var_chaste_interface__slow_delayed_rectifying_potassium_current_xs_gate__xs + 0.0015555555555555555 * var_x94 * (1.0 - var_chaste_interface__slow_delayed_rectifying_potassium_current_xs_gate__xs) / pow(var_x95, 2);
        IJth(rJacobian, 14, 0) = -4.7058823529411767e-5 * var_x96 * var_chaste_interface__hyperpolarisation_activated_current_y_gate__y - 3.7551633496057078e-5 * var_x97 * (1.0 - var_chaste_interface__hyperpolarisation_activated_current_y_gate__y);
        IJth(rJacobian, 1, 1) = -0.001 * var_x19;
        IJth(rJacobian, 2, 2) = -0.001 * var_x27;
        IJth(rJacobian, 3, 3) = -0.001 * var_x33;
        IJth(rJacobian, 0, 4) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x5 * var_x9);
        IJth(rJacobian, 4, 4) = 0.014195000000000001 * var_x43 * var_x50 + 0.042450000000000002 * var_x47 * var_chaste_interface__membrane__V - 0.0057149999999999996 * var_x39 * var_x49;
        IJth(rJacobian, 0, 5) = mSetVoltageDerivativeToZero ? 0.0 : (-0.28968999999999995 * var_x9 * var_chaste_interface__L_type_Ca_channel_d_gate__d_L);
        IJth(rJacobian, 5, 5) = -0.025000000000000001 * var_x61 - 0.0031250000000000002 * var_x56 * var_x60;
        IJth(rJacobian, 0, 6) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x10 * var_x6);
        IJth(rJacobian, 6, 6) = -1.0680000000000001 * var_x65 - 1.0680000000000001 * var_x66;
        IJth(rJacobian, 0, 7) = mSetVoltageDerivativeToZero ? 0.0 : (-0.21390299999999998 * var_x10 * var_chaste_interface__T_type_Ca_channel_d_gate__d_T);
        IJth(rJacobian, 7, 7) = -0.014999999999999999 * var_x70 - 0.015300000000000001 * var_x71;
        IJth(rJacobian, 0, 8) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x11 * var_x7);
        IJth(rJacobian, 8, 8) = -0.001 * var_x77;
        IJth(rJacobian, 0, 9) = mSetVoltageDerivativeToZero ? 0.0 : (-0.28785641666913719 - 0.0033227519999999996 * var_chaste_interface__membrane__V - 0.24524999999999994 * var_x11 * var_chaste_interface__four_AP_sensitive_currents_q_gate__q);
        IJth(rJacobian, 9, 9) = -0.001 * var_x83;
        IJth(rJacobian, 0, 10) = mSetVoltageDerivativeToZero ? 0.0 : (-0.023911199999999997 * var_x12);
        IJth(rJacobian, 10, 10) = -0.037200000000000004 * var_x87 - 0.00096000000000000002 * var_x88;
        IJth(rJacobian, 0, 11) = mSetVoltageDerivativeToZero ? 0.0 : (-0.015940800000000001 * var_x12);
        IJth(rJacobian, 11, 11) = -0.0042000000000000006 * var_x90 - 0.00014999999999999999 * var_x91;
        IJth(rJacobian, 0, 12) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x11 * var_x4);
        ...
        IJth(rJacobian, 13, 13) = -0.001 * var_x93 - 0.014 / var_x95;
        IJth(rJacobian, 0, 14) = mSetVoltageDerivativeToZero ? 0.0 : (-0.28582110376219411 - 0.05464999999999999 * var_chaste_interface__membrane__V);
        IJth(rJacobian, 14, 14) = -0.001 * var_x96 - 0.001 * var_x97;
    }
