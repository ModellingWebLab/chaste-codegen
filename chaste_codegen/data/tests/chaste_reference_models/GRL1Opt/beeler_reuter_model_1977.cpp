...
protected:
    ...
    void RegenerateTables()
    {
        ...
        if (mNeedsRegeneration[0])
        {
            ...
            {
                auto f = [](double var_chaste_interface__membrane__V) {
                    return ((fabs(47.0 + var_chaste_interface__membrane__V) < 1.0000000000287557e-6) ? (1.0000000000287557e-6 / (-1.0 + exp(1.0000000000287557e-7)) + 499999.99998562218 * (47.000000999999997 + var_chaste_interface__membrane__V) * (-1.0000000000287557e-6 / (-1.0 + exp(1.0000000000287557e-7)) - 1.0000000000287557e-6 / (-1.0 + exp(-1.0000000000287557e-7)))) : (-(47.0 + var_chaste_interface__membrane__V) / (-1.0 + exp(-4.7000000000000002 - 0.10000000000000001 * var_chaste_interface__membrane__V))));
                };
                const double var_chaste_interface__membrane__V = mTableMins[0] + i*mTableSteps[0];
                double val = f(var_chaste_interface__membrane__V);
                //Expressions which are part of a piecewise could be inf / nan, this is generally accptable, due to the piecewise, however occasionally interpolation of the lookup table from a nan/inf version can give problems.
...
// LCOV_EXCL_STOP
        const double* const _lt_0_row = Cellbeeler_reuter_model_1977FromCellMLGRL1Opt_LookupTables::Instance()->IndexTable0(var_chaste_interface__membrane__V);
            ...
            const double var_x38 = -1.0000000000287557e-6 * var_x27;
            
            partialF = ((var_x32) ? (var_x37 + var_x38 - 499999.99998562218 * (47.000000999999997 + var_chaste_interface__membrane__V) * (var_x38 - 1.0000000000287557e-6 * var_x28)) : (var_x37 + var_x31 * var_x36));
        }
        else
        {
