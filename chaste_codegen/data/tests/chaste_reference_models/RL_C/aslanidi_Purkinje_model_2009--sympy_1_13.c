...
void compute()
{
   // time: time (millisecond)
    ...
    CMDN = 2 * CMDN_max * Y[1] / pow((km_CMDN + Y[1]), 2);
    TRPN = 2 * TRPN_max * Y[1] / pow((km_TRPN + Y[1]), 2);
    b_myo = 1 / (1 + CMDN + TRPN);
    Ca_MK_bound = (1 - Y[24]) * Ca_MK_0 / (1 + km_Ca_MK / Y[27]);
    Ca_MK_act = Ca_MK_bound + Y[24];
    b_SL = 2 * b_SL_max * Y[27] / pow((km_b_SL + Y[27]), 2);
    b_SR = 2 * b_SR_max * Y[27] / pow((km_b_SR + Y[27]), 2);
    Ca_r_tot = 1 / (1 + b_SL + b_SR);
    q_diff = (-Y[1] + Y[27]) / tau_ss;
    dY[24] = -beta_Ca_MK * Y[24] + (Ca_MK_bound + Y[24]) * Ca_MK_bound * alpha_Ca_MK;
    I_ext = I / (Ampl_gain * Cext);
    g_leak = 1 / R_seal;
    tau_f_Ca = 0.5 + 1 / (1 + 333.33333333333331 * Y[27]) + 10 * Ca_MK_act / (1 * Ca_MK_act + km_Ca_MK);
    i_Ca_p = Y[1] * i_Ca_p_max / (Y[1] + km_Ca_p);
    g_Ks = (1 + 0.59999999999999998 / (1 + 6.4818210260626455e-7 * pow((1 / Y[1]), 1.3999999999999999))) * g_Ks_max;
    a_infinity = 1 / (1 + km_to_2 / Y[27]);
    dY[20] = (-Y[20] + a_infinity) / tau_a;
    d_infinity = 1 / (1 + exp(0.59347181008902072 - 0.14836795252225518 * Y[0]));
    tau_d = 0.58999999999999997 + 0.80000000000000004 * exp(0.67599999999999993 + 0.051999999999999998 * Y[0]) / (1 + exp(1.7160000000000002 + 0.13200000000000001 * Y[0]));
    dY[7] = (-Y[7] + d_infinity) / tau_d;
    f2_infinity = 1 / (1 + exp(1.8 + 0.10000000000000001 * Y[0]));
    tau_f2 = 38 + 24.217200000000005 * pow((-1 + 0.053763440860215048 * Y[0]), 2);
    dY[9] = (-Y[9] + f2_infinity) / tau_f2;
    f_infinity = 1 / (1 + exp(1.8 + 0.10000000000000001 * Y[0]));
    tau_f = 4 + 0.03125 * pow((-1 + 0.40000000000000002 * Y[0]), 2);
    dY[8] = (-Y[8] + f_infinity) / tau_f;
    i_Ca_T = (-50 + Y[0]) * Y[12] * Y[13] * g_Ca_T;
    alpha_b = 1.0680000000000001 * exp(0.54333333333333333 + 0.033333333333333333 * Y[0]);
    b_infinity = 1 / (1 + exp(-5.4098360655737707 - 0.16393442622950821 * Y[0]));
    ...
    g_infinity = 1 / (1 + exp(9.0909090909090917 + 0.15151515151515152 * Y[0]));
    tau_g = 1 / (alpha_g + beta_g);
    dY[13] = (-Y[13] + g_infinity) / tau_g;
    kp = 1 / (1 + exp(1.2521739130434781 - 0.16722408026755853 * Y[0]));
    rr_infinity = 1 / (1 + exp(-0.26470588235294124 + 0.049019607843137261 * Y[0]));
    tau_xr = 100 + 900 / (1 + exp(0.20000000000000001 * Y[0]));
    xr_infinity = 1 / (1 + exp(-0.0069387755102040824 - 0.081632653061224483 * Y[0]));
    dY[17] = (-Y[17] + xr_infinity) / tau_xr;
    tau_xs1 = 1 / ((((Y[0] >= 0.54999921875000002) && (Y[0] <= 0.55000078125000007)) ? (-2.8124999999965681e-10 / (-1 + exp(-9.999999999987797e-8)) + 640000.00000042573 * (-0.54999921875000002 + Y[0]) * (2.8124999999996902e-10 / (-1 + exp(9.9999999999988984e-8)) + 2.8124999999965681e-10 / (-1 + exp(-9.999999999987797e-8)))) : (0.00036000000000000002 * (-0.55000000000000004 + Y[0]) / (-1 + exp(-0.070400000000000004 + 0.128 * Y[0])))) + (((Y[0] >= -44.600000010030094) && (Y[0] <= -44.599999989969909)) ? (-7.6328986866047728e-13 / (1 - exp(9.9999999875755032e-8)) + 49850000.061936118 * (44.600000010030094 + Y[0]) * (7.6328986866047728e-13 / (1 - exp(9.9999999875755032e-8)) + 7.6328986866047728e-13 / (1 - exp(-9.9999999875755032e-8)))) : (7.6100000000000007e-5 * (44.600000000000001 + Y[0]) / (1 - exp(-444.66200000000003 - 9.9700000000000006 * Y[0])))));
    xs1_infinity = 1 / (1 + exp(0.65693430656934315 - 0.072992700729927015 * Y[0]));
    dY[18] = (-Y[18] + xs1_infinity) / tau_xs1;
    tau_xs2 = 2 / ((((Y[0] >= 0.54999921875000002) && (Y[0] <= 0.55000078125000007)) ? (-2.8124999999965681e-10 / (-1 + exp(-9.999999999987797e-8)) + 640000.00000042573 * (-0.54999921875000002 + Y[0]) * (2.8124999999996902e-10 / (-1 + exp(9.9999999999988984e-8)) + 2.8124999999965681e-10 / (-1 + exp(-9.999999999987797e-8)))) : (0.00036000000000000002 * (-0.55000000000000004 + Y[0]) / (-1 + exp(-0.070400000000000004 + 0.128 * Y[0])))) + (((Y[0] >= -44.600000010030094) && (Y[0] <= -44.599999989969909)) ? (-7.6328986866047728e-13 / (1 - exp(9.9999999875755032e-8)) + 49850000.061936118 * (44.600000010030094 + Y[0]) * (7.6328986866047728e-13 / (1 - exp(9.9999999875755032e-8)) + 7.6328986866047728e-13 / (1 - exp(-9.9999999875755032e-8)))) : (7.6100000000000007e-5 * (44.600000000000001 + Y[0]) / (1 - exp(-444.66200000000003 - 9.9700000000000006 * Y[0])))));
    xs2_infinity = 1 / (1 + exp(0.65693430656934315 - 0.072992700729927015 * Y[0]));
    dY[19] = (-Y[19] + xs2_infinity) / tau_xs2;
    h_L_infinity = 1 / (1 + exp(11.311475409836067 + 0.16393442622950821 * Y[0]));
    tau_h_L = 175 + 125 / (1 + exp(-4.166666666666667 - 0.16666666666666666 * Y[0]));
    dY[6] = (-Y[6] + h_L_infinity) / tau_h_L;
    if ((Y[0] >= -47.130001) && (Y[0] <= -47.129999000000005))
        alpha_m_L = -3.200000000092018e-7 / (1 - exp(1.0000000000287557e-7)) + 499999.99998562218 * (47.130001 + Y[0]) * (3.200000000092018e-7 / (1 - exp(1.0000000000287557e-7)) + 3.200000000092018e-7 / (1 - exp(-1.0000000000287557e-7)))
    else 
        alpha_m_L = 0.32000000000000001 * (47.130000000000003 + Y[0]) / (1 - exp(-4.7130000000000001 - 0.10000000000000001 * Y[0]))
    ...
    if (Y[0] < -40)
        beta_h = 310000 * exp(0.34999999999999998 * Y[0] - 0.34999999999999998 * shift_INa_inact) + 3.5600000000000001 * exp(0.079000000000000001 * Y[0] - 0.079000000000000001 * shift_INa_inact)
    else 
        beta_h = 7.6923076923076916 / (1 + exp(-0.96036036036036043 + 0.0900900900900901 * shift_INa_inact - 0.0900900900900901 * Y[0]))

    h_infinity = 0.01 * perc_reduced_inact_for_IpNa + (1 - 0.01 * perc_reduced_inact_for_IpNa) * alpha_h / (alpha_h + beta_h);
    tau_h = 1 / (alpha_h + beta_h);
    dY[3] = (-Y[3] + h_infinity) / tau_h;
    if (Y[0] < -40)
        alpha_j = (37.780000000000001 + Y[0]) * (-127140 * exp(0.24440000000000001 * Y[0] - 0.24440000000000001 * shift_INa_inact) - 3.4740000000000003e-5 * exp(0.043909999999999998 * shift_INa_inact - 0.043909999999999998 * Y[0])) / (1 + exp(24.640530000000002 + 0.311 * Y[0] - 0.311 * shift_INa_inact))
    else 
        alpha_j = 0

    if (Y[0] < -40)
        beta_j = 0.1212 * exp(0.01052 * shift_INa_inact - 0.01052 * Y[0]) / (1 + exp(-5.5312920000000005 + 0.13780000000000001 * shift_INa_inact - 0.13780000000000001 * Y[0]))
    else 
        beta_j = 0.29999999999999999 * exp(2.5349999999999999e-7 * shift_INa_inact - 2.5349999999999999e-7 * Y[0]) / (1 + exp(-3.2000000000000002 + 0.10000000000000001 * shift_INa_inact - 0.10000000000000001 * Y[0]))

    j_infinity = 0.01 * perc_reduced_inact_for_IpNa + (1 - 0.01 * perc_reduced_inact_for_IpNa) * alpha_j / (alpha_j + beta_j);
    tau_j = 1 / (alpha_j + beta_j);
    dY[4] = (-Y[4] + j_infinity) / tau_j;
    if ((Y[0] >= -47.130001) && (Y[0] <= -47.129999000000005))
        alpha_m = -3.200000000092018e-7 / (1 - exp(1.0000000000287557e-7)) + 499999.99998562218 * (47.130001 + Y[0]) * (3.200000000092018e-7 / (1 - exp(1.0000000000287557e-7)) + 3.200000000092018e-7 / (1 - exp(-1.0000000000287557e-7)))
    else 
        alpha_m = 0.32000000000000001 * (47.130000000000003 + Y[0]) / (1 - exp(-4.7130000000000001 - 0.10000000000000001 * Y[0]))
    ...
    alpha_a = 25 * exp(-3.7999999999999998 + 0.050000000000000003 * Y[0]) / (1 + exp(-3.7999999999999998 + 0.050000000000000003 * Y[0]));
    beta_a = 25 * exp(-2.7000000000000002 - 0.050000000000000003 * Y[0]) / (1 + exp(-2.7000000000000002 - 0.050000000000000003 * Y[0]));
    a_infinity = alpha_a / (alpha_a + beta_a);
    tau_a = 1 / (alpha_a + beta_a);
    dY[14] = (-Y[14] + a_infinity) / tau_a;
    alpha_i2 = 0.0044200000000000003 / (1 + exp(1.7333333333333334 + 0.066666666666666666 * Y[0]));
    beta_i2 = 0.050000000000000003 * exp(-0.66666666666666663 + 0.066666666666666666 * Y[0]) / (1 + exp(-0.66666666666666663 + 0.066666666666666666 * Y[0]));
    i2_infinity = alpha_i2 / (alpha_i2 + beta_i2);
    tau_i2 = 21.5 + 30 / (1 + exp(-2.5 + 0.10000000000000001 * Y[0]));
    dY[16] = (-Y[16] + i2_infinity) / tau_i2;
    alpha_i = 0.029999999999999999 / (1 + exp(1.6666666666666667 + 0.066666666666666666 * Y[0]));
    beta_i = 0.10000000000000001 * exp(-2.6666666666666665 + 0.066666666666666666 * Y[0]) / (1 + exp(-2.6666666666666665 + 0.066666666666666666 * Y[0]));
    i_infinity = alpha_i / (alpha_i + beta_i);
    tau_i = 6 + 5 / (1 + exp(-1.6499999999999999 + 0.10000000000000001 * Y[0]));
    dY[15] = (-Y[15] + i_infinity) / tau_i;
    if ((Y[0] >= -2.6712338705498262e-6) && (Y[0] <= 2.6712338705498262e-6))
        i_to_2_max = 187179.41753901311 * (2.6712338705498262e-6 + Y[0]) * (3.8593999999999993e-9 * (-100 * exp(9.9999999999999995e-8) + Y[22]) / (1 - exp(9.9999999999999995e-8)) + 3.8593999999999993e-9 * (-100 * exp(-9.9999999999999995e-8) + Y[22]) / (1 - exp(-9.9999999999999995e-8))) - 3.8593999999999993e-9 * (-100 * exp(-9.9999999999999995e-8) + Y[22]) / (1 - exp(-9.9999999999999995e-8))
    else 
        i_to_2_max = 0.0014448004881001341 * (-100 * exp(0.037435883507802616 * Y[0]) + Y[22]) * Y[0] / (1 - exp(0.037435883507802616 * Y[0]))

    i_to_2 = 20 * Y[20] * i_to_2_max;
    ...
    if ((Y[0] >= -1.3356169352749131e-6) && (Y[0] <= 1.3356169352749131e-6))
        i_Ca_b = 374358.83507802623 * (1.3356169352749131e-6 + Y[0]) * (3.8499135947999997e-9 * (-0.34100000000000003 * Ca_o + 1 * Y[1] * exp(9.9999999999999995e-8)) / (-1 + exp(9.9999999999999995e-8)) + 3.8499135947999997e-9 * (-0.34100000000000003 * Ca_o + 1 * Y[1] * exp(-9.9999999999999995e-8)) / (-1 + exp(-9.9999999999999995e-8))) - 3.8499135947999997e-9 * (-0.34100000000000003 * Ca_o + 1 * Y[1] * exp(-9.9999999999999995e-8)) / (-1 + exp(-9.9999999999999995e-8))
    else 
        i_Ca_b = 0.0028824983370007683 * (-0.34100000000000003 * Ca_o + 1 * Y[1] * exp(0.074871767015605231 * Y[0])) * Y[0] / (-1 + exp(0.074871767015605231 * Y[0]))

    if ((Y[0] >= 14.999998664383064) && (Y[0] <= 15.000001335616936))
        i_Ca_L_max = 374358.83507888648 * (-14.999998664383064 + Y[0]) * (4.6891709999892238e-6 * (-0.34100000000000003 * Ca_o + 1 * Y[27] * exp(9.9999999999770198e-8)) / (-1 + exp(9.9999999999770198e-8)) + 4.6891709999892238e-6 * (-0.34100000000000003 * Ca_o + 1 * Y[27] * exp(-9.9999999999770198e-8)) / (-1 + exp(-9.9999999999770198e-8))) - 4.6891709999892238e-6 * (-0.34100000000000003 * Ca_o + 1 * Y[27] * exp(-9.9999999999770198e-8)) / (-1 + exp(-9.9999999999770198e-8))
    else 
        i_Ca_L_max = 3.510865186083326 * (-15 + Y[0]) * (-0.34100000000000003 * Ca_o + 1 * Y[27] * exp(-1.1230765052340785 + 0.074871767015605231 * Y[0])) / (-1 + exp(-1.1230765052340785 + 0.074871767015605231 * Y[0]))

    i_Ca_L = Y[10] * Y[11] * Y[7] * Y[8] * Y[9] * g_Ca_L * i_Ca_L_max;
    f_Ca2_infinity = 1 / (1 - 100 * i_Ca_L);
    tau_f_Ca2 = 125 + 300 / (1 + exp(-4.375 - 25 * i_Ca_L));
    dY[11] = (-Y[11] + f_Ca2_infinity) / tau_f_Ca2;
    f_Ca_infinity = 0.14999999999999999 + 0.55000000000000004 / (1 + 333.33333333333331 * Y[27]) + 0.29999999999999999 / (1 - 20 * i_Ca_L);
    dY[10] = (-Y[10] + f_Ca_infinity) / tau_f_Ca;
    dNaCa_1 = pow(Y[21], 3) * km_Ca_o + 1.5 * pow(km_Na_o, 3) * Y[1] + pow(km_Na_i_1, 3) * (1 + 1.5 * Y[1] / km_Ca_i) * Ca_o;
    g_K1 = 0.43033148291193518 * sqrt(K_o) * g_K1_max;
    g_Kr = 0.43033148291193518 * sqrt(K_o) * g_Kr_max;
    dNaCa_2 = pow(Y[21], 3) * Ca_o + 1.5 * pow(Na_o, 3) * Y[1] + pow(Na_o, 3) * (1 + Y[21] / km_Na_i_1) * km_Ca_i;
    ...
    alpha_xK1 = 1.02 / (1 + exp(-14.1227775 + 0.23849999999999999 * Y[0] - 0.23849999999999999 * E_K));
    beta_xK1 = (0.49124000000000001 * exp(0.43983232 + 0.080320000000000003 * Y[0] - 0.080320000000000003 * E_K) + exp(-36.698642499999998 + 0.061749999999999999 * Y[0] - 0.061749999999999999 * E_K)) / (1 + exp(-2.4444678999999998 + 0.51429999999999998 * E_K - 0.51429999999999998 * Y[0]));
    ...
    i_NaCa = (-1.5 * pow(Na_o, 3) * Y[1] * exp(-0.65000000000000002 * Y[0] * F / (R * T)) + pow(Y[21], 3) * X_NaCa * i_NaCa_max * Ca_o * exp(0.34999999999999998 * Y[0] * F / (R * T))) / ((1 + k_sat * exp(-0.65000000000000002 * Y[0] * F / (R * T))) * (1 + 0.44444444444444442 * pow(km_Ca_act, 2) / pow(Y[1], 2)) * (dNaCa_1 + dNaCa_2));
    f_NaK = 1 / (1 + 0.1245 * exp(-0.10000000000000001 * Y[0] * F / (R * T)) + 0.036499999999999998 * sigma * exp(-Y[0] * F / (R * T)));
    i_NaK = 1 * f_NaK * g_NaK * K_o / ((1 + pow(km_Na_i_2, 2) / pow(Y[21], 2)) * (km_K_o + K_o));
    ...
    dY[23] = (-(-2 * i_NaK + i_K_b + i_K1 + i_K_p + i_Kr + i_Ks + i_to_1) * Cm * a_cap / (F * Vol_myo) + CT_K_Cl) * conc_clamp;
    dY[21] = (-(3 * i_NaCa + 3 * i_NaK + i_Na_b + i_Na + i_Na_L) * Cm * a_cap / (F * Vol_myo) + CT_Na_Cl) * conc_clamp;
    q_leak = Y[25] * q_leak_max / NSR_max;
    vg = 1 / (1 + exp(2.6000000000000001 + 0.20000000000000001 * g_Ca_L * i_Ca_L_max));
    g_rel = g_rel_max * vg;
    Ca_fac = 1 / (1 + exp(3.3333333333333335 + 66.666666666666671 * i_Ca_L));
    ri_infinity = 1 / (1 + exp(-16 + 80 * Ca_fac + 40000 * Y[27]));
    tau_Ca_MK = 1 * Ca_MK_act * tau_Ca_MK_max / (1 * Ca_MK_act + km_Ca_MK);
    tau_ri = 3 + (350 - tau_Ca_MK) / (1 + exp(-15 + 5000 * Y[27] + 15 * Ca_fac)) + tau_Ca_MK;
    ...
    ro_infinity = pow(i_Ca_L, 2) * ro_infinity_JSR / (1 + pow(i_Ca_L, 2));
    dY[29] = (-Y[29] + ro_infinity) / tau_ro;
    q_tr = (-Y[26] + Y[25]) / tau_tr;
    dY[26] = (-q_rel + q_tr) / (1 + CSQN_max * km_CSQN / pow((km_CSQN + Y[26]), 2));
    dkm_plb = 1 * Ca_MK_act * dkm_plb_max / (1 * Ca_MK_act + km_Ca_MK);
    dq_up_Ca_MK = 1 * Ca_MK_act * dq_up_Ca_MK_max / (1 * Ca_MK_act + km_Ca_MK);
    q_up = (1 + dq_up_Ca_MK) * Y[1] * X_q_up * q_up_max / (-dkm_plb + Y[1] + km_up);
    dY[25] = -q_leak - Vol_jsr * q_tr / Vol_nsr + q_up;
    dY[1] = -((-q_leak + q_up) * Vol_nsr / Vol_myo - q_diff * Vol_ss / Vol_myo + 0.5 * (-2 * i_NaCa + i_Ca_b + i_Ca_p) * Cm * a_cap / (F * Vol_myo)) * b_myo;
}
