...
protected:
    ...
    void RegenerateTables()
    {
        ...
        if (mNeedsRegeneration[0])
        {
            ...
            {
                auto f = [](double var_chaste_interface__membrane__V) {
                    return (((var_chaste_interface__membrane__V >= -41.000000999999997) && (var_chaste_interface__membrane__V <= -40.999999000000003)) ? (-0.00020000000000575113 / (1.0 - exp(1.0000000000287557e-7)) + 499999.99998562218 * (41.000000999999997 + var_chaste_interface__membrane__V) * (0.00020000000000575113 / (1.0 - exp(1.0000000000287557e-7)) + 0.00020000000000575113 / (1.0 - exp(-1.0000000000287557e-7)))) : (200.0 * (41.0 + var_chaste_interface__membrane__V) / (1.0 - exp(-4.1000000000000005 - 0.10000000000000001 * var_chaste_interface__membrane__V))));
                };
                const double var_chaste_interface__membrane__V = mTableMins[0] + i*mTableSteps[0];
                double val = f(var_chaste_interface__membrane__V);
                //Expressions which are part of a piecewise could be inf / nan, this is generally accptable, due to the piecewise, however occasionally interpolation of the lookup table from a nan/inf version can give problems.
            ...
            {
                auto f = [](double var_chaste_interface__membrane__V) {
                    return (((var_chaste_interface__membrane__V >= -19.000000400000001) && (var_chaste_interface__membrane__V <= -18.999999599999999)) ? (-1.2000000000345068e-5 / (1.0 - exp(1.0000000000287557e-7)) + 1249999.9999640554 * (19.000000400000001 + var_chaste_interface__membrane__V) * (1.2000000000345068e-5 / (1.0 - exp(1.0000000000287557e-7)) + 1.2000000000345068e-5 / (1.0 - exp(-1.0000000000287557e-7)))) : (30.0 * (19.0 + var_chaste_interface__membrane__V) / (1.0 - exp(-4.75 - 0.25 * var_chaste_interface__membrane__V))));
                };
                const double var_chaste_interface__membrane__V = mTableMins[0] + i*mTableSteps[0];
                double val = f(var_chaste_interface__membrane__V);
                //Expressions which are part of a piecewise could be inf / nan, this is generally accptable, due to the piecewise, however occasionally interpolation of the lookup table from a nan/inf version can give problems.
