...
// LCOV_EXCL_STOP
        ...
        const double var_time_independent_outward_current__i_K1_converted = 100.00000000000001 * ((fabs(23.0 + var_chaste_interface__membrane__V) < 2.5000000007935341e-6) ? ((_lt_0_row[2]) * mParameters[5]) : ((_lt_0_row[3]) * mParameters[5])); // uA_per_cm2
...
// LCOV_EXCL_STOP
    ...
    double Cellbeeler_reuter_model_1977FromCellMLGRL2Opt::EvaluatePartialDerivative0(double var_chaste_interface__environment__time, std::vector<double>& rY, double delta, bool forceNumerical)
    {
        ...
        if (!forceNumerical && this->mUseAnalyticJacobian)
        {
            ...
            partialF = ((fabs(var_x16) < 2.5000000007935341e-6) ? (var_x0 * (var_x15 + (var_x6 - 0.10000000000000001 / (1.0 - exp(1.0000000003174136e-7)) - 0.10000000000000001 / (1.0 - exp(-1.0000000003174136e-7))) * mParameters[5])) : (var_x0 * (var_x15 + (var_x6 - 0.20000000000000001 / var_x18 + 0.0080000000000000002 * var_x16 * var_x17 / pow(var_x18, 2)) * mParameters[5])));
        }
        else
        {
...
// LCOV_EXCL_STOP
        const double* const _lt_0_row = Cellbeeler_reuter_model_1977FromCellMLGRL2Opt_LookupTables::Instance()->IndexTable0(var_chaste_interface__membrane__V);
            ...
            const double var_x28 = 1 / (-1.0 + exp(1.0000000045806701e-7));
            const double var_x29 = _lt_0_row[13];
            const double var_x31 = 47.0 + var_chaste_interface__membrane__V;
            const double var_x32 = fabs(var_x31) < 1.0000000010279564e-6;
            ...
            const double var_x37 = -40.0 * var_x29;
            const double var_x38 = -1.0000000045806701e-6 * var_x28;
            
            partialF = ((var_x32) ? (var_x37 + var_x38 - 499999.99948602181 * (47.000001000000005 + var_chaste_interface__membrane__V) * (var_x38 - 9.9999999747524271e-7 * var_x27)) : (var_x37 + var_x31 * var_x36));
        }
        else
        {
