...
// LCOV_EXCL_STOP
        const double* const _lt_0_row = Cellmahajan_shiferaw_2008FromCellMLGRL2Opt_LookupTables::Instance()->IndexTable0(var_chaste_interface__cell__V);
            ...
            const double var_x155 = fabs(var_x154) < 1.0000000010279564e-6;
            ...
            const double var_x160 = 3.2000000146581445e-7 * var_x151;
            const double var_x161 = -0.080000000000000002 * var_x152;
            
            partialF = ((var_x155) ? (var_x160 + var_x161 - 499999.99948602181 * (47.130001000000007 + var_chaste_interface__cell__V) * (var_x160 + 3.1999999919207769e-7 * var_x150)) : (var_x161 - var_x154 * var_x159));
        }
        else
        {
...
// LCOV_EXCL_STOP
    ...
    double Cellmahajan_shiferaw_2008FromCellMLGRL2Opt::EvaluatePartialDerivative11(double var_chaste_interface__Environment__time, std::vector<double>& rY, double delta, bool forceNumerical)
    {
        ...
        if (!forceNumerical && this->mUseAnalyticJacobian)
        {
            ...
            const double var_x298 = 1 / (-1.0 + exp(-9.9999999951805303e-8));
            const double var_x299 = 1 / (-1.0 + exp(1.0000000020937704e-7));
            const double var_x300 = exp(-0.86099999999999999 - 0.123 * var_chaste_interface__cell__V);
            const double var_x301 = 1.0 - var_x300;
            const double var_x302 = 0.0013799999999999999 / var_x301;
            const double var_x303 = 7.0 + var_chaste_interface__cell__V;
            const double var_x305 = 4.2068965605324136e-10 * var_x299;
            const double var_x306 = var_x302 * var_x303;
            const double var_x307 = -var_x305;
            const double var_x308 = 724999.99941571394 * (9.9999993103448261 + var_chaste_interface__cell__V) * (var_x307 - 4.206896549696637e-10 * var_x298);
            const double var_x310 = 10.0 + var_chaste_interface__cell__V;
            const double var_x311 = fabs(var_x310) < 6.8965517296959433e-7;
            const double var_x312 = 1 / (1.0 - exp(1.0000000000864873e-7));
            const double var_x313 = 1 / (1.0 - exp(-1.0000000000864873e-7));
            const double var_x314 = exp(1.45 + 0.14499999999999999 * var_chaste_interface__cell__V);
            const double var_x315 = -1.0 + var_x314;
            const double var_x316 = 0.00060999999999999997 / var_x315;
            const double var_x318 = 1.1219512196092296e-9 * var_x312;
            const double var_x319 = var_x310 * var_x316;
            const double var_x320 = 614999.99994681031 * (7.0000008130081302 + var_chaste_interface__cell__V) * (var_x318 + 1.1219512196092296e-9 * var_x313);
            const double var_x321 = fabs(var_x303) < 8.1300813015161566e-7;
...
// LCOV_EXCL_STOP
    ...
    double Cellmahajan_shiferaw_2008FromCellMLGRL2Opt::EvaluatePartialDerivative12(double var_chaste_interface__Environment__time, std::vector<double>& rY, double delta, bool forceNumerical)
    {
        ...
        if (!forceNumerical && this->mUseAnalyticJacobian)
        {
            ...
            const double var_x327 = 1 / (-1.0 + exp(9.9999999925515227e-8));
            const double var_x328 = 1 / (-1.0 + exp(-9.9999999925515227e-8));
            const double var_x329 = 1 / (1.0 - exp(2.1542940304186685e-7));
            const double var_x330 = 1 / (1.0 - exp(-2.1542940304186685e-7));
            const double var_x331 = 30.0 + var_chaste_interface__cell__V;
            const double var_x332 = fabs(var_x331) < 1.4556040746072085e-6;
            ...
            const double var_x340 = 1.9068413377354434e-10 * var_x328;
            const double var_x341 = 1.0465793296425829e-10 * var_x329;
            const double var_x342 = ((var_x332) ? (-var_x340 - var_x341 + 343500.00025585521 * (30.000001455604075 + var_chaste_interface__cell__V) * (var_x340 + var_x341 + 1.9068413377354434e-10 * var_x327 + 1.0465793296425829e-10 * var_x330)) : (var_x331 * var_x335 + var_x331 * var_x338));
