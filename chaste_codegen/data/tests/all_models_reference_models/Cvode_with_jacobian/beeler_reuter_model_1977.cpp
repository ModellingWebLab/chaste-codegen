...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellbeeler_reuter_model_1977FromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x15 = -50 + var_chaste_interface__membrane__V;
        const double var_x16 = var_x13 * var_x15;
        const double var_x17 = log(var_chaste_interface__slow_inward_current__Cai);
        const double var_x18 = 0.69291640829471179 - 089999999999999997 * var_chaste_interface__membrane__V - 1.1725829999999999 * var_x17;
        const double var_x19 = 6.9291640829471178e-5 - 9.0000000000000002e-6 * var_chaste_interface__membrane__V - 0001172583 * var_x17;
        ...
        // Matrix entries
        IJth(rJacobian, 0, 0) = mSetVoltageDerivativeToZero ? 0 : (-0030000000000000001 - 070000000000000007 / var_x2 - 089999999999999997 * var_x7 + var_x10 * var_x11 - var_x12 * var_x13 - 056000000000000001 * var_x6 / var_x5 - var_x11 * var_x8 * var_x9 + 0028000000000000004 * var_x1 * (23.0 + var_chaste_interface__membrane__V) / pow(var_x2, 2) - 1.3999999999999999 * (-1.0 + var_x6) * (-040000000000000001 * var_x3 - 080000000000000002 * var_x4) / pow(var_x5, 2));
        ...
        IJth(rJacobian, 4, 0) = -01375 * var_x31 * var_x35 - 029999999999999999 * var_x36 * var_chaste_interface__sodium_current_j_gate__j / pow(var_x37, 2) + 011000000000000001 * var_x31 * var_x32 * var_x34 / pow(var_x33, 2);
        IJth(rJacobian, 5, 0) = 0011864406779661019 * var_x41 * var_chaste_interface__slow_inward_current_d_gate__d - 00095 * var_x42 * var_x46 + 0068394528437724983 * var_x42 * var_x43 * var_x45 / pow(var_x44, 2) + 0035000000000000005 * var_x38 * var_x40 * var_chaste_interface__slow_inward_current_d_gate__d / pow(var_x39, 2);
        IJth(rJacobian, 6, 0) = 00012999999999999999 * var_x50 * var_chaste_interface__slow_inward_current_f_gate__f - 9.6000000000000002e-5 * var_x51 * var_x55 - 0012999999999999999 * var_x47 * var_x49 * var_chaste_interface__slow_inward_current_f_gate__f / pow(var_x48, 2) - 0017991004497751124 * var_x51 * var_x52 * var_x54 / pow(var_x53, 2);
        ...
        IJth(rJacobian, 0, 1) = mSetVoltageDerivativeToZero ? 0 : (-1.1725829999999999 * var_x14);
        IJth(rJacobian, 1, 1) = -070000000000000007 - 0001172583 * var_x14;
        IJth(rJacobian, 0, 2) = mSetVoltageDerivativeToZero ? 0 : (-12.0 * var_x12 * var_x15 * pow(var_chaste_interface__sodium_current_m_gate__m, 2));
        IJth(rJacobian, 2, 2) = -40 * var_x20 + var_x25 * var_x26;
        IJth(rJacobian, 0, 3) = mSetVoltageDerivativeToZero ? 0 : (-var_x16 * var_chaste_interface__sodium_current_j_gate__j);
        IJth(rJacobian, 3, 3) = -0.126 * var_x28 - 1.7 / var_x30;
        IJth(rJacobian, 0, 4) = mSetVoltageDerivativeToZero ? 0 : (-var_x16 * var_chaste_interface__sodium_current_h_gate__h);
        IJth(rJacobian, 4, 4) = -0.29999999999999999 / var_x37 - 055 * var_x35;
        IJth(rJacobian, 0, 5) = mSetVoltageDerivativeToZero ? 0 : (var_x18 * var_chaste_interface__slow_inward_current_f_gate__f);
        IJth(rJacobian, 1, 5) = var_x19 * var_chaste_interface__slow_inward_current_f_gate__f;
        IJth(rJacobian, 5, 5) = -070000000000000007 * var_x41 - 095000000000000001 * var_x46;
        IJth(rJacobian, 0, 6) = mSetVoltageDerivativeToZero ? 0 : (var_x18 * var_chaste_interface__slow_inward_current_d_gate__d);
        IJth(rJacobian, 1, 6) = var_x19 * var_chaste_interface__slow_inward_current_d_gate__d;
        IJth(rJacobian, 6, 6) = -012 * var_x55 - 0064999999999999997 * var_x50;
        IJth(rJacobian, 0, 7) = mSetVoltageDerivativeToZero ? 0 : (-0.80000000000000004 * var_x10);
        IJth(rJacobian, 7, 7) = -00050000000000000001 * var_x64 - 0012999999999999999 * var_x59;
    }
