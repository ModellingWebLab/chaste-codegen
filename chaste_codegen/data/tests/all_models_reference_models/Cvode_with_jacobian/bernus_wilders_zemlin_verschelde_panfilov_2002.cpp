...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellbernus_wilders_zemlin_verschelde_panfilov_2002FromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : NV_Ith_S(rY, 0));
        // Units: millivolt; Initial value: -90.2
        double var_chaste_interface__sodium_current_m_gate__m = NV_Ith_S(rY, 1);
        ...
        const double var_x20 = pow(NV_Ith_S(mParameters, 2), 3) * NV_Ith_S(mParameters, 3) * exp(0.012999999999999999 * var_chaste_interface__membrane__V);
        ...
        const double var_x73 = pow(var_x69, 2);
        ...
        const double var_x84 = 1.0 - var_chaste_interface__transient_outward_current_to_gate__to;
        ...
        const double var_x92 = exp(-0.86099999999999999 - 0.062 * var_chaste_interface__membrane__V);
        ...
        IJth(rJacobian, 1, 0) = (1.0 - var_chaste_interface__sodium_current_m_gate__m) * ((var_x60) ? (0.15999999943156581 * var_x57 + 0.16000000056843419 * var_x58) : (var_x63 - 0.032000000000000001 * var_x59 * var_x61 / pow(var_x62, 2))) + 0.0072727272727272727 * var_x64 * var_chaste_interface__sodium_current_m_gate__m;
        IJth(rJacobian, 2, 0) = var_x72 * (-0.059999999999999998 + 0.059999999999999998 * var_x73) + 0.19929846938775506 * (-2.2400000000000002 * var_x68 * (-0.12 + 0.12 * var_x73) - 2.2400000000000002 * var_x70 * (0.070000000000000007 - 0.070000000000000007 * pow(var_x66, 2)) / pow(var_x67, 2)) * (0.5 - var_chaste_interface__sodium_current_v_gate__v - 0.5 * var_x69) / pow((0.11160714285714285 + var_x71), 2);
        IJth(rJacobian, 3, 0) = (0.75900000000000012 * var_x76 * var_x77 - 0.27800000000000002 * var_x74 * var_x78 / pow(var_x75, 2)) * var_chaste_interface__calcium_current_f_gate__f - 0.0011225490196078432 * var_x79 * (1.0 - var_chaste_interface__calcium_current_f_gate__f) / pow(var_x80, 2);
        IJth(rJacobian, 4, 0) = var_x83 * var_x84 * (5.6119999999999998e-5 - 0.012473299999999998 * var_x85) - var_x88 * (0.0001215 - 1.2732200000000002e-10 * var_x89) * var_chaste_interface__transient_outward_current_to_gate__to + 0.17319999999999999 * var_x81 * var_x84 * var_x90 / pow(var_x82, 2) - 0.16039999999999999 * var_x86 * var_x91 * var_chaste_interface__transient_outward_current_to_gate__to / pow(var_x87, 2);
        IJth(rJacobian, 5, 0) = 0.061255999999999998 * var_x92 * var_x98 / pow(var_x93, 2) + 1.7361111111111111e-5 * (-var_chaste_interface__delayed_rectifier_potassium_current_X_gate__X + 0.98799999999999999 / var_x93) * (77.888800000000003 + 2.1111999999999997 * pow(var_x96, 2) - 80.0 * pow(var_x97, 2) - var_x95 * (-0.32692307692307693 - 0.01282051282051282 * var_chaste_interface__membrane__V)) / pow((0.92500000000000004 + var_x94 + 0.7583333333333333 * var_x96 - 0.16666666666666666 * var_x97), 2);
        IJth(rJacobian, 0, 1) = mSetVoltageDerivativeToZero ? 0.0 : (-3 * var_x2 * var_x56 * pow(var_chaste_interface__sodium_current_m_gate__m, 2));
        IJth(rJacobian, 1, 1) = -((var_x60) ? (-var_x65 + 499999.99948602181 * (47.130001000000007 + var_chaste_interface__membrane__V) * (var_x65 + 3.1999999919207769e-7 * var_x57)) : (var_x59 * var_x63)) - 0.080000000000000002 * var_x64;
