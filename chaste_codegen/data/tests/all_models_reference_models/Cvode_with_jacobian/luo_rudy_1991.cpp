...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellluo_rudy_1991FromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x14 = sqrt(NV_Ith_S(mParameters, 2));
        const double var_x15 = 0.43033148291193518 * var_x14 * NV_Ith_S(mParameters, 6) * var_chaste_interface__time_dependent_potassium_current_X_gate__X;
        const double var_x16 = log(NV_Ith_S(mParameters, 2) / NV_Ith_S(mParameters, 0));
        ...
        const double var_x29 = var_x14 * var_x20 * NV_Ith_S(mParameters, 10);
        const double var_x30 = -26.712449447891164 * log((0.018329999999999999 * NV_Ith_S(mParameters, 3) + NV_Ith_S(mParameters, 2)) / (0.018329999999999999 * NV_Ith_S(mParameters, 1) + NV_Ith_S(mParameters, 0))) + var_chaste_interface__membrane__V;
        const double var_x31 = var_x18 / pow(var_x19, 2);
        const double var_x32 = 1 / NV_Ith_S(mParameters, 5);
        const double var_x33 = var_x32;
        const double var_x34 = var_x3 / var_chaste_interface__intracellular_calcium_concentration__Cai;
        const double var_x35 = -26.712449447891164 * log(NV_Ith_S(mParameters, 3) / NV_Ith_S(mParameters, 1)) + var_chaste_interface__membrane__V;
        const double var_x36 = NV_Ith_S(mParameters, 7) * var_chaste_interface__fast_sodium_current_h_gate__h;
        const double var_x37 = var_x33 * var_x35;
        const double var_x38 = log(var_chaste_interface__intracellular_calcium_concentration__Cai);
        const double var_x39 = var_x33 * (-7.7000000000000002 + 13.028700000000001 * var_x38 + var_chaste_interface__membrane__V);
        const double var_x40 = NV_Ith_S(mParameters, 4) * var_chaste_interface__slow_inward_current_d_gate__d;
        const double var_x41 = 0.00077000000000000007 - 0.0001 * var_chaste_interface__membrane__V - 0.00130287 * var_x38;
        const double var_x42 = 1.0 - var_chaste_interface__fast_sodium_current_m_gate__m;
        ...
        const double var_x49 = exp(-11.764705882352942 + 0.14705882352941177 * NV_Ith_S(mParameters, 9) - 0.14705882352941177 * var_chaste_interface__membrane__V);
        ...
        const double var_x53 = exp(0.34999999999999998 * var_chaste_interface__membrane__V - 0.34999999999999998 * NV_Ith_S(mParameters, 9));
        const double var_x54 = exp(0.079000000000000001 * var_chaste_interface__membrane__V - 0.079000000000000001 * NV_Ith_S(mParameters, 9));
        const double var_x55 = exp(-0.96036036036036043 + 0.0900900900900901 * NV_Ith_S(mParameters, 9) - 0.0900900900900901 * var_chaste_interface__membrane__V);
        ...
        const double var_x59 = ((var_x50) ? (-0.019852941176470591 * var_x49) : (0));
        const double var_x60 = var_x51 + var_x57;
        const double var_x61 = 0.01 * NV_Ith_S(mParameters, 8);
        const double var_x62 = 1.0 - var_x61;
        const double var_x63 = var_x62 / var_x60;
        const double var_x64 = ((var_x50) ? (0.28123999999999999 * var_x54 + 108500.0 * var_x53) : (0.693000693000693 * var_x55 / pow(var_x56, 2)));
        const double var_x65 = exp(-5.5312920000000005 + 0.13780000000000001 * NV_Ith_S(mParameters, 9) - 0.13780000000000001 * var_chaste_interface__membrane__V);
        const double var_x66 = 1.0 + var_x65;
        const double var_x67 = exp(0.01052 * NV_Ith_S(mParameters, 9) - 0.01052 * var_chaste_interface__membrane__V);
        const double var_x68 = var_x67 / var_x66;
        const double var_x69 = exp(-3.2000000000000002 + var_x43 + 0.10000000000000001 * NV_Ith_S(mParameters, 9));
        const double var_x70 = 1.0 + var_x69;
        const double var_x71 = exp(2.5349999999999999e-7 * NV_Ith_S(mParameters, 9) - 2.5349999999999999e-7 * var_chaste_interface__membrane__V);
        ...
        const double var_x76 = exp(24.640530000000002 + 0.311 * var_chaste_interface__membrane__V - 0.311 * NV_Ith_S(mParameters, 9));
        const double var_x77 = 1.0 + var_x76;
        const double var_x78 = 1 / var_x77;
        const double var_x79 = exp(0.24440000000000001 * var_chaste_interface__membrane__V - 0.24440000000000001 * NV_Ith_S(mParameters, 9));
        const double var_x80 = exp(0.043909999999999998 * NV_Ith_S(mParameters, 9) - 0.043909999999999998 * var_chaste_interface__membrane__V);
        ...
        const double var_x85 = ((var_x50) ? (var_x82 + var_x75 * var_x78 * (1.5254334000000001e-6 * var_x80 - 31073.016 * var_x79) - 0.311 * var_x75 * var_x76 * var_x81 / pow(var_x77, 2)) : (0));
        const double var_x86 = var_x73 + var_x83;
        const double var_x87 = var_x62 / var_x86;
        const double var_x88 = ((var_x50) ? (-0.001275024 * var_x68 + 0.016701360000000002 * var_x65 * var_x67 / pow(var_x66, 2)) : (-7.6049999999999996e-8 * var_x72 + 0.029999999999999999 * var_x69 * var_x71 / pow(var_x70, 2)));
        ...
        const double var_x93 = 1.0 - var_chaste_interface__slow_inward_current_d_gate__d;
        ...
        const double var_x102 = 1.0 - var_chaste_interface__slow_inward_current_f_gate__f;
        ...
        const double var_x111 = 1.0 - var_chaste_interface__time_dependent_potassium_current_X_gate__X;
        ...
        // Matrix entries
        IJth(rJacobian, 0, 0) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x33 * (var_x3 + var_x13 * var_x15 + var_x5 * var_chaste_interface__fast_sodium_current_h_gate__h + NV_Ith_S(mParameters, 12) / var_x1 + 0.4389381125701739 * var_x28 * var_x29 + var_x15 * var_x30 * ((var_x12) ? (-0.11348000000000001 * var_x11 + 0.11348000000000001 * var_x10 * var_x7 - 0.0004784955304435824 * var_x8 * var_x9 / pow((1 + 0.012987012987012988 * var_chaste_interface__membrane__V), 2)) : (0)) + 0.42189361069797565 * var_x17 * var_x29 * (0.24326999999999999 * var_x31 - var_x23 * (0.061749999999999999 * var_x24 + 0.039456396800000001 * var_x25) - 0.51429999999999998 * var_x21 * var_x26 / pow(var_x22, 2)) / pow((var_x20 + 0.98039215686274506 * var_x27), 2) + 0.16722408026755853 * var_x0 * var_x17 * NV_Ith_S(mParameters, 12) / pow(var_x1, 2) - 0.10468673984798647 * var_x14 * var_x17 * var_x28 * var_x31 * NV_Ith_S(mParameters, 10) + NV_Ith_S(mParameters, 11)));
        IJth(rJacobian, 1, 0) = -0.0001 * var_x3;
        IJth(rJacobian, 2, 0) = var_x42 * var_x46 + 0.0072727272727272727 * var_x47 * var_chaste_interface__fast_sodium_current_m_gate__m - 0.032000000000000001 * var_x42 * var_x44 * var_x48 / pow(var_x45, 2);
        IJth(rJacobian, 3, 0) = (var_x52 + var_x58) * (var_x59 * var_x63 + var_x51 * var_x62 * (-var_x59 - var_x64) / pow(var_x60, 2)) + (var_x59 + var_x64) * (var_x61 - var_chaste_interface__fast_sodium_current_h_gate__h + var_x51 * var_x63);
        IJth(rJacobian, 4, 0) = (var_x74 + var_x84) * (var_x85 * var_x87 + var_x62 * var_x83 * (-var_x85 - var_x88) / pow(var_x86, 2)) + (var_x85 + var_x88) * (var_x61 - var_chaste_interface__fast_sodium_current_j_gate__j + var_x83 * var_x87);
        IJth(rJacobian, 5, 0) = 0.0011900000000000003 * var_x92 * var_chaste_interface__slow_inward_current_d_gate__d - 0.00095 * var_x93 * var_x97 + 0.0068399999999999997 * var_x93 * var_x94 * var_x96 / pow(var_x95, 2) + 0.0035000000000000005 * var_x89 * var_x91 * var_chaste_interface__slow_inward_current_d_gate__d / pow(var_x90, 2);
        IJth(rJacobian, 6, 0) = 0.00012999999999999999 * var_x101 * var_chaste_interface__slow_inward_current_f_gate__f - 9.6000000000000002e-5 * var_x102 * var_x106 - 0.0018 * var_x102 * var_x103 * var_x105 / pow(var_x104, 2) - 0.0012999999999999999 * var_x100 * var_x98 * var_chaste_interface__slow_inward_current_f_gate__f / pow(var_x99, 2);
        IJth(rJacobian, 7, 0) = 7.7999999999999999e-5 * var_x110 * var_chaste_interface__time_dependent_potassium_current_X_gate__X + 4.1500000000000006e-5 * var_x111 * var_x115 - 5.1999999999999997e-5 * var_x107 * var_x109 * var_chaste_interface__time_dependent_potassium_current_X_gate__X / pow(var_x108, 2) - 2.8500000000000002e-5 * var_x111 * var_x112 * var_x114 / pow(var_x113, 2);
        IJth(rJacobian, 0, 1) = mSetVoltageDerivativeToZero ? 0.0 : (-13.028700000000001 * var_x32 * var_x34);
        IJth(rJacobian, 1, 1) = -0.070000000000000007 - 0.00130287 * var_x34;
        IJth(rJacobian, 0, 2) = mSetVoltageDerivativeToZero ? 0.0 : (-3.0 * var_x32 * var_x35 * var_x36 * pow(var_chaste_interface__fast_sodium_current_m_gate__m, 2) * var_chaste_interface__fast_sodium_current_j_gate__j);
        IJth(rJacobian, 2, 2) = -0.080000000000000002 * var_x47 - var_x46 * var_x48;
        IJth(rJacobian, 0, 3) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x37 * var_x5);
        IJth(rJacobian, 3, 3) = -var_x52 - var_x58;
        IJth(rJacobian, 0, 4) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x36 * var_x37 * var_x4);
        IJth(rJacobian, 4, 4) = -var_x74 - var_x84;
        IJth(rJacobian, 0, 5) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x2 * var_x39);
        IJth(rJacobian, 1, 5) = var_x2 * var_x41;
        IJth(rJacobian, 5, 5) = -0.070000000000000007 * var_x92 - 0.095000000000000001 * var_x97;
        IJth(rJacobian, 0, 6) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x39 * var_x40);
        IJth(rJacobian, 1, 6) = var_x40 * var_x41;
        IJth(rJacobian, 6, 6) = -0.012 * var_x106 - 0.0064999999999999997 * var_x101;
        IJth(rJacobian, 0, 7) = mSetVoltageDerivativeToZero ? 0.0 : (-0.43033148291193518 * var_x13 * var_x14 * var_x30 * var_x32 * NV_Ith_S(mParameters, 6));
        IJth(rJacobian, 7, 7) = -0.00050000000000000001 * var_x115 - 0.0012999999999999999 * var_x110;
    }
