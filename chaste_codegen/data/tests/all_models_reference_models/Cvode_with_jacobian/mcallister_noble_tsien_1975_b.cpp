...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellmcallister_noble_tsien_1975_bFromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x17 = pow(var_chaste_interface__fast_sodium_current_m_gate__m, 3) * NV_Ith_S(mParameters, 2);
        ...
        const double var_x37 = 1.0 - var_chaste_interface__secondary_inward_current_d_gate__d;
        ...
        const double var_x61 = 1.0 - var_chaste_interface__plateau_potassium_current1_x1_gate__x1;
        ...
        const double var_x66 = var_x57 * var_x59 / pow(var_x58, 2);
        ...
        IJth(rJacobian, 3, 0) = var_x37 * var_x41 + 0.001776 * var_x43 * var_chaste_interface__secondary_inward_current_d_gate__d - 0.00020000000000000001 * var_x37 * var_x39 * var_x44 / pow(var_x40, 2);
        IJth(rJacobian, 4, 0) = -0.040000000000000001 * var_x45 * (0.00098700000000000003 - 0.00098700000000000003 * var_chaste_interface__secondary_inward_current_f_gate__f) - 0.00174 * var_x46 * var_chaste_interface__secondary_inward_current_f_gate__f / pow(var_x47, 2);
        IJth(rJacobian, 5, 0) = (1.0 - var_chaste_interface__pacemaker_potassium_current_s_gate__s) * ((var_x51) ? (0.00049999999289457265 * var_x48 + 0.00050000000710542737 * var_x49) : (var_x54 - 0.00020000000000000001 * var_x50 * var_x52 / pow(var_x53, 2))) + 3.3500000000000005e-6 * var_x55 * var_chaste_interface__pacemaker_potassium_current_s_gate__s;
        IJth(rJacobian, 6, 0) = 7.7984403119376114e-5 * var_x60 * var_chaste_interface__plateau_potassium_current1_x1_gate__x1 + 4.132231404958678e-5 * var_x61 * var_x65 - 5.1999999999999997e-5 * var_x66 * var_chaste_interface__plateau_potassium_current1_x1_gate__x1 - 2.8571428571428571e-5 * var_x61 * var_x62 * var_x64 / pow(var_x63, 2);
        IJth(rJacobian, 7, 0) = 1.7996400719856025e-5 * var_x60 * var_chaste_interface__plateau_potassium_current2_x2_gate__x2 - 1.1999999999999999e-5 * var_x66 * var_chaste_interface__plateau_potassium_current2_x2_gate__x2 + 2.5400000000000001e-5 * var_x67 * (1.0 - var_chaste_interface__plateau_potassium_current2_x2_gate__x2) / pow(var_x68, 2);
        IJth(rJacobian, 8, 0) = (1.0 - var_chaste_interface__transient_chloride_current_q_gate__q) * ((var_x71) ? (0.0040000000000000001 * var_x69 + 0.0040000000000000001 * var_x70) : (var_x74 - 0.00080000000000000004 * var_x72 * var_chaste_interface__membrane__V / pow(var_x73, 2))) + 0.0071040000000000001 * var_x75 * var_chaste_interface__transient_chloride_current_q_gate__q;
        IJth(rJacobian, 9, 0) = -0.058823529411764705 * var_x77 * (3.3000000000000003e-5 - 3.3000000000000003e-5 * var_chaste_interface__transient_chloride_current_r_gate__r) - 0.0041250000000000002 * var_x78 * var_chaste_interface__transient_chloride_current_r_gate__r / pow(var_x79, 2);
        ...
        IJth(rJacobian, 1, 1) = -((var_x28) ? (-var_x33 + 500000.00126237865 * (47.000000999999997 + var_chaste_interface__membrane__V) * (var_x33 + 9.9999999747524271e-7 * var_x26)) : (var_x27 * var_x31)) - 40.0 * var_x32;
        IJth(rJacobian, 0, 2) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x17 * var_x22);
        ...
        IJth(rJacobian, 3, 3) = -0.02 * var_x43 - var_x41 * var_x44;
        IJth(rJacobian, 0, 4) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x23 * var_chaste_interface__secondary_inward_current_d_gate__d);
        IJth(rJacobian, 4, 4) = -0.00098700000000000003 * var_x45 - 0.02 / var_x47;
        IJth(rJacobian, 0, 5) = mSetVoltageDerivativeToZero ? 0.0 : (-2.7999999999999998 * var_x0 * var_x5 * var_x8);
        IJth(rJacobian, 5, 5) = -((var_x51) ? (-var_x56 + 1000000.0025247573 * (52.000000500000006 + var_chaste_interface__membrane__V) * (var_x56 + 4.9999999163219401e-10 * var_x48)) : (var_x50 * var_x54)) - 5.0000000000000002e-5 * var_x55;
        IJth(rJacobian, 0, 6) = mSetVoltageDerivativeToZero ? 0.0 : (-1.2 * var_x0 * var_x20);
        IJth(rJacobian, 6, 6) = -0.00050000000000000001 * var_x65 - 0.0012999999999999999 * var_x60;
        ...
        IJth(rJacobian, 7, 7) = -0.000127 / var_x68 - 0.00029999999999999997 * var_x60;
        IJth(rJacobian, 0, 8) = mSetVoltageDerivativeToZero ? 0.0 : (var_x24 * var_chaste_interface__transient_chloride_current_r_gate__r);
        IJth(rJacobian, 8, 8) = -((var_x71) ? (-var_x76 + 500000.0 * (9.9999999999999995e-7 + var_chaste_interface__membrane__V) * (var_x76 + 8.0000000000000005e-9 * var_x70)) : (var_x74 * var_chaste_interface__membrane__V)) - 0.080000000000000002 * var_x75;
        IJth(rJacobian, 0, 9) = mSetVoltageDerivativeToZero ? 0.0 : (var_x24 * var_chaste_interface__transient_chloride_current_q_gate__q);
        IJth(rJacobian, 9, 9) = -0.033000000000000002 / var_x79 - 3.3000000000000003e-5 * var_x77;
    }
