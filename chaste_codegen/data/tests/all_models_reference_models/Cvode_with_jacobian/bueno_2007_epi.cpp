...
#include "MathsCustomFunctions.hpp"
    ...
    boost::shared_ptr<RegularStimulus> Cellbueno_2007_epiFromCellMLCvode::UseCellMLDefaultStimulus()
    {
    ...
    Cellbueno_2007_epiFromCellMLCvode::Cellbueno_2007_epiFromCellMLCvode(boost::shared_ptr<AbstractIvpOdeSolver> pOdeSolver /* unused; should be empty */, boost::shared_ptr<AbstractStimulusFunction> pIntracellularStimulus)
        : AbstractCvodeCell(
                pOdeSolver,
                4,
                0,
                pIntracellularStimulus)
    {
        ...
        //
        this->mpSystemInfo = OdeSystemInformation<Cellbueno_2007_epiFromCellMLCvode>::Instance();
        Init();
    ...
    Cellbueno_2007_epiFromCellMLCvode::~Cellbueno_2007_epiFromCellMLCvode()
    {
    }

    
    double Cellbueno_2007_epiFromCellMLCvode::GetIIonic(const std::vector<double>* pStateVariables)
    {
    ...
    void Cellbueno_2007_epiFromCellMLCvode::EvaluateYDerivatives(double var_chaste_interface__environment__time, const N_Vector rY, N_Vector rDY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : NV_Ith_S(rY, 0));
    ...
    void Cellbueno_2007_epiFromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : NV_Ith_S(rY, 0));
        // Units: mV; Initial value: 0.0
        double var_chaste_interface__fast_inward_current_v_gate__v = NV_Ith_S(rY, 1);
        ...
        const double var_x10 = 0.5298013245033113 * var_x2 * var_x8;
        ...
        // Matrix entries
        IJth(rJacobian, 0, 0) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x9 * (var_x5 * var_x6 + var_x5 * (-2.7272727272727271 + var_x3) + (1.0 - var_x2) / (400.0 - 394.0 * var_x1) + 0.0041586275611754688 * var_x2 * (29.687012959999997 - 29.687012959999997 * pow(var_x7, 2)) / pow((1 - 0.93578987418504023 * var_x7), 2)));
        IJth(rJacobian, 2, 0) = var_x13 * (-14.285714285714285 + 14.285714285714285 * var_x1) + 0.00071111111111111115 * var_x12 * (1462.5 - 1462.5 * pow(var_x11, 2)) * (-var_chaste_interface__slow_inward_current_w_gate__w + 0.93999999999999995 * var_x1 + var_x12 * (1.0 - 14.285714285714285 * var_chaste_interface__membrane__V)) / pow((1 - 0.59999999999999998 * var_x11), 2);
        ...
        IJth(rJacobian, 0, 1) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x4 * var_x6 * var_x9 * (-0.29999999999999999 + var_x0));
        IJth(rJacobian, 1, 1) = -0.68936991589687036 * var_x4 - (1.0 - var_x4) / (60.0 + 1090.0 * var_x1);
        IJth(rJacobian, 0, 2) = mSetVoltageDerivativeToZero ? 0.0 : (var_x10 * var_chaste_interface__slow_inward_current_s_gate__s);
        IJth(rJacobian, 2, 2) = -var_x13 - 0.0050000000000000001 * var_x1;
    ...
    N_Vector Cellbueno_2007_epiFromCellMLCvode::ComputeDerivedQuantities(double var_chaste_interface__environment__time, const N_Vector & rY)
    {
        // Inputs:
        // Time units: millisecond
...
template<>
void OdeSystemInformation<Cellbueno_2007_epiFromCellMLCvode>::Initialise(void)
{
