...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellnoble_model_1991FromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time_converted, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x57 = var_x48 * (0050000000000000001 * var_chaste_interface__membrane__V - 0.13356880329847826 * var_x2);
        ...
        const double var_x90 = 0030000000000000001 * var_chaste_interface__L_type_Ca_channel_d_gate__d;
        ...
        IJth(rJacobian, 2, 0) = (001 - 001 * var_chaste_interface__fast_sodium_current_m_gate__m) * ((var_x81) ? (0) : (var_x85 - 20 * var_x80 * var_x83 / pow(var_x84, 2))) + 0.44800000000000001 * var_x86 * var_chaste_interface__fast_sodium_current_m_gate__m;
        IJth(rJacobian, 3, 0) = -0.125 * var_x88 * (02 - 02 * var_chaste_interface__fast_sodium_current_h_gate__h) - 00062500000000000001 * var_x89 * var_chaste_interface__fast_sodium_current_h_gate__h / pow((0031250000000000002 + var_x89), 2);
        IJth(rJacobian, 4, 0) = (0030000000000000001 - var_x90) * ((var_x92) ? (0) : (var_x97 - 7.5 * var_x91 * var_x95 / pow(var_x96, 2))) - var_x90 * ((var_x92) ? (0) : (var_x100 - 1.2 * var_x91 * var_x98 / pow(var_x99, 2)));
        IJth(rJacobian, 5, 0) = (00050000000000000001 - 00050000000000000001 * var_chaste_interface__L_type_Ca_channel_f_gate__f) * ((var_x102) ? (0) : (var_x105 - 1.5625 * var_x101 * var_x103 / pow(var_x104, 2))) - 0062500000000000003 * var_x106 * var_chaste_interface__L_type_Ca_channel_f_gate__f / pow(var_x107, 2);
        IJth(rJacobian, 6, 0) = -058823529411764705 * var_x108 * (3.3000000000000003e-5 - 3.3000000000000003e-5 * var_chaste_interface__transient_outward_current_s_gate__s) - 0041250000000000002 * var_x109 * var_chaste_interface__transient_outward_current_s_gate__s / pow(var_x110, 2);
        IJth(rJacobian, 7, 0) = 066600000000000006 * var_x111 / pow((1.0 + var_x111), 2);
        IJth(rJacobian, 10, 0) = -3.7909647610109532e-7 + 2.2134435192462872e-9 * var_x46 - 9.477411902527384e-7 * var_x14 - 3.0988209269448018e-7 * var_x42 - 0015795686504212307 * var_x7 - var_x120 * var_x31;
        IJth(rJacobian, 11, 0) = -3.7909647610109532e-7 + 1.3515291521577086e-5 * var_x19 + 4.4268870384925742e-10 * var_x45 - 6.7576457607885431e-7 * var_x18 - 3.1591373008424612e-6 * var_x15 - 00018052213147671208 * var_x5 - 1.7707548153970297e-9 * var_x42 - var_x121 * var_x31;
        ...
        IJth(rJacobian, 0, 1) = mSetVoltageDerivativeToZero ? 0 : (-var_x48 * (0071428571428571426 * var_chaste_interface__intracellular_potassium_concentration__K_i - 028571428571428571 * var_x17));
        IJth(rJacobian, 1, 1) = -00050000000000000001 * var_x79 - 0013000000000000002 * var_x74;
        ...
        IJth(rJacobian, 0, 2) = mSetVoltageDerivativeToZero ? 0 : (-0074999999999999997 * var_x47 * var_x51);
        IJth(rJacobian, 2, 2) = -001 * ((var_x81) ? (2000) : (var_x80 * var_x85)) - 8.0 * var_x86;
        IJth(rJacobian, 10, 2) = -0047387059512636921 * var_x51;
        IJth(rJacobian, 0, 3) = mSetVoltageDerivativeToZero ? 0 : (-0025000000000000001 * var_x47 * var_x52);
        IJth(rJacobian, 3, 3) = -02 * var_x88 - 2.0 / (1.0 + 320 * var_x89);
        IJth(rJacobian, 10, 3) = -0015795686504212307 * var_x52;
        IJth(rJacobian, 0, 4) = mSetVoltageDerivativeToZero ? 0 : (-var_x48 * (var_x27 * var_x53 + var_x34 * var_x54 + var_x36 * var_x54));
        IJth(rJacobian, 4, 4) = -0030000000000000001 * ((var_x92) ? (120) : (var_x100 * var_x91)) - 0030000000000000001 * ((var_x92) ? (120) : (var_x91 * var_x97));
        IJth(rJacobian, 10, 4) = -var_x120 * var_x54;
        IJth(rJacobian, 11, 4) = -var_x121 * var_x54;
        IJth(rJacobian, 12, 4) = -var_x123 * var_x53;
        IJth(rJacobian, 0, 5) = mSetVoltageDerivativeToZero ? 0 : (-var_x48 * (var_x27 * var_x55 + var_x34 * var_x56 + var_x36 * var_x56));
        IJth(rJacobian, 5, 5) = -00050000000000000001 * ((var_x102) ? (25.0) : (var_x101 * var_x105)) - 025000000000000001 / var_x107;
        IJth(rJacobian, 10, 5) = -var_x120 * var_x56;
        IJth(rJacobian, 11, 5) = -var_x121 * var_x56;
        IJth(rJacobian, 12, 5) = -var_x123 * var_x55;
        IJth(rJacobian, 0, 6) = mSetVoltageDerivativeToZero ? 0 : (-var_x57 * var_chaste_interface__transient_outward_current_r_gate__r);
        IJth(rJacobian, 6, 6) = -033000000000000002 / var_x110 - 3.3000000000000003e-5 * var_x108;
        IJth(rJacobian, 11, 6) = var_x122 * var_chaste_interface__transient_outward_current_r_gate__r;
        IJth(rJacobian, 0, 7) = mSetVoltageDerivativeToZero ? 0 : (-var_x57 * var_chaste_interface__transient_outward_current_s_gate__s);
        ...
        IJth(rJacobian, 8, 8) = -059999999999999998 - 1.0 * var_x115;
        IJth(rJacobian, 9, 8) = 059999999999999998 + var_x116;
        ...
        IJth(rJacobian, 0, 10) = mSetVoltageDerivativeToZero ? 0 : (-var_x48 * (0030000000000000001 * var_x61 + 0.55999999999999994 * var_x58 + 9381.8727436851113 * var_x62 + 016028256395817387 * var_x59 + 9.3584727056863679e-5 * var_x63 - 00035 * var_x60));
        IJth(rJacobian, 10, 10) = 6.634188331769168e-7 * var_x60 - 1.0127092529498693e-5 * var_x59 - 5.9129400406899753e-8 * var_x63 - 0010614701330830668 * var_x58 - 5.9277248272665686 * var_x62 - 5.6864471415164302e-6 * var_x61;
        IJth(rJacobian, 11, 10) = 00070764675538871125 * var_x58 - 4.4227922211794452e-7 * var_x60;
        IJth(rJacobian, 12, 10) = 1.8954823805054768e-6 * var_x61;
        IJth(rJacobian, 0, 11) = mSetVoltageDerivativeToZero ? 0 : (-var_x48 * (1.8716945411372735e-5 * var_x63 + 0.13356880329847826 * var_x65 + 016028256395817387 * var_x1 + 1125.8247292422134 * var_x62 + 0071428571428571426 * var_chaste_interface__time_dependent_potassium_current_x_gate__x + 7.6325030456273284 * var_x64 - 0.5714285714285714 * var_x66));
        IJth(rJacobian, 10, 11) = -0.71132697927198818 * var_x62;
        IJth(rJacobian, 11, 11) = 00036104426295342415 * var_x66 - 004822425014046997 * var_x64 - 1.0127092529498693e-5 * var_x1 - 4.5130532869178019e-6 * var_chaste_interface__time_dependent_potassium_current_x_gate__x - 8.4392437745822447e-5 * var_x65 - 1.1825880081379952e-8 * var_x63;
        IJth(rJacobian, 0, 12) = mSetVoltageDerivativeToZero ? 0 : (-var_x48 * (037433890822745473 * var_x70 + 0033392200824619565 * var_x67 - 1372.0 * var_x68 - 3.45e-6 * var_x69));
        ...
        IJth(rJacobian, 12, 12) = -7.0000000000000009 + var_x130 + var_x131 + 00040000000000000002 * var_x135 - 0.86686727535117136 * var_x68 - 1.1825880081379951e-5 * var_x70 - 7.2e-9 * var_x136 - 1.0549054718227806e-6 * var_x67 - 2.1798047375812981e-9 * var_x69 - 00040000000000000002 * var_x133;
        IJth(rJacobian, 13, 12) = 019599999999999999 * var_x133 + 3.5279999999999993e-7 * var_x136 - 019599999999999999 * var_x135;
        ...
        IJth(rJacobian, 13, 13) = -050000000000000003 + 8.4671999999999982e-11 * var_x136 - 3.5279999999999993e-7 * var_x133 - 4.7040000000000002e-6 * var_x135;
        IJth(rJacobian, 14, 13) = 0050000000000000001;
        IJth(rJacobian, 12, 14) = 051020408163265314 * var_x137;
        IJth(rJacobian, 13, 14) = 050000000000000003;
        IJth(rJacobian, 14, 14) = -0050000000000000001 - 0.25 * var_x137;
        IJth(rJacobian, 12, 15) = 050000000000000003 + var_x138;
        IJth(rJacobian, 15, 15) = -050000000000000003 + var_x140;
