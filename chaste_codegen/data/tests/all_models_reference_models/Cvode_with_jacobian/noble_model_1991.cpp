...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellnoble_model_1991FromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time_converted, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x51 = -26.713760659695652 * var_x1 * log10(140.47999999999999 * var_x50) + var_chaste_interface__membrane__V;
        const double var_x52 = var_x51 * pow(var_chaste_interface__fast_sodium_current_m_gate__m, 2) * var_chaste_interface__fast_sodium_current_h_gate__h;
        const double var_x53 = var_x51 * var_x7;
        const double var_x54 = var_x38 * var_chaste_interface__L_type_Ca_channel_f_gate__f;
        const double var_x55 = var_x31 * var_x54;
        const double var_x56 = var_x38 * var_chaste_interface__L_type_Ca_channel_d_gate__d;
        const double var_x57 = var_x31 * var_x56;
        const double var_x58 = var_x49 * (0.0050000000000000001 * var_chaste_interface__membrane__V - 0.13356880329847826 * var_x3);
        ...
        const double var_x76 = 1.0 - var_chaste_interface__time_dependent_potassium_current_x_gate__x;
        ...
        const double var_x91 = 0.0030000000000000001 * var_chaste_interface__L_type_Ca_channel_d_gate__d;
        ...
        const double var_x117 = 0.5 * var_x116;
        const double var_x118 = 1.0 * var_x115 / pow(var_x113, 3);
        const double var_x119 = 1.0 * var_x114 * var_chaste_interface__intracellular_calcium_concentration__Ca_i;
        const double var_x120 = 1.0 - var_chaste_interface__calcium_release__ActFrac - var_chaste_interface__calcium_release__ProdFrac;
        const double var_x121 = 5.9129400406899753e-8 * var_x36;
        const double var_x122 = 1.182588008137995e-8 * var_x34;
        const double var_x123 = 8.4392437745822447e-5 * var_x3 - 3.1591373008424612e-6 * var_chaste_interface__membrane__V;
        const double var_x124 = 1.1825880081379951e-5 * var_x27;
        const double var_x125 = 0.10204081632653063 * var_chaste_interface__intracellular_calcium_concentration__Ca_rel;
        const double var_x126 = 0.25 + var_chaste_interface__calcium_release__ActFrac;
        const double var_x127 = pow(var_x126, (-2));
        const double var_x128 = var_x127 * var_chaste_interface__calcium_release__ActFrac;
        const double var_x129 = pow(var_chaste_interface__calcium_release__ActFrac, 2);
        const double var_x130 = var_x129 / pow(var_x126, 3);
        ...
        const double var_x140 = 0.5 * var_chaste_interface__intracellular_calcium_concentration__Ca_rel;
        const double var_x141 = -var_x139;
        
        // Matrix entries
        IJth(rJacobian, 0, 0) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x49 * (0.0014499999999999999 + 2.5 * var_x8 + 0.00050000000000000001 * var_x15 + 0.0050000000000000001 * var_x16 + 0.0056051847285169109 * var_x41 + 0.0010695397377927277 * var_x19 + 0.00049325625610948819 * var_x43 + 0.2857142857142857 * var_x6 - 0.0028025923642584554 * var_x44 - 0.021390794755854554 * var_x20 - 7.0064809106461387e-7 * var_x46 - 3.503240455323069e-6 * var_x47 + var_x21 * var_x28 + var_x32 * var_x35 + var_x32 * var_x37));
        IJth(rJacobian, 1, 0) = 7.7999999999999999e-5 * var_x75 * var_chaste_interface__time_dependent_potassium_current_x_gate__x + 4.1300000000000001e-5 * var_x76 * var_x80 - 2.8500000000000002e-5 * var_x76 * var_x77 * var_x79 / pow(var_x78, 2) - 5.2000000000000004e-5 * var_x72 * var_x74 * var_chaste_interface__time_dependent_potassium_current_x_gate__x / pow(var_x73, 2);
        IJth(rJacobian, 2, 0) = (0.001 - 0.001 * var_chaste_interface__fast_sodium_current_m_gate__m) * ((var_x82) ? (0) : (var_x86 - 20.0 * var_x81 * var_x84 / pow(var_x85, 2))) + 0.44800000000000001 * var_x87 * var_chaste_interface__fast_sodium_current_m_gate__m;
        IJth(rJacobian, 3, 0) = -0.125 * var_x89 * (0.02 - 0.02 * var_chaste_interface__fast_sodium_current_h_gate__h) - 0.00062500000000000001 * var_x90 * var_chaste_interface__fast_sodium_current_h_gate__h / pow((0.0031250000000000002 + var_x90), 2);
        IJth(rJacobian, 4, 0) = (0.0030000000000000001 - var_x91) * ((var_x93) ? (0) : (var_x98 - 7.5 * var_x92 * var_x96 / pow(var_x97, 2))) - var_x91 * ((var_x93) ? (0) : (var_x101 - 1.2 * var_x92 * var_x99 / pow(var_x100, 2)));
        IJth(rJacobian, 5, 0) = (0.00050000000000000001 - 0.00050000000000000001 * var_chaste_interface__L_type_Ca_channel_f_gate__f) * ((var_x103) ? (0) : (var_x106 - 1.5625 * var_x102 * var_x104 / pow(var_x105, 2))) - 0.0062500000000000003 * var_x107 * var_chaste_interface__L_type_Ca_channel_f_gate__f / pow(var_x108, 2);
        IJth(rJacobian, 6, 0) = -0.058823529411764705 * var_x109 * (3.3000000000000003e-5 - 3.3000000000000003e-5 * var_chaste_interface__transient_outward_current_s_gate__s) - 0.0041250000000000002 * var_x110 * var_chaste_interface__transient_outward_current_s_gate__s / pow(var_x111, 2);
        IJth(rJacobian, 7, 0) = 0.066600000000000006 * var_x112 / pow((1.0 + var_x112), 2);
        IJth(rJacobian, 10, 0) = -3.7909647610109532e-7 + 2.2134435192462872e-9 * var_x47 - 0.0015795686504212305 * var_x8 - 3.0988209269448018e-7 * var_x43 - 9.477411902527384e-7 * var_x15 - var_x121 * var_x32;
        IJth(rJacobian, 11, 0) = -3.7909647610109532e-7 + 1.3515291521577086e-5 * var_x20 + 4.4268870384925742e-10 * var_x46 - 0.00018052213147671208 * var_x6 - 3.1591373008424612e-6 * var_x16 - 6.7576457607885431e-7 * var_x19 - 1.7707548153970297e-9 * var_x43 - var_x122 * var_x32;
        IJth(rJacobian, 12, 0) = -7.8978432521061529e-8 + 3.1591373008424611e-7 * var_x15 + 8.8537740769851485e-7 * var_x44 - 1.7707548153970297e-6 * var_x41 - var_x124 * var_x21;
        IJth(rJacobian, 0, 1) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x49 * (0.0071428571428571426 * var_chaste_interface__intracellular_potassium_concentration__K_i - 0.028571428571428571 * var_x18));
        IJth(rJacobian, 1, 1) = -0.00050000000000000001 * var_x80 - 0.0012999999999999999 * var_x75;
        IJth(rJacobian, 11, 1) = 1.8052213147671208e-5 * var_x18 - 4.5130532869178019e-6 * var_chaste_interface__intracellular_potassium_concentration__K_i;
        IJth(rJacobian, 0, 2) = mSetVoltageDerivativeToZero ? 0.0 : (-0.0074999999999999997 * var_x48 * var_x52);
        IJth(rJacobian, 2, 2) = -8.0 * var_x87 - 0.001 * ((var_x82) ? (2000.0) : (var_x81 * var_x86));
        IJth(rJacobian, 10, 2) = -0.0047387059512636921 * var_x52;
        IJth(rJacobian, 0, 3) = mSetVoltageDerivativeToZero ? 0.0 : (-0.0025000000000000001 * var_x48 * var_x53);
        IJth(rJacobian, 3, 3) = -2.0 / (1.0 + 320.0 * var_x90) - 0.02 * var_x89;
        IJth(rJacobian, 10, 3) = -0.0015795686504212305 * var_x53;
        IJth(rJacobian, 0, 4) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x49 * (var_x28 * var_x54 + var_x35 * var_x55 + var_x37 * var_x55));
        IJth(rJacobian, 4, 4) = -0.0030000000000000001 * ((var_x93) ? (120.0) : (var_x101 * var_x92)) - 0.0030000000000000001 * ((var_x93) ? (120.0) : (var_x92 * var_x98));
        IJth(rJacobian, 10, 4) = -var_x121 * var_x55;
        IJth(rJacobian, 11, 4) = -var_x122 * var_x55;
        IJth(rJacobian, 12, 4) = -var_x124 * var_x54;
        IJth(rJacobian, 0, 5) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x49 * (var_x28 * var_x56 + var_x35 * var_x57 + var_x37 * var_x57));
        IJth(rJacobian, 5, 5) = -0.00050000000000000001 * ((var_x103) ? (25.0) : (var_x102 * var_x106)) - 0.025000000000000001 / var_x108;
        IJth(rJacobian, 10, 5) = -var_x121 * var_x57;
        IJth(rJacobian, 11, 5) = -var_x122 * var_x57;
        IJth(rJacobian, 12, 5) = -var_x124 * var_x56;
        IJth(rJacobian, 0, 6) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x58 * var_chaste_interface__transient_outward_current_r_gate__r);
        IJth(rJacobian, 6, 6) = -0.033000000000000002 / var_x111 - 3.3000000000000003e-5 * var_x109;
        IJth(rJacobian, 11, 6) = var_x123 * var_chaste_interface__transient_outward_current_r_gate__r;
        IJth(rJacobian, 0, 7) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x58 * var_chaste_interface__transient_outward_current_s_gate__s);
        IJth(rJacobian, 7, 7) = -0.33300000000000002;
        IJth(rJacobian, 11, 7) = var_x123 * var_chaste_interface__transient_outward_current_s_gate__s;
        IJth(rJacobian, 8, 8) = -0.059999999999999998 - 1.0 * var_x116;
        IJth(rJacobian, 9, 8) = 0.059999999999999998 + var_x117;
        IJth(rJacobian, 12, 8) = var_x125 * var_x128 - var_x125 * var_x130;
        IJth(rJacobian, 14, 8) = var_x130 * var_x140 - var_x128 * var_x140;
        IJth(rJacobian, 8, 9) = -var_x117;
        IJth(rJacobian, 9, 9) = -0.001;
        IJth(rJacobian, 0, 10) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x49 * (0.0030000000000000001 * var_x62 + 66.784401649239129 * var_x63 + 0.55999999999999994 * var_x59 + 0.016028256395817387 * var_x60 + 9.3584727056863679e-5 * var_x64 - 0.00035 * var_x61));
        IJth(rJacobian, 10, 10) = 6.634188331769168e-7 * var_x61 - 0.0010614701330830668 * var_x59 - 1.0127092529498693e-5 * var_x60 - 5.6864471415164302e-6 * var_x62 - 0.042196218872911223 * var_x63 - 5.9129400406899753e-8 * var_x64;
        IJth(rJacobian, 11, 10) = 0.00070764675538871125 * var_x59 - 4.4227922211794452e-7 * var_x61;
        IJth(rJacobian, 12, 10) = 1.8954823805054768e-6 * var_x62;
        IJth(rJacobian, 0, 11) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x49 * (1.8716945411372735e-5 * var_x64 + 8.0141281979086951 * var_x63 + 0.13356880329847826 * var_x66 + 0.016028256395817387 * var_x2 + 0.0071428571428571426 * var_chaste_interface__time_dependent_potassium_current_x_gate__x + 7.6325030456273284 * var_x65 - 0.5714285714285714 * var_x67));
        IJth(rJacobian, 10, 11) = -0.0050635462647493461 * var_x63;
        IJth(rJacobian, 11, 11) = 0.00036104426295342415 * var_x67 - 4.5130532869178019e-6 * var_chaste_interface__time_dependent_potassium_current_x_gate__x - 1.0127092529498693e-5 * var_x2 - 8.4392437745822447e-5 * var_x66 - 0.0048224250140469962 * var_x65 - 1.182588008137995e-8 * var_x64;
        IJth(rJacobian, 0, 12) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x49 * (0.037433890822745473 * var_x71 + 0.0033392200824619565 * var_x68 - 1372.0 * var_x69 - 3.45e-6 * var_x70));
        IJth(rJacobian, 8, 12) = var_x119 * var_x120 + (var_x118 - var_x119) * var_chaste_interface__calcium_release__ActFrac - var_x118 * var_x120;
        IJth(rJacobian, 9, 12) = (var_x119 - var_x118) * var_chaste_interface__calcium_release__ActFrac;
        IJth(rJacobian, 10, 12) = 6.5394142127438948e-9 * var_x70 + 2.6006018260535142 * var_x69;
        IJth(rJacobian, 12, 12) = -7.0 + var_x131 + var_x132 + 0.00040000000000000002 * var_x136 - 0.00040000000000000002 * var_x134 - 1.0549054718227806e-6 * var_x68 - 0.86686727535117136 * var_x69 - 2.1798047375812981e-9 * var_x70 - 1.1825880081379951e-5 * var_x71 - 7.1999999999999991e-9 * var_x137;
        ...
        IJth(rJacobian, 12, 13) = 9.5999999999999999e-8 * var_x136 + 7.1999999999999991e-9 * var_x134 - 1.7279999999999999e-12 * var_x137;
        ...
        IJth(rJacobian, 12, 14) = 0.051020408163265314 * var_x138;
        IJth(rJacobian, 13, 14) = 0.050000000000000003;
        IJth(rJacobian, 14, 14) = -0.0050000000000000001 - 0.25 * var_x138;
        IJth(rJacobian, 12, 15) = 0.050000000000000003 + var_x139;
        IJth(rJacobian, 15, 15) = -0.050000000000000003 + var_x141;
        IJth(rJacobian, 12, 16) = 0.20000000000000001 + var_x139;
        IJth(rJacobian, 16, 16) = -0.20000000000000001 + var_x141;
    }
