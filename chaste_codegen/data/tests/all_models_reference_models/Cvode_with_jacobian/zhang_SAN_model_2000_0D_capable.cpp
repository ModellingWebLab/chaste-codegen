...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellzhang_SAN_model_2000_0D_capableFromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time_converted, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x39 = exp(-2.0 + 0.40000000000000002 * var_chaste_interface__membrane__V);
        const double var_x40 = -1.0 + var_x39;
        const double var_x41 = 1 / var_x40;
        const double var_x42 = 5.7149999999999999 * var_x41;
        const double var_x43 = exp(-14.0 - 0.40000000000000002 * var_chaste_interface__membrane__V);
        const double var_x44 = -1.0 + var_x43;
        const double var_x45 = 1 / var_x44;
        const double var_x46 = 14.195 * var_x45;
        const double var_x47 = exp(-0.20799999999999999 * var_chaste_interface__membrane__V);
        const double var_x48 = -1.0 + var_x47;
        const double var_x49 = 1 / var_x48;
        const double var_x50 = 42.450000000000003 * var_x49;
        const double var_x51 = 35.0 + var_chaste_interface__membrane__V;
        const double var_x52 = -5.0 + var_chaste_interface__membrane__V;
        const double var_x53 = exp(9.0 + 0.20000000000000001 * var_chaste_interface__membrane__V);
        const double var_x54 = 1.0 + var_x53;
        const double var_x55 = 0.25 * var_chaste_interface__membrane__V;
        const double var_x56 = exp(7.0 + var_x55);
        const double var_x57 = -1.0 + var_x56;
        const double var_x58 = 1 / var_x57;
        const double var_x59 = 3.125 * var_x58;
        const double var_x60 = exp(-7.0 - var_x55);
        const double var_x61 = 1.0 + var_x60;
        const double var_x62 = 28.0 + var_chaste_interface__membrane__V;
        const double var_x63 = 1 / var_x61;
        const double var_x64 = exp(-5.4411764705882355 - 0.14705882352941177 * var_chaste_interface__membrane__V);
        const double var_x65 = 1.0 + var_x64;
        const double var_x66 = 0.033333333333333333 * var_chaste_interface__membrane__V;
        const double var_x67 = exp(0.87666666666666671 + var_x66);
        const double var_x68 = exp(-0.87666666666666671 - var_x66);
        const double var_x69 = 0.1111111111111111 * var_chaste_interface__membrane__V;
        const double var_x70 = exp(7.8888888888888893 + var_x69);
        const double var_x71 = 1.0 + var_x70;
        const double var_x72 = exp(4.6163849154746419 + 0.065019505851755519 * var_chaste_interface__membrane__V);
        const double var_x73 = exp(-0.85234093637454988 - 0.012004801920768308 * var_chaste_interface__membrane__V);
        const double var_x74 = exp(4.5320610687022898 + 0.076335877862595422 * var_chaste_interface__membrane__V);
        const double var_x75 = 1.0 + var_x74;
        const double var_x76 = exp(-3.1827900000000002 - 0.081610000000000002 * var_chaste_interface__membrane__V);
        const double var_x77 = exp(11.128867 + 0.27189999999999998 * var_chaste_interface__membrane__V);
        const double var_x78 = 1 / (0.56859999999999999 * var_x76 + 0.71740000000000004 * var_x77);
        const double var_x79 = 1 / (0.010103333333333334 + 0.065166666666666664 * var_x78);
        const double var_x80 = exp(0.5548223350253807 - 0.050761421319796954 * var_chaste_interface__membrane__V);
        const double var_x81 = 1.0 + var_x80;
        const double var_x82 = exp(2.7585732000000003 + 0.090120000000000006 * var_chaste_interface__membrane__V);
        const double var_x83 = exp(-2.8369599999999999 - 0.11899999999999999 * var_chaste_interface__membrane__V);
        const double var_x84 = 1 / (1.0369999999999999 * var_x82 + 0.36899999999999999 * var_x83);
        const double var_x85 = 1 / (0.0029775000000000001 + 0.019595000000000001 * var_x84);
        const double var_x86 = exp(-1.3396226415094339 - 0.094339622641509441 * var_chaste_interface__membrane__V);
        const double var_x87 = 1.0 + var_x86;
        const double var_x88 = 0.001 / var_x87;
        const double var_x89 = exp(-0.56603773584905659 + 0.062893081761006289 * var_chaste_interface__membrane__V);
        const double var_x90 = exp(0.40000000000000002 - 0.044444444444444446 * var_chaste_interface__membrane__V);
        const double var_x91 = 9.4339622641509443e-5 * var_x86 / pow(var_x87, 2);
        const double var_x92 = exp(-0.52941176470588236 + 0.058823529411764705 * var_chaste_interface__membrane__V);
        const double var_x93 = exp(0.41666666666666663 - 0.046296296296296294 * var_chaste_interface__membrane__V);
        const double var_x94 = exp(1.8415841584158419 + 0.099009900990099015 * var_chaste_interface__membrane__V);
        const double var_x95 = exp(-0.022222222222222223 * var_chaste_interface__membrane__V);
        const double var_x96 = exp(4.4444444444444446 - var_x69);
        const double var_x97 = 1.0 + var_x96;
        const double var_x98 = exp(3.5355294117647058 + 0.047058823529411764 * var_chaste_interface__membrane__V);
        const double var_x99 = exp(-2.9631993991738641 - 0.037551633496057078 * var_chaste_interface__membrane__V);
        ...
        IJth(rJacobian, 4, 0) = (0.001 / var_x38 - 0.001 * var_chaste_interface__L_type_Ca_channel_d_gate__d_L) * (var_x42 - var_x46 - var_x50 - 5.6779999999999999 * var_x43 * var_x51 / pow(var_x44, 2) - 8.829600000000001 * var_x47 * var_chaste_interface__membrane__V / pow(var_x48, 2) - 2.286 * var_x39 * var_x52 / pow(var_x40, 2)) + 0.00016666666666666666 * var_x37 * (var_x42 * var_x52 - var_x46 * var_x51 - var_x50 * var_chaste_interface__membrane__V) / pow(var_x38, 2);
        IJth(rJacobian, 5, 0) = (0.001 / var_x54 - 0.001 * var_chaste_interface__L_type_Ca_channel_f_gate__f_L) * (var_x59 + 6.25 * var_x60 / pow(var_x61, 2) - 0.78125 * var_x56 * var_x62 / pow(var_x57, 2)) - 0.00020000000000000001 * var_x53 * (25.0 * var_x63 + var_x59 * var_x62) / pow(var_x54, 2);
        IJth(rJacobian, 6, 0) = (0.001 / var_x65 - 0.001 * var_chaste_interface__T_type_Ca_channel_d_gate__d_T) * (35.600000000000001 * var_x67 - 35.600000000000001 * var_x68) + 0.00014705882352941178 * var_x64 * (1068.0 * var_x67 + 1068.0 * var_x68) / pow(var_x65, 2);
        IJth(rJacobian, 7, 0) = (0.001 / var_x71 - 0.001 * var_chaste_interface__T_type_Ca_channel_f_gate__f_T) * (0.97529258777633288 * var_x72 - 0.18367346938775511 * var_x73) - 0.00011111111111111112 * var_x70 * (15.0 * var_x72 + 15.300000000000001 * var_x73) / pow(var_x71, 2);
        IJth(rJacobian, 8, 0) = -7.6335877862595422e-5 * var_x74 * var_x79 / pow(var_x75, 2) - 0.02981616261141468 * (-var_chaste_interface__four_AP_sensitive_currents_q_gate__q + 1 / var_x75) * (0.046403446000000001 * var_x76 - 0.19506105999999998 * var_x77) / (pow((0.1550383631713555 + var_x78), 2) * pow((var_x77 + 0.79258433231112346 * var_x76), 2));
        IJth(rJacobian, 9, 0) = 5.076142131979696e-5 * var_x80 * var_x85 / pow(var_x81, 2) - 0.047456665474470604 * (-var_chaste_interface__four_AP_sensitive_currents_r_gate__r + 1 / var_x81) * (0.043910999999999999 * var_x83 - 0.09345444 * var_x82) / (pow((0.15195202857871906 + var_x84), 2) * pow((var_x82 + 0.35583413693346194 * var_x83), 2));
        IJth(rJacobian, 10, 0) = var_x91 * (0.95999999999999996 * var_x90 + 37.200000000000003 * var_x89) + (var_x88 - 0.001 * var_chaste_interface__rapid_delayed_rectifying_potassium_current_P_af_gate__P_af) * (2.3396226415094339 * var_x89 - 0.042666666666666665 * var_x90);
        IJth(rJacobian, 11, 0) = var_x91 * (4.2000000000000002 * var_x92 + 0.14999999999999999 * var_x93) + (var_x88 - 0.001 * var_chaste_interface__rapid_delayed_rectifying_potassium_current_P_as_gate__P_as) * (0.24705882352941178 * var_x92 - 0.0069444444444444441 * var_x93);
        IJth(rJacobian, 12, 0) = -0.049504950495049507 * var_x94 / pow((1.0 + var_x94), 2);
        IJth(rJacobian, 13, 0) = 2.2222222222222223e-5 * var_x95 * var_chaste_interface__slow_delayed_rectifying_potassium_current_xs_gate__xs + 0.0015555555555555555 * var_x96 * (1.0 - var_chaste_interface__slow_delayed_rectifying_potassium_current_xs_gate__xs) / pow(var_x97, 2);
        IJth(rJacobian, 14, 0) = -4.7058823529411767e-5 * var_x98 * var_chaste_interface__hyperpolarisation_activated_current_y_gate__y - 0.037551633496057078 * var_x99 * (0.001 - 0.001 * var_chaste_interface__hyperpolarisation_activated_current_y_gate__y);
        ...
        IJth(rJacobian, 9, 9) = -0.001 * var_x85;
        ...
        IJth(rJacobian, 10, 10) = -0.00095999999999999992 * var_x90 - 0.037200000000000004 * var_x89;
        ...
        IJth(rJacobian, 11, 11) = -0.00014999999999999999 * var_x93 - 0.0042000000000000006 * var_x92;
        ...
        IJth(rJacobian, 13, 13) = -0.001 * var_x95 - 0.014 / var_x97;
        ...
        IJth(rJacobian, 14, 14) = -0.001 * var_x98 - 0.001 * var_x99;
    }
