...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellzhang_SAN_model_2000_0D_capableFromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time_converted, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x37 = 1 / (-1.0 + exp(-9.9999999747524276e-8));
        const double var_x38 = 1 / (-1.0 + exp(1.0000000010279565e-7));
        const double var_x39 = -5.0 + var_chaste_interface__membrane__V;
        const double var_x40 = fabs(var_x39) < 2.4999999981289989e-7;
        ...
        const double var_x44 = 1 / (-1.0 + exp(9.9999999999999995e-8));
        const double var_x45 = 1 / (-1.0 + exp(-9.9999999999999995e-8));
        const double var_x46 = exp(-14.0 - 0.40000000000000002 * var_chaste_interface__membrane__V);
        const double var_x47 = -1.0 + var_x46;
        const double var_x48 = 28.390000000000001 / var_x47;
        const double var_x49 = 35.0 + var_chaste_interface__membrane__V;
        const double var_x50 = -var_x48 - 11.356 * var_x46 * var_x49 / pow(var_x47, 2);
        const double var_x51 = fabs(var_chaste_interface__membrane__V) < 4.8076923076923074e-7;
        const double var_x52 = exp(-0.20799999999999999 * var_chaste_interface__membrane__V);
        const double var_x53 = -1.0 + var_x52;
        const double var_x54 = 84.900000000000006 / var_x53;
        const double var_x55 = exp(-3.7166666666666668 + var_x0);
        const double var_x56 = 1.0 + var_x55;
        const double var_x57 = 2.8575000029373855e-6 * var_x38;
        const double var_x58 = ((var_x40) ? (var_x57 - 2000000.0014968009 * (-5.0000002500000003 + var_chaste_interface__membrane__V) * (-var_x57 - 2.8574999927855059e-6 * var_x37)) : (var_x39 * var_x43));
        const double var_x59 = 4.0817307692307694e-5 * var_x44;
        const double var_x60 = -var_x48 * var_x49;
        const double var_x61 = ((var_x51) ? (var_x59 + var_x60 + 1040000.0 * (4.8076923076923074e-7 + var_chaste_interface__membrane__V) * (-var_x59 - 4.0817307692307694e-5 * var_x45)) : (var_x60 - var_x54 * var_chaste_interface__membrane__V));
        const double var_x62 = exp(9.0 + 0.20000000000000001 * var_chaste_interface__membrane__V);
        const double var_x63 = 1.0 + var_x62;
        const double var_x64 = 0.25 * var_chaste_interface__membrane__V;
        const double var_x65 = exp(7.0 + var_x64);
        const double var_x66 = -1.0 + var_x65;
        const double var_x67 = 1 / var_x66;
        const double var_x68 = 3.125 * var_x67;
        const double var_x69 = exp(-7.0 - var_x64);
        const double var_x70 = 1.0 + var_x69;
        const double var_x71 = 28.0 + var_chaste_interface__membrane__V;
        const double var_x72 = 1 / var_x70;
        const double var_x73 = exp(-5.4411764705882355 - 0.14705882352941177 * var_chaste_interface__membrane__V);
        const double var_x74 = 1.0 + var_x73;
        const double var_x75 = 0.033333333333333333 * var_chaste_interface__membrane__V;
        const double var_x76 = exp(0.87666666666666671 + var_x75);
        const double var_x77 = exp(-0.87666666666666671 - var_x75);
        const double var_x78 = 0.1111111111111111 * var_chaste_interface__membrane__V;
        const double var_x79 = exp(7.8888888888888893 + var_x78);
        const double var_x80 = 1.0 + var_x79;
        const double var_x81 = exp(4.6163849154746419 + 0.065019505851755519 * var_chaste_interface__membrane__V);
        const double var_x82 = exp(-0.85234093637454988 - 0.012004801920768308 * var_chaste_interface__membrane__V);
        const double var_x83 = exp(4.5320610687022898 + 0.076335877862595422 * var_chaste_interface__membrane__V);
        const double var_x84 = 1.0 + var_x83;
        const double var_x85 = exp(-3.1827900000000002 - 0.081610000000000002 * var_chaste_interface__membrane__V);
        const double var_x86 = exp(11.128867 + 0.27189999999999998 * var_chaste_interface__membrane__V);
        const double var_x87 = 1 / (0.56859999999999999 * var_x85 + 0.71740000000000004 * var_x86);
        const double var_x88 = 1 / (0.010103333333333334 + 0.065166666666666664 * var_x87);
        const double var_x89 = exp(0.5548223350253807 - 0.050761421319796954 * var_chaste_interface__membrane__V);
        const double var_x90 = 1.0 + var_x89;
        const double var_x91 = exp(2.7585732000000003 + 0.090120000000000006 * var_chaste_interface__membrane__V);
        const double var_x92 = exp(-2.8369599999999999 - 0.11899999999999999 * var_chaste_interface__membrane__V);
        const double var_x93 = 1 / (1.0369999999999999 * var_x91 + 0.36899999999999999 * var_x92);
        const double var_x94 = 1 / (0.0029775000000000001 + 0.019595000000000001 * var_x93);
        const double var_x95 = exp(-1.3396226415094339 - 0.094339622641509441 * var_chaste_interface__membrane__V);
        const double var_x96 = 1.0 + var_x95;
        const double var_x97 = 0.001 / var_x96;
        const double var_x98 = exp(-0.56603773584905659 + 0.062893081761006289 * var_chaste_interface__membrane__V);
        const double var_x99 = exp(0.40000000000000002 - 0.044444444444444446 * var_chaste_interface__membrane__V);
        const double var_x100 = 9.4339622641509443e-5 * var_x95 / pow(var_x96, 2);
        const double var_x101 = exp(-0.52941176470588236 + 0.058823529411764705 * var_chaste_interface__membrane__V);
        const double var_x102 = exp(0.41666666666666663 - 0.046296296296296294 * var_chaste_interface__membrane__V);
        const double var_x103 = exp(1.8415841584158419 + 0.099009900990099015 * var_chaste_interface__membrane__V);
        const double var_x104 = exp(-0.022222222222222223 * var_chaste_interface__membrane__V);
        const double var_x105 = exp(4.4444444444444446 - var_x78);
        const double var_x106 = 1.0 + var_x105;
        const double var_x107 = exp(3.5355294117647058 + 0.047058823529411764 * var_chaste_interface__membrane__V);
        const double var_x108 = exp(-2.9631993991738641 - 0.037551633496057078 * var_chaste_interface__membrane__V);
        ...
        IJth(rJacobian, 4, 0) = (0.5 * ((var_x40) ? (5.7149999898481205 * var_x37 + 5.7150000101518792 * var_x38) : (var_x43 - 4.5720000000000001 * var_x39 * var_x41 / pow(var_x42, 2))) + 0.5 * ((var_x51) ? (var_x50 - 42.450000000000003 * var_x44 - 42.450000000000003 * var_x45) : (var_x50 - var_x54 - 17.659200000000002 * var_x52 * var_chaste_interface__membrane__V / pow(var_x53, 2)))) * (0.001 / var_x56 - 0.001 * var_chaste_interface__L_type_Ca_channel_d_gate__d_L) + 0.00016666666666666666 * var_x55 * (0.5 * var_x58 + 0.5 * var_x61) / pow(var_x56, 2);
        IJth(rJacobian, 5, 0) = (0.001 / var_x63 - 0.001 * var_chaste_interface__L_type_Ca_channel_f_gate__f_L) * (var_x68 + 6.25 * var_x69 / pow(var_x70, 2) - 0.78125 * var_x65 * var_x71 / pow(var_x66, 2)) - 0.00020000000000000001 * var_x62 * (25.0 * var_x72 + var_x68 * var_x71) / pow(var_x63, 2);
        IJth(rJacobian, 6, 0) = (0.001 / var_x74 - 0.001 * var_chaste_interface__T_type_Ca_channel_d_gate__d_T) * (35.600000000000001 * var_x76 - 35.600000000000001 * var_x77) + 0.00014705882352941178 * var_x73 * (1068.0 * var_x76 + 1068.0 * var_x77) / pow(var_x74, 2);
        IJth(rJacobian, 7, 0) = (0.001 / var_x80 - 0.001 * var_chaste_interface__T_type_Ca_channel_f_gate__f_T) * (0.97529258777633288 * var_x81 - 0.18367346938775511 * var_x82) - 0.00011111111111111112 * var_x79 * (15.0 * var_x81 + 15.300000000000001 * var_x82) / pow(var_x80, 2);
        IJth(rJacobian, 8, 0) = -7.6335877862595422e-5 * var_x83 * var_x88 / pow(var_x84, 2) - 0.02981616261141468 * (-var_chaste_interface__four_AP_sensitive_currents_q_gate__q + 1 / var_x84) * (0.046403446000000001 * var_x85 - 0.19506105999999998 * var_x86) / (pow((0.1550383631713555 + var_x87), 2) * pow((var_x86 + 0.79258433231112346 * var_x85), 2));
        IJth(rJacobian, 9, 0) = 5.076142131979696e-5 * var_x89 * var_x94 / pow(var_x90, 2) - 0.047456665474470604 * (-var_chaste_interface__four_AP_sensitive_currents_r_gate__r + 1 / var_x90) * (0.043910999999999999 * var_x92 - 0.09345444 * var_x91) / (pow((0.15195202857871906 + var_x93), 2) * pow((var_x91 + 0.35583413693346194 * var_x92), 2));
        IJth(rJacobian, 10, 0) = var_x100 * (0.95999999999999996 * var_x99 + 37.200000000000003 * var_x98) + (var_x97 - 0.001 * var_chaste_interface__rapid_delayed_rectifying_potassium_current_P_af_gate__P_af) * (2.3396226415094339 * var_x98 - 0.042666666666666665 * var_x99);
        IJth(rJacobian, 11, 0) = var_x100 * (4.2000000000000002 * var_x101 + 0.14999999999999999 * var_x102) + (var_x97 - 0.001 * var_chaste_interface__rapid_delayed_rectifying_potassium_current_P_as_gate__P_as) * (0.24705882352941178 * var_x101 - 0.0069444444444444441 * var_x102);
        IJth(rJacobian, 12, 0) = -0.049504950495049507 * var_x103 / pow((1.0 + var_x103), 2);
        IJth(rJacobian, 13, 0) = 2.2222222222222223e-5 * var_x104 * var_chaste_interface__slow_delayed_rectifying_potassium_current_xs_gate__xs + 0.0015555555555555555 * var_x105 * (1.0 - var_chaste_interface__slow_delayed_rectifying_potassium_current_xs_gate__xs) / pow(var_x106, 2);
        IJth(rJacobian, 14, 0) = -4.7058823529411767e-5 * var_x107 * var_chaste_interface__hyperpolarisation_activated_current_y_gate__y - 0.037551633496057078 * var_x108 * (0.001 - 0.001 * var_chaste_interface__hyperpolarisation_activated_current_y_gate__y);
        ...
        IJth(rJacobian, 9, 9) = -0.001 * var_x94;
        ...
        IJth(rJacobian, 10, 10) = -0.00095999999999999992 * var_x99 - 0.037200000000000004 * var_x98;
        ...
        IJth(rJacobian, 11, 11) = -0.00014999999999999999 * var_x102 - 0.0042000000000000006 * var_x101;
        ...
        IJth(rJacobian, 13, 13) = -0.001 * var_x104 - 0.014 / var_x106;
        ...
        IJth(rJacobian, 14, 14) = -0.001 * var_x107 - 0.001 * var_x108;
    }
