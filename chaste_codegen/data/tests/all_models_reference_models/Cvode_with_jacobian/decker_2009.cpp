...
#include "MathsCustomFunctions.hpp"
    ...
    void Celldecker_2009FromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x381 = exp(-0.96036036036036043 - 0.0900900900900901 * var_chaste_interface__membrane__Vm);
        const double var_x382 = 1.0 + var_x381;
        const double var_x383 = var_chaste_interface__membrane__Vm >= -40.0;
        const double var_x384 = exp(0.079000000000000001 * var_chaste_interface__membrane__Vm);
        const double var_x385 = exp(0.34999999999999998 * var_chaste_interface__membrane__Vm);
        const double var_x386 = exp(-11.764705882352942 - 0.14705882352941177 * var_chaste_interface__membrane__Vm);
        const double var_x387 = exp(-3.2000000000000002 + var_x372);
        const double var_x388 = 1.0 + var_x387;
        const double var_x389 = exp(-2.5349999999999999e-7 * var_chaste_interface__membrane__Vm);
        const double var_x390 = var_x389 / var_x388;
        const double var_x391 = 1.0 - var_chaste_interface__INa_j_gate__j;
        const double var_x392 = exp(24.640530000000002 + 0.311 * var_chaste_interface__membrane__Vm);
        const double var_x393 = 1.0 + var_x392;
        const double var_x394 = 1 / var_x393;
        const double var_x395 = exp(0.24440000000000001 * var_chaste_interface__membrane__Vm);
        const double var_x396 = exp(-0.043909999999999998 * var_chaste_interface__membrane__Vm);
        const double var_x397 = -127140.0 * var_x395 - 6.9480000000000006e-5 * var_x396;
        const double var_x398 = var_x394 * var_x397;
        ...
        const double var_x403 = 37.780000000000001 + var_chaste_interface__membrane__Vm;
        const double var_x404 = var_x391 * var_x403;
        const double var_x405 = 1.0 - var_chaste_interface__INaL_mL_gate__mL;
        const double var_x406 = exp(14.918032786885247 + 0.16393442622950821 * var_chaste_interface__membrane__Vm);
        const double var_x407 = exp(-2.3729411764705883 - 0.23529411764705882 * var_chaste_interface__membrane__Vm);
        const double var_x408 = 1.0 + var_x407;
        const double var_x409 = exp(5.8385137599999997 + 0.1522 * var_chaste_interface__membrane__Vm);
        const double var_x410 = -1.0 + var_x409;
        const double var_x411 = 0.00029999999999999997 / var_x410;
        const double var_x412 = exp(0.2364224 - 0.13600000000000001 * var_chaste_interface__membrane__Vm);
        const double var_x413 = 1.0 - var_x412;
        const double var_x414 = 0.00059999999999999995 / var_x413;
        const double var_x415 = -1.7383999999999999 + var_chaste_interface__membrane__Vm;
        const double var_x416 = 38.360799999999998 + var_chaste_interface__membrane__Vm;
        const double var_x417 = var_x411 * var_x416;
        const double var_x418 = var_x414 * var_x415;
        const double var_x419 = exp(-1.3230057479321462 - 0.14019346698443852 * var_chaste_interface__membrane__Vm);
        const double var_x420 = 1.0 + var_x419;
        const double var_x421 = 0.034035137876343539 * var_chaste_interface__membrane__Vm;
        const double var_x422 = exp(0.62658348478969694 - var_x421);
        const double var_x423 = 1.0 + var_x422;
        const double var_x424 = exp(3.403513787634354 + var_x421);
        const double var_x425 = 1.0 + var_x424;
        const double var_x426 = 3.5 / var_x425;
        const double var_x427 = 0.82719827942757873 / var_x423;
        const double var_x428 = 0.20000000000000001 * var_chaste_interface__membrane__Vm;
        const double var_x429 = exp(11.6 + var_x428);
        const double var_x430 = 1.0 + var_x429;
        const double var_x431 = var_x429 / pow(var_x430, 2);
        const double var_x432 = 0.0050000000000000001 * var_x431;
        const double var_x433 = exp(-2.1111111111111112 - 0.1111111111111111 * var_chaste_interface__membrane__Vm);
        const double var_x434 = 1.0 + var_x433;
        const double var_x435 = var_x433 / pow(var_x434, 2);
        const double var_x436 = 0.011343308638950427 * var_x435;
        const double var_x437 = 1 / var_x430;
        const double var_x438 = 0.025000000000000001 * var_x437;
        const double var_x439 = 1 / var_x434;
        const double var_x440 = -0.10208977775055385 * var_x439;
        const double var_x441 = exp(12.0 + var_x428);
        const double var_x442 = 1.0 + var_x441;
        const double var_x443 = 1 / var_chaste_interface__Ca__Ca_JSR;
        const double var_x444 = pow(var_x443, 8.0);
        const double var_x445 = 1.0 + 1.0 * var_x444;
        const double var_x446 = 1 / var_x445;
        const double var_x447 = 1 / var_x197;
        const double var_x448 = pow(var_x447, 10.0);
        const double var_x449 = 1.0 + 2.9619676669542428e-6 * var_x448;
        const double var_x450 = 1 / var_x449;
        const double var_x451 = 0.53437500000000004 + 0.53437500000000004 * var_x450;
        const double var_x452 = var_x446 * var_x451;
        const double var_x453 = var_x452 * var_x50;
        const double var_x454 = 1 / (4.75 + 4.75 * var_x450);
        const double var_x455 = 1.0 + 0.0123 * var_x443;
        const double var_x456 = var_x454 * var_x455;
        const double var_x457 = var_x452 * var_x456;
        const double var_x458 = -var_x139 * var_x457;
        const double var_x459 = -var_chaste_interface__Irel__Irel;
        const double var_x460 = var_x459 - var_x453 * var_chaste_interface__membrane__Vm;
        const double var_x461 = var_x456 * var_x46 * var_x48 * var_chaste_interface__membrane__Vm;
        const double var_x462 = var_x455 * var_x460 / pow((1 + var_x450), 2);
        const double var_x463 = var_x447 * var_x448 / pow(var_x449, 2);
        const double var_x464 = var_x208 * var_x463;
        const double var_x465 = var_x446 * var_x461;
        const double var_x466 = var_x463 * (-1 + var_x193);
        const double var_x467 = 0.80000000000000004 + var_chaste_interface__Ca__Ca_JSR;
        const double var_x468 = pow(var_x467, (-2));
        const double var_x469 = 1 / (1.0 + 8.0 * var_x468);
        const double var_x470 = 0.040000000000000001 * var_x469;
        const double var_x471 = 0.0043750000000000004 * var_chaste_interface__Ca__Ca_i;
        const double var_x472 = var_x205 * var_x471;
        const double var_x473 = var_x200 * var_x471;
        const double var_x474 = 0.0020916619007553534 * var_x65;
        const double var_x475 = var_x120 * var_x123;
        const double var_x476 = 0.0086999999999999994 + var_chaste_interface__Ca__Ca_ss_sr;
        const double var_x477 = 0.00087000000000000001 + var_chaste_interface__Ca__Ca_ss_sr;
        const double var_x478 = 1.0 + 0.0097788000000000007 / pow(var_x476, 2) + 4.0890000000000003e-5 / pow(var_x477, 2);
        const double var_x479 = 1 / var_x478;
        const double var_x480 = 1.0 * var_x479;
        const double var_x481 = var_x154 * var_x65;
        const double var_x482 = 0.023500311662270543 * var_x48;
        const double var_x483 = var_x46 * var_x482;
        const double var_x484 = 0.0086999999999999994 + var_chaste_interface__Ca__Ca_ss_CaL;
        const double var_x485 = 0.00087000000000000001 + var_chaste_interface__Ca__Ca_ss_CaL;
        const double var_x486 = 1.0 + 0.0097788000000000007 / pow(var_x484, 2) + 4.0890000000000003e-5 / pow(var_x485, 2);
        const double var_x487 = 1 / var_x486;
        const double var_x488 = 1.0 * var_x487;
        const double var_x489 = -var_x482 * var_x487 * var_chaste_interface__membrane__Vm;
        const double var_x490 = 4.0105611666400582e-7 * var_x10;
        const double var_x491 = 0.00055993603980397731 * var_x44;
        const double var_x492 = log(100.0 * var_x173);
        const double var_x493 = var_x142 + var_x492;
        ...
        const double var_x497 = var_x141 * var_x495 - var_x141 * var_x496;
        const double var_x498 = var_x173 * var_x495 - var_x173 * var_x496;
        const double var_x499 = var_x51 + 26.711785007306684 * var_x492;
        ...
        const double var_x502 = var_x173 * var_x501 - var_x173 * var_x500;
        const double var_x503 = var_x19 * var_x501 - var_x19 * var_x500;
        const double var_x504 = 6.799858698573652e-6 * var_x106;
        const double var_x505 = -5.0964977286841363e-6 - 3.0578986372104813e-6 * var_x6;
        const double var_x506 = var_x505 * var_x8;
        const double var_x507 = 8.5481717715176459e-7 * var_x41;
        const double var_x508 = 3.0696218160052752e-5 * var_x87;
        const double var_x509 = 3.0245559326093954e-5 * var_x94;
        const double var_x510 = var_x136 * var_x505;
        const double var_x511 = var_x162 * var_x506;
        const double var_x512 = 0.0025000000000000005 * var_x192;
        ...
        IJth(rJacobian, 28, 0) = ((var_x383) ? (-0.693000693000693 * var_x381 * var_chaste_interface__INa_h_gate__h / pow(var_x382, 2)) : ((-0.28123999999999999 * var_x384 - 108500.0 * var_x385) * var_chaste_interface__INa_h_gate__h - 0.14705882352941177 * var_x386 * (0.13500000000000001 - 0.13500000000000001 * var_chaste_interface__INa_h_gate__h)));
        IJth(rJacobian, 29, 0) = ((var_x383) ? (7.6049999999999996e-8 * var_x390 * var_chaste_interface__INa_j_gate__j - 0.029999999999999999 * var_x387 * var_x389 * var_chaste_interface__INa_j_gate__j / pow(var_x388, 2)) : (var_x391 * var_x398 + 0.001275024 * var_x402 * var_chaste_interface__INa_j_gate__j + var_x394 * var_x404 * (3.0508668000000001e-6 * var_x396 - 31073.016 * var_x395) - 0.311 * var_x392 * var_x397 * var_x404 / pow(var_x393, 2) - 0.016701360000000002 * var_x399 * var_x401 * var_chaste_interface__INa_j_gate__j / pow(var_x400, 2)));
        IJth(rJacobian, 30, 0) = var_x375 * var_x405 + var_x377 * var_chaste_interface__INaL_mL_gate__mL - var_x379 * var_x405;
        IJth(rJacobian, 31, 0) = -0.00027322404371584699 * var_x406 / pow((1.0 + var_x406), 2);
        IJth(rJacobian, 32, 0) = (-var_chaste_interface__IKr_xr_gate__xr + 1.0 / var_x408) * (var_x411 + var_x414 - 8.1600000000000005e-5 * var_x412 * var_x415 / pow(var_x413, 2) - 4.566e-5 * var_x409 * var_x416 / pow(var_x410, 2)) + 0.23529411764705882 * var_x407 * (var_x417 + var_x418) / pow(var_x408, 2);
        IJth(rJacobian, 33, 0) = (-var_chaste_interface__Ito1_a_gate__a + 1.0 / var_x420) * (0.028153807491391791 * var_x422 / pow(var_x423, 2) - 0.11912298256720238 * var_x424 / pow(var_x425, 2)) + 0.14019346698443852 * var_x419 * (var_x426 + var_x427) / pow(var_x420, 2);
        IJth(rJacobian, 34, 0) = -var_x432 * (1.0 - var_chaste_interface__Ito1_i1f_gate__i1f) - var_x436 * var_chaste_interface__Ito1_i1f_gate__i1f;
        IJth(rJacobian, 35, 0) = -var_x436 * var_chaste_interface__Ito1_i1s_gate__i1s - 0.00080000000000000004 * var_x441 * (1.0 - var_chaste_interface__Ito1_i1s_gate__i1s) / pow(var_x442, 2);
        IJth(rJacobian, 36, 0) = -0.00062500000000000001 * var_x431 / (var_x438 + 0.20000000000000001 * var_x439) + 0.078125 * var_x437 * (var_x432 - 0.022222222222222223 * var_x435) / pow((var_x439 + 0.125 * var_x437), 2);
        IJth(rJacobian, 37, 0) = var_x456 * (-var_x453 + var_x112 * var_x452 - var_x115 * var_x452);
        IJth(rJacobian, 40, 0) = var_x480 * (1.8292009309104452e-5 * var_x475 + var_x474 * var_x84);
        IJth(rJacobian, 41, 0) = var_x488 * (-var_x483 + 0.0017595463317664707 * var_x111 - 0.0017595463317664707 * var_x114);
        IJth(rJacobian, 42, 0) = 2.6699599581291967e-11 * var_x58 - 7.1319396379664657e-10 * var_x18 - 1.6187618857614559e-6 * var_x184 - 4.3197915704978414e-7 * var_x100 - 2.6699599581291967e-11 * var_x91 - var_x490 * var_chaste_interface__INaL_hL_gate__hL - var_x491 * var_chaste_interface__INa_h_gate__h - 0.00018510282307569501 * var_x65 * var_x75;
        IJth(rJacobian, 43, 0) = -5.4876027927313356e-5 * var_x475 - 0.0062749857022660607 * var_x65 * var_x84;
        IJth(rJacobian, 44, 0) = 1.3882711730677125e-8;
        IJth(rJacobian, 45, 0) = 2.5456399475788057e-7 * var_x124 - 2.5456399475788059e-5 * var_x110 + var_x504 * var_chaste_interface__Ito2_i2f_gate__i2f;
        IJth(rJacobian, 46, 0) = var_x506 + 5.5507608905958737e-8 * var_x59 + 7.5049939616040537e-6 * var_x62 + 2.8798610469985608e-7 * var_x100 - 2.8477357396260765e-8 * var_x53 - 3.1467479922868152e-5 * var_x32 - 1.7029459722963939e-7 * var_x2 - 5.5811305745550463e-8 * var_x117 - var_x507 * var_chaste_interface__IKr_xr_gate__xr - var_x508 * var_chaste_interface__Ito1_i1f_gate__i1f - var_x509 * var_x93;
        ...
        IJth(rJacobian, 39, 1) = -var_x472 + 0.0043750000000000004 * var_x202;
        IJth(rJacobian, 40, 1) = 5.0 * var_x479;
        ...
        IJth(rJacobian, 28, 28) = ((var_x383) ? (-7.6923076923076916 / var_x382) : (-310000.0 * var_x385 - 0.13500000000000001 * var_x386 - 3.5600000000000001 * var_x384));
        IJth(rJacobian, 42, 28) = -var_x143 * var_x491;
        IJth(rJacobian, 0, 29) = mSetVoltageDerivativeToZero ? 0.0 : (-9.0749999999999993 * var_x146);
        IJth(rJacobian, 29, 29) = ((var_x383) ? (-0.29999999999999999 * var_x390) : (-0.1212 * var_x402 - var_x398 * var_x403));
        ...
        IJth(rJacobian, 37, 37) = -var_x456;
        IJth(rJacobian, 38, 37) = -1.0 * var_x469;
        IJth(rJacobian, 40, 37) = 0.23999999999999996 * var_x479;
        IJth(rJacobian, 45, 37) = 2.175954783543569e-6 * var_x151;
        IJth(rJacobian, 37, 38) = -0.0123 * var_x454 * var_x460 / pow(var_chaste_interface__Ca__Ca_JSR, 2) - 17.976374980131517 * var_x443 * var_x444 * var_x451 * var_x461 / pow(var_x445, 2);
        IJth(rJacobian, 38, 38) = -var_x470 + 0.25 * (var_x459 + 0.040000000000000001 * var_chaste_interface__Ca__Ca_NSR - 0.040000000000000001 * var_chaste_interface__Ca__Ca_JSR) / (pow(var_x467, 3) * pow((0.125 + var_x468), 2));
        IJth(rJacobian, 39, 38) = 0.0034782608695652171;
        IJth(rJacobian, 1, 39) = 2.3746312684365782e-5 * var_x187;
        IJth(rJacobian, 38, 39) = var_x470;
        ...
        IJth(rJacobian, 37, 40) = -4.6767910530856466e-10 * var_x462 * var_x464 - 2.667471823152336e-9 * var_x464 * var_x465;
        IJth(rJacobian, 39, 40) = var_x212 * var_x473 + var_x214 * var_x472;
        IJth(rJacobian, 40, 40) = var_x480 * (-5.5 + 6.53644343986048e-11 * var_x481 - 4.8146657448698097e-10 * var_x170 - 5165.5682301054212 * var_x152) + 1.0 * (0.019557600000000001 / pow(var_x476, 3) + 8.1780000000000006e-5 / pow(var_x477, 3)) * (0.5 * var_chaste_interface__Ca__Ca_ss_CaL + 0.23999999999999996 * var_chaste_interface__Irel__Irel + 5.0 * var_chaste_interface__Ca__Ca_i - 5.5 * var_chaste_interface__Ca__Ca_ss_sr + var_x123 * var_x474) / pow(var_x478, 2);
        IJth(rJacobian, 41, 40) = 5.0 * var_x487;
        IJth(rJacobian, 43, 40) = 15496.704690316265 * var_x152 + 1.4443997234609428e-9 * var_x170 - 1.960933031958144e-10 * var_x481;
        ...
        IJth(rJacobian, 37, 41) = -var_x155 * var_x457;
        IJth(rJacobian, 40, 41) = 0.5 * var_x479;
        IJth(rJacobian, 41, 41) = var_x488 * (-5.0 - 0.023500311662270543 * var_x113) + 1.0 * (0.019557600000000001 / pow(var_x484, 3) + 8.1780000000000006e-5 / pow(var_x485, 3)) * (5.0 * var_chaste_interface__Ca__Ca_ss_sr - 5.0 * var_chaste_interface__Ca__Ca_ss_CaL - var_x483 * var_chaste_interface__membrane__Vm) / pow(var_x486, 2);
        ...
        IJth(rJacobian, 42, 42) = -0.14749262536873156 + var_x497 + 2.4740119415402899e-16 * var_x166 + 1.3314155043174696e-5 * var_x157 - 3.4616803112254212e-5 * var_x159 - 7.1319396379664657e-10 * var_x160 - 1.0712924764194231e-5 * var_x161 - 0.0035983988805915112 * var_x168 - 0.014956891113086561 * var_x164;
        IJth(rJacobian, 43, 42) = 5.0;
        IJth(rJacobian, 44, 42) = var_x497;
        IJth(rJacobian, 46, 42) = 2.3077868741502807e-5 * var_x159 + 3.900466760223035 * var_x511 - 8.8761033621164642e-6 * var_x157;
        IJth(rJacobian, 0, 43) = mSetVoltageDerivativeToZero ? 0.0 : (1.3365608910938003e-12 * var_x171 - 4.8600000000000003 * var_x172);
        IJth(rJacobian, 40, 43) = var_x480 * (0.010165476837671019 * var_x172 - 2.7956334939405277e-15 * var_x171);
        ...
        IJth(rJacobian, 44, 44) = -0.14749262536873156 + var_x498 + var_x502 - 3.7083201106826184e-7 * var_x173;
        ...
        IJth(rJacobian, 46, 46) = var_x503 + 212.7914217252065 * var_x511 + 0.00020047178518350236 * var_x180 - 4.5488726691040119e-6 * var_x175 - 2.2833692656630728e-5 * var_x176 - 0.00081995078002871237 * var_x181 - 0.0008405525584213936 * var_x177 - var_x179 * var_x509;
        IJth(rJacobian, 1, 47) = var_x188 * (-var_x206 * var_x219 - var_x213 * var_x218);
        IJth(rJacobian, 37, 47) = 3.5566290975364474e-5 * var_x465 * var_x466 + 6.2357214041141957e-6 * var_x462 * var_x466;
        IJth(rJacobian, 39, 47) = var_x218 * var_x473 + var_x219 * var_x472;
        IJth(rJacobian, 47, 47) = -0.00068000000000000005 - var_x197 * var_x512 + var_x190 * var_x215 * var_x512;
    }
