...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellearm_noble_model_1990FromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time_converted, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x65 = var_x23 * var_x64;
        ...
        const double var_x68 = -26.713760659695652 * var_x1 * log10(140.47999999999999 * var_x67) + var_chaste_interface__membrane__V;
        ...
        const double var_x70 = var_x68 * var_x7;
        const double var_x71 = var_x55 * (0.01 * var_chaste_interface__membrane__V - 0.26713760659695651 * var_x3);
        const double var_x72 = var_x29 * var_x44;
        const double var_x73 = var_x23 * var_x72;
        const double var_x74 = var_x28 * var_x63;
        const double var_x75 = var_x23 * var_x74;
        ...
        const double var_x110 = -0.125 * var_chaste_interface__membrane__V;
        const double var_x111 = exp(-9.375 + var_x110);
        const double var_x112 = exp(-7.5 + var_x105);
        const double var_x113 = exp(-0.80000000000000004 - 0.20000000000000001 * var_chaste_interface__membrane__V);
        const double var_x114 = exp(-0.058823529411764705 * var_chaste_interface__membrane__V);
        const double var_x115 = exp(-1.25 + var_x110);
        const double var_x116 = 1.0 + var_x115;
        const double var_x117 = 0.001 * var_chaste_interface__L_type_calcium_current_d_gate__d;
        const double var_x118 = 19.0 + var_chaste_interface__membrane__V;
        const double var_x119 = fabs(var_x118) < 0.0001;
        const double var_x120 = 0.25 * var_chaste_interface__membrane__V;
        const double var_x121 = -var_x120;
        const double var_x122 = exp(-4.75 + var_x121);
        const double var_x123 = 1.0 - var_x122;
        const double var_x124 = 30.0 / var_x123;
        const double var_x125 = exp(1.8999999999999999 + 0.10000000000000001 * var_chaste_interface__membrane__V);
        const double var_x126 = -1.0 + var_x125;
        const double var_x127 = 12.0 / var_x126;
        const double var_x128 = 34.0 + var_chaste_interface__membrane__V;
        const double var_x129 = fabs(var_x128) < 0.0001;
        const double var_x130 = exp(8.5 + var_x120);
        const double var_x131 = -1.0 + var_x130;
        const double var_x132 = 6.25 / var_x131;
        const double var_x133 = exp(-8.5 + var_x121);
        const double var_x134 = 1.0 + var_x133;
        const double var_x135 = 120.0 * var_x24;
        const double var_x136 = 0.012 / var_x134;
        ...
        const double var_x148 = 1.5752072268398093e-8 * var_x33;
        const double var_x149 = 3.1504144536796189e-9 * var_x42;
        const double var_x150 = 0.00022482145415487096 * var_x3 - 8.4159417694443166e-6 * var_chaste_interface__membrane__V;
        const double var_x151 = 0.5 * var_chaste_interface__intracellular_calcium_concentration__Ca_rel;
        const double var_x152 = -var_x102;
        
        // Matrix entries
        IJth(rJacobian, 0, 0) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x55 * (0.0018699999999999999 + 0.5 * var_x8 + 0.01 * var_x9 + 0.0001 * var_x19 + 0.0011210369457033821 * var_x47 + 0.0048571428571428576 * var_x6 + 9.8651251221897629e-5 * var_x49 - 7.0064809106461387e-7 * var_x51 - 0.00036364351084952748 * var_x20 - 0.00056051847285169105 * var_x52 - 1.4012961821292276e-7 * var_x53 + var_x30 * var_x41 + var_x31 * var_x34 + var_x31 * var_x43));
        IJth(rJacobian, 1, 0) = -2.103985442361079e-8 + 8.4159417694443161e-8 * var_x19 + 2.3586454141088433e-7 * var_x52 - 4.7172908282176866e-7 * var_x47 - var_x30 * var_x87;
        ...
        IJth(rJacobian, 4, 0) = 0.066600000000000006 * var_x113 / pow((1.0 + var_x113), 2);
        IJth(rJacobian, 5, 0) = -0.058823529411764705 * var_x114 * (3.3000000000000003e-5 - 3.3000000000000003e-5 * var_chaste_interface__transient_outward_current_s_gate__s) - 0.0041250000000000002 * var_x115 * var_chaste_interface__transient_outward_current_s_gate__s / pow(var_x116, 2);
        IJth(rJacobian, 6, 0) = (0.001 - var_x117) * ((var_x119) ? (0) : (var_x124 - 7.5 * var_x118 * var_x122 / pow(var_x123, 2))) - var_x117 * ((var_x119) ? (0) : (var_x127 - 1.2 * var_x118 * var_x125 / pow(var_x126, 2)));
        IJth(rJacobian, 7, 0) = -0.001 * ((var_x129) ? (0) : (var_x132 - 1.5625 * var_x128 * var_x130 / pow(var_x131, 2))) * var_chaste_interface__L_type_calcium_current_f_Ca_gate__f_Ca + 0.0030000000000000001 * var_x133 * (var_x29 + var_x135 * var_x27) / pow(var_x134, 2);
        IJth(rJacobian, 8, 0) = 0.048000000000000001 * var_x137 * var_x138;
        IJth(rJacobian, 10, 0) = -1.009913012333318e-7 + 5.8966135352721086e-10 * var_x51 - 2.5247825308332947e-7 * var_x19 - 8.2552589493809517e-8 * var_x49 - 0.00042079708847221582 * var_x8 - var_x148 * var_x31;
        IJth(rJacobian, 11, 0) = -1.4307101008055335e-6 + 1.1793227070544217e-10 * var_x53 + 3.0604026121459156e-7 * var_x20 - 4.0877431451586679e-6 * var_x6 - 4.7172908282176869e-10 * var_x49 - 8.4159417694443166e-6 * var_x9 - var_x149 * var_x31;
        IJth(rJacobian, 0, 1) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x55 * (0.0074867781645490943 * var_x66 + 0.00066784401649239123 * var_x56 - 3.6443148688046648e-7 * var_x58 - 274.40000000000003 * var_x59 - 6.9000000000000006e-7 * var_x60 + var_x34 * var_x65 + var_x41 * var_x64 + var_x43 * var_x65));
        IJth(rJacobian, 1, 1) = -17.0 + var_x88 + var_x89 + 0.0030000000000000001 * var_x93 - 3.1504144536796187e-6 * var_x66 - 3.0670341725380159e-10 * var_x58 - 0.23093344215355205 * var_x59 - 5.8069998209165779e-10 * var_x60 - 5.5199999999999998e-8 * var_x94 - 2.8102681769358871e-7 * var_x56 - 0.0030000000000000001 * var_x91 - var_x64 * var_x87;
        IJth(rJacobian, 7, 1) = var_x136 * (var_x62 + var_x135 * var_x26 - var_x135 * var_x61);
        IJth(rJacobian, 8, 1) = var_x137 * var_x145 + (var_x142 - var_x144) * var_chaste_interface__calcium_release__ActFrac;
        IJth(rJacobian, 9, 1) = var_x145 * var_chaste_interface__calcium_release__ActFrac;
        IJth(rJacobian, 10, 1) = 9.2011025176140482e-10 * var_x58 + 0.69280032646065615 * var_x59 + 1.7420999462749735e-9 * var_x60 - var_x148 * var_x65;
        IJth(rJacobian, 11, 1) = -var_x149 * var_x65;
        IJth(rJacobian, 12, 1) = 2.7047999999999999e-6 * var_x94 + 0.14699999999999999 * var_x91 - 0.14699999999999999 * var_x93;
        IJth(rJacobian, 14, 1) = 2.0 - var_x88;
        IJth(rJacobian, 15, 1) = 15.0 - var_x89;
        IJth(rJacobian, 0, 2) = mSetVoltageDerivativeToZero ? 0.0 : (-0.0015 * var_x54 * var_x69);
        IJth(rJacobian, 2, 2) = -8.0 * var_x109 - 0.001 * ((var_x104) ? (2000.0) : (var_x103 * var_x108));
        IJth(rJacobian, 10, 2) = -0.0012623912654166473 * var_x69;
        IJth(rJacobian, 0, 3) = mSetVoltageDerivativeToZero ? 0.0 : (-0.00050000000000000001 * var_x54 * var_x70);
        IJth(rJacobian, 3, 3) = -2.0 / (1.0 + 320.0 * var_x112) - 0.02 * var_x111;
        IJth(rJacobian, 10, 3) = -0.00042079708847221582 * var_x70;
        IJth(rJacobian, 0, 4) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x71 * var_chaste_interface__transient_outward_current_s_gate__s);
        IJth(rJacobian, 4, 4) = -0.33300000000000002;
        IJth(rJacobian, 11, 4) = var_x150 * var_chaste_interface__transient_outward_current_s_gate__s;
        IJth(rJacobian, 0, 5) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x71 * var_chaste_interface__transient_outward_current_r_gate__r);
        IJth(rJacobian, 5, 5) = -0.033000000000000002 / var_x116 - 3.3000000000000003e-5 * var_x114;
        IJth(rJacobian, 11, 5) = var_x150 * var_chaste_interface__transient_outward_current_r_gate__r;
        IJth(rJacobian, 0, 6) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x55 * (var_x34 * var_x73 + var_x41 * var_x72 + var_x43 * var_x73));
        IJth(rJacobian, 1, 6) = -var_x72 * var_x87;
        IJth(rJacobian, 6, 6) = -0.001 * ((var_x119) ? (120.0) : (var_x118 * var_x124)) - 0.001 * ((var_x119) ? (120.0) : (var_x118 * var_x127));
        IJth(rJacobian, 10, 6) = -var_x148 * var_x73;
        IJth(rJacobian, 11, 6) = -var_x149 * var_x73;
        IJth(rJacobian, 0, 7) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x55 * (-var_x34 * var_x75 - var_x41 * var_x74 - var_x43 * var_x75));
        IJth(rJacobian, 1, 7) = var_x74 * var_x87;
        IJth(rJacobian, 7, 7) = -0.001 * ((var_x129) ? (25.0) : (var_x128 * var_x132)) + var_x136 * (-1.0 - 119.0 * var_x27);
        IJth(rJacobian, 10, 7) = var_x148 * var_x75;
        IJth(rJacobian, 11, 7) = var_x149 * var_x75;
        IJth(rJacobian, 1, 8) = var_x95 * var_x98 - var_x100 * var_x95;
        IJth(rJacobian, 8, 8) = -0.059999999999999998 + var_x146 - var_x141 * var_x143;
        IJth(rJacobian, 9, 8) = 0.059999999999999998 + var_x147;
        IJth(rJacobian, 13, 8) = var_x100 * var_x151 - var_x151 * var_x98;
        IJth(rJacobian, 8, 9) = var_x146 - var_x147;
        IJth(rJacobian, 9, 9) = -0.00059999999999999995;
        IJth(rJacobian, 0, 10) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x55 * (13.356880329847826 * var_x79 + 0.00060000000000000006 * var_x81 + 1.8716945411372739e-5 * var_x83 + 0.0032056512791634783 * var_x76 + 0.11200000000000002 * var_x77 - 7.9686185177944559e-13 * var_x82 - 7.0000000000000007e-5 * var_x78));
        IJth(rJacobian, 1, 10) = 5.0495650616665894e-7 * var_x81 - 6.7063429428673823e-16 * var_x82;
        IJth(rJacobian, 10, 10) = 1.7673477715833064e-7 * var_x78 + 2.0119028828602145e-15 * var_x82 - 0.00028277564345332906 * var_x77 - 1.5752072268398093e-8 * var_x83 - 2.6978574498584515e-6 * var_x76 - 0.011241072707743547 * var_x79 - 1.5148695184999769e-6 * var_x81;
        IJth(rJacobian, 11, 10) = 0.0001885170956355527 * var_x77 - 1.1782318477222043e-7 * var_x78;
        IJth(rJacobian, 0, 11) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x55 * (0.26713760659695651 * var_x85 + 3.7433890822745475e-6 * var_x83 + 0.12975255177566458 * var_x84 + 0.045413393121482605 * var_x2 + 1.6028256395817388 * var_x79 - 0.0097142857142857152 * var_x86));
        IJth(rJacobian, 10, 11) = -0.0013489287249292257 * var_x79;
        IJth(rJacobian, 11, 11) = 8.1754862903173357e-6 * var_x86 - 3.1504144536796189e-9 * var_x83 - 3.8219647206328062e-5 * var_x2 - 0.0001091989920180802 * var_x84 - 0.00022482145415487096 * var_x85;
        IJth(rJacobian, 1, 12) = 5.5199999999999998e-8 * var_x91 + 7.1999999999999999e-7 * var_x93 - 1.3248e-11 * var_x94;
        IJth(rJacobian, 12, 12) = -0.050000000000000003 + 6.4915199999999989e-10 * var_x94 - 2.7047999999999999e-6 * var_x91 - 3.5279999999999994e-5 * var_x93;
        IJth(rJacobian, 13, 12) = 0.0050000000000000001;
        IJth(rJacobian, 1, 13) = 0.051020408163265314 * var_x101;
        IJth(rJacobian, 12, 13) = 0.050000000000000003;
        IJth(rJacobian, 13, 13) = -0.0050000000000000001 - 0.25 * var_x101;
        IJth(rJacobian, 1, 14) = 0.050000000000000003 + var_x102;
        IJth(rJacobian, 14, 14) = -0.050000000000000003 + var_x152;
        IJth(rJacobian, 1, 15) = 0.20000000000000001 + var_x102;
        IJth(rJacobian, 15, 15) = -0.20000000000000001 + var_x152;
    }
