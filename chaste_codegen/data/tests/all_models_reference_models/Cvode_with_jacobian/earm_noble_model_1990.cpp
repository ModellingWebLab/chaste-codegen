...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellearm_noble_model_1990FromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time_converted, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x127 = exp(-7.5 + var_x122 + 0.10000000000000001 * NV_Ith_S(mParameters, 12));
        ...
        const double var_x131 = exp(-9.375 + var_x130 + 0.125 * NV_Ith_S(mParameters, 12));
        const double var_x132 = 0.02 * var_x131;
        const double var_x133 = 2.5 * var_x131;
        const double var_x134 = 20.0 * var_x131;
        const double var_x135 = 0.01 * NV_Ith_S(mParameters, 11);
        const double var_x136 = 1.0 - var_x135;
        const double var_x137 = var_x136 / (var_x134 + 2000.0 * var_x128);
        const double var_x138 = var_x127 / pow((0.0031250000000000002 + var_x127), 2);
        const double var_x139 = exp(-0.80000000000000004 - 0.20000000000000001 * var_chaste_interface__membrane__V);
        ...
        const double var_x143 = 0.001 * var_chaste_interface__L_type_calcium_current_d_gate__d;
        ...
        const double var_x161 = 120.0 * var_x29;
        ...
        const double var_x165 = NV_Ith_S(mParameters, 2) + var_chaste_interface__intracellular_calcium_concentration__Ca_i;
        ...
        const double var_x168 = var_x167 / pow(var_x165, 3);
        const double var_x169 = pow(var_x165, (-2));
        const double var_x170 = var_x169 * var_chaste_interface__intracellular_calcium_concentration__Ca_i;
        const double var_x171 = var_x170 - var_x168;
        const double var_x172 = -0.59999999999999998 * var_x164;
        const double var_x173 = 0.5 * var_x166 * var_x169;
        const double var_x174 = 3.1504144536796185e-7 * var_x40;
        const double var_x175 = 0.00084159417694443163 * NV_Ith_S(mParameters, 4);
        const double var_x176 = var_x174 * NV_Ith_S(mParameters, 4);
        const double var_x177 = 6.3008289073592373e-8 * var_x51;
        const double var_x178 = var_x177 * NV_Ith_S(mParameters, 4);
        const double var_x179 = var_x175 * var_x85;
        const double var_x180 = 0.002 * NV_Ith_S(mParameters, 1);
        ...
        IJth(rJacobian, 2, 0) = (0.001 - 0.001 * var_chaste_interface__fast_sodium_current_m_gate__m) * ((var_x121) ? (0) : (var_x125 - 20.0 * var_x120 * var_x123 / pow(var_x124, 2))) + 0.44800000000000001 * var_x126 * var_chaste_interface__fast_sodium_current_m_gate__m;
        IJth(rJacobian, 3, 0) = (var_x129 + var_x132) * (-var_x133 * var_x137 + 5.0000000000000004e-6 * var_x131 * var_x136 * (var_x133 - 0.625 * var_x138) / pow((var_x128 + 0.01 * var_x131), 2)) + (0.00062500000000000001 * var_x138 - 0.0025000000000000001 * var_x131) * (var_x135 - var_chaste_interface__fast_sodium_current_h_gate__h + var_x134 * var_x137);
        IJth(rJacobian, 4, 0) = 0.066600000000000006 * var_x139 / pow((1.0 + var_x139), 2);
        IJth(rJacobian, 5, 0) = -0.058823529411764705 * var_x140 * (3.3000000000000003e-5 - 3.3000000000000003e-5 * var_chaste_interface__transient_outward_current_s_gate__s) - 0.0041250000000000002 * var_x141 * var_chaste_interface__transient_outward_current_s_gate__s / pow(var_x142, 2);
        IJth(rJacobian, 6, 0) = (0.001 - var_x143) * ((var_x145) ? (0) : (var_x150 - 7.5 * var_x144 * var_x148 / pow(var_x149, 2))) - var_x143 * ((var_x145) ? (0) : (var_x153 - 1.2 * var_x144 * var_x151 / pow(var_x152, 2)));
        IJth(rJacobian, 7, 0) = -0.001 * ((var_x155) ? (0) : (var_x158 - 1.5625 * var_x154 * var_x156 / pow(var_x157, 2))) * var_chaste_interface__L_type_calcium_current_f_Ca_gate__f_Ca + 0.0030000000000000001 * var_x159 * (var_x34 + var_x161 * var_x32) / pow(var_x160, 2);
        IJth(rJacobian, 8, 0) = 0.048000000000000001 * var_x163 * var_x164;
        IJth(rJacobian, 10, 0) = (-1.009913012333318e-7 + 1.1793227070544217e-8 * var_x66 - 0.0025247825308332946 * var_x16 - 1.1793227070544217e-8 * var_x62 - 0.00084159417694443163 * var_x1 - var_x174 * var_x37) * NV_Ith_S(mParameters, 4);
        IJth(rJacobian, 11, 0) = (-1.4307101008055335e-6 + 6.3008289073592378e-5 * var_x55 + 2.3586454141088434e-9 * var_x64 - 2.3586454141088434e-9 * var_x58 - 0.00084159417694443163 * var_x25 - 0.00084159417694443163 * var_x3 - var_x177 * var_x37) * NV_Ith_S(mParameters, 4);
        ...
        IJth(rJacobian, 7, 1) = var_x162 * (var_x76 + var_x161 * var_x31 - var_x161 * var_x75);
        IJth(rJacobian, 8, 1) = var_x163 * var_x171 + (var_x168 - var_x170) * var_chaste_interface__calcium_release__ActFrac;
        IJth(rJacobian, 9, 1) = var_x171 * var_chaste_interface__calcium_release__ActFrac;
        IJth(rJacobian, 10, 1) = (2.5247825308332947e-7 * var_x74 + 1.7420999462749734e-5 * var_x71 + 0.0025247825308332946 * var_x72 - var_x174 * var_x79) * NV_Ith_S(mParameters, 4);
        IJth(rJacobian, 11, 1) = -var_x178 * var_x79;
        ...
        IJth(rJacobian, 2, 2) = -8.0 * var_x126 - 0.001 * ((var_x121) ? (2000.0) : (var_x120 * var_x125));
        ...
        IJth(rJacobian, 3, 3) = -var_x129 - var_x132;
        IJth(rJacobian, 10, 3) = -var_x175 * var_x84;
        IJth(rJacobian, 0, 4) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x2 * var_x86);
        IJth(rJacobian, 4, 4) = -0.33300000000000002;
        IJth(rJacobian, 11, 4) = -var_x179 * var_x2;
        IJth(rJacobian, 0, 5) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x86 * var_x87);
        IJth(rJacobian, 5, 5) = -0.033000000000000002 / var_x142 - 3.3000000000000003e-5 * var_x140;
        IJth(rJacobian, 11, 5) = -var_x179 * var_x87;
        ...
        IJth(rJacobian, 6, 6) = -0.001 * ((var_x145) ? (120.0) : (var_x144 * var_x150)) - 0.001 * ((var_x145) ? (120.0) : (var_x144 * var_x153));
        IJth(rJacobian, 10, 6) = -var_x176 * var_x89;
        IJth(rJacobian, 11, 6) = -var_x178 * var_x89;
        ...
        IJth(rJacobian, 7, 7) = -0.001 * ((var_x155) ? (25.0) : (var_x154 * var_x158)) + var_x162 * (-1.0 - 119.0 * var_x32);
        IJth(rJacobian, 10, 7) = var_x176 * var_x91;
        IJth(rJacobian, 11, 7) = var_x178 * var_x91;
        IJth(rJacobian, 1, 8) = (var_x112 * var_x115 - var_x112 * var_x117) * var_chaste_interface__intracellular_calcium_concentration__Ca_rel;
        IJth(rJacobian, 8, 8) = -0.059999999999999998 + var_x172 - var_x167 * var_x169;
        IJth(rJacobian, 9, 8) = 0.059999999999999998 + var_x173;
        IJth(rJacobian, 13, 8) = (var_x117 * var_x180 - var_x115 * var_x180) * var_chaste_interface__intracellular_calcium_concentration__Ca_rel;
        IJth(rJacobian, 8, 9) = var_x172 - var_x173;
        ...
        IJth(rJacobian, 14, 14) = -0.050000000000000003 + var_x181;
        IJth(rJacobian, 1, 15) = 0.20000000000000001 + var_x119;
        IJth(rJacobian, 15, 15) = -0.20000000000000001 + var_x181;
    }
