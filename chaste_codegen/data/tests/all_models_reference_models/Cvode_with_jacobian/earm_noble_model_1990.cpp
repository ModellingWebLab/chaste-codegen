...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellearm_noble_model_1990FromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time_converted, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x185 = exp(-7.5 + var_x180 + 0.10000000000000001 * NV_Ith_S(mParameters, 12));
        ...
        const double var_x189 = exp(-9.375 + var_x188 + 0.125 * NV_Ith_S(mParameters, 12));
        const double var_x190 = 0.02 * var_x189;
        const double var_x191 = 2.5 * var_x189;
        const double var_x192 = 20.0 * var_x189;
        const double var_x193 = 0.01 * NV_Ith_S(mParameters, 11);
        const double var_x194 = 1.0 - var_x193;
        const double var_x195 = var_x194 / (var_x192 + 2000.0 * var_x186);
        const double var_x196 = var_x185 / pow((0.0031250000000000002 + var_x185), 2);
        const double var_x197 = exp(-0.80000000000000004 - 0.20000000000000001 * var_chaste_interface__membrane__V);
        ...
        const double var_x201 = 0.001 * var_chaste_interface__L_type_calcium_current_d_gate__d;
        ...
        const double var_x219 = 120.0 * var_x33;
        ...
        const double var_x223 = NV_Ith_S(mParameters, 2) + var_chaste_interface__intracellular_calcium_concentration__Ca_i;
        ...
        const double var_x226 = var_x225 / pow(var_x223, 3);
        ...
        const double var_x231 = 0.5 * var_x224 * var_x227;
        const double var_x232 = 0.00084159417694443163 * NV_Ith_S(mParameters, 4);
        const double var_x233 = var_x119 * var_x232;
        const double var_x234 = 0.002 * NV_Ith_S(mParameters, 1);
        ...
        IJth(rJacobian, 2, 0) = (0.001 - 0.001 * var_chaste_interface__fast_sodium_current_m_gate__m) * ((var_x179) ? (0) : (var_x183 - 20.0 * var_x178 * var_x181 / pow(var_x182, 2))) + 0.44800000000000001 * var_x184 * var_chaste_interface__fast_sodium_current_m_gate__m;
        IJth(rJacobian, 3, 0) = (var_x187 + var_x190) * (-var_x191 * var_x195 + 5.0000000000000004e-6 * var_x189 * var_x194 * (var_x191 - 0.625 * var_x196) / pow((var_x186 + 0.01 * var_x189), 2)) + (0.00062500000000000001 * var_x196 - 0.0025000000000000001 * var_x189) * (var_x193 - var_chaste_interface__fast_sodium_current_h_gate__h + var_x192 * var_x195);
        IJth(rJacobian, 4, 0) = 0.066600000000000006 * var_x197 / pow((1.0 + var_x197), 2);
        IJth(rJacobian, 5, 0) = -0.058823529411764705 * var_x198 * (3.3000000000000003e-5 - 3.3000000000000003e-5 * var_chaste_interface__transient_outward_current_s_gate__s) - 0.0041250000000000002 * var_x199 * var_chaste_interface__transient_outward_current_s_gate__s / pow(var_x200, 2);
        IJth(rJacobian, 6, 0) = (0.001 - var_x201) * ((var_x203) ? (0) : (var_x208 - 7.5 * var_x202 * var_x206 / pow(var_x207, 2))) - var_x201 * ((var_x203) ? (0) : (var_x211 - 1.2 * var_x202 * var_x209 / pow(var_x210, 2)));
        IJth(rJacobian, 7, 0) = -0.001 * ((var_x213) ? (0) : (var_x216 - 1.5625 * var_x212 * var_x214 / pow(var_x215, 2))) * var_chaste_interface__L_type_calcium_current_f_Ca_gate__f_Ca + 0.0030000000000000001 * var_x217 * (var_x38 + var_x219 * var_x36) / pow(var_x218, 2);
        IJth(rJacobian, 8, 0) = 0.048000000000000001 * var_x221 * var_x222;
        ...
        IJth(rJacobian, 7, 1) = var_x220 * (var_x94 + var_x219 * var_x35 - var_x219 * var_x93);
        IJth(rJacobian, 8, 1) = var_x221 * var_x229 + (var_x226 - var_x228) * var_chaste_interface__calcium_release__ActFrac;
        IJth(rJacobian, 9, 1) = var_x229 * var_chaste_interface__calcium_release__ActFrac;
        ...
        IJth(rJacobian, 11, 1) = -var_x104 * var_x232;
        ...
        IJth(rJacobian, 2, 2) = -8.0 * var_x184 - 0.001 * ((var_x179) ? (2000.0) : (var_x178 * var_x183));
        ...
        IJth(rJacobian, 3, 3) = -var_x187 - var_x190;
        IJth(rJacobian, 10, 3) = -var_x118 * var_x232;
        IJth(rJacobian, 0, 4) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x120 * var_x2);
        IJth(rJacobian, 4, 4) = -0.33300000000000002;
        IJth(rJacobian, 11, 4) = -var_x2 * var_x233;
        IJth(rJacobian, 0, 5) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x120 * var_x121);
        IJth(rJacobian, 5, 5) = -0.033000000000000002 / var_x200 - 3.3000000000000003e-5 * var_x198;
        IJth(rJacobian, 11, 5) = -var_x121 * var_x233;
        ...
        IJth(rJacobian, 6, 6) = -0.001 * ((var_x203) ? (120.0) : (var_x202 * var_x208)) - 0.001 * ((var_x203) ? (120.0) : (var_x202 * var_x211));
        IJth(rJacobian, 10, 6) = -var_x126 * var_x232;
        IJth(rJacobian, 11, 6) = -var_x129 * var_x232;
        ...
        IJth(rJacobian, 7, 7) = -0.001 * ((var_x213) ? (25.0) : (var_x212 * var_x216)) + var_x220 * (-1.0 - 119.0 * var_x36);
        IJth(rJacobian, 10, 7) = -var_x138 * var_x232;
        IJth(rJacobian, 11, 7) = -var_x141 * var_x232;
        IJth(rJacobian, 1, 8) = (var_x170 * var_x173 - var_x170 * var_x175) * var_chaste_interface__intracellular_calcium_concentration__Ca_rel;
        IJth(rJacobian, 8, 8) = -0.059999999999999998 + var_x230 - var_x225 * var_x227;
        IJth(rJacobian, 9, 8) = 0.059999999999999998 + var_x231;
        IJth(rJacobian, 13, 8) = (var_x175 * var_x234 - var_x173 * var_x234) * var_chaste_interface__intracellular_calcium_concentration__Ca_rel;
        IJth(rJacobian, 8, 9) = var_x230 - var_x231;
        ...
        IJth(rJacobian, 14, 14) = -0.050000000000000003 + var_x235;
        IJth(rJacobian, 1, 15) = 0.20000000000000001 + var_x177;
        IJth(rJacobian, 15, 15) = -0.20000000000000001 + var_x235;
    }
