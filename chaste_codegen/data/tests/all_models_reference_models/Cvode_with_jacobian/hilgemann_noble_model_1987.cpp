...
#include "MathsCustomFunctions.hpp"
    ...
    void Cellhilgemann_noble_model_1987FromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time_converted, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        ...
        const double var_x194 = exp(-7.5 + var_x188 + 0.10000000000000001 * NV_Ith_S(mParameters, 11));
        const double var_x195 = 1 / (1.0 + 320.0 * var_x194);
        const double var_x196 = 2.0 * var_x195;
        const double var_x197 = exp(-9.375 + 0.125 * NV_Ith_S(mParameters, 11) - 0.125 * var_chaste_interface__membrane__V);
        const double var_x198 = 0.02 * var_x197;
        const double var_x199 = 2.5 * var_x197;
        const double var_x200 = 20.0 * var_x197;
        const double var_x201 = 0.01 * NV_Ith_S(mParameters, 10);
        const double var_x202 = 1.0 - var_x201;
        const double var_x203 = var_x202 / (var_x200 + 2000.0 * var_x195);
        const double var_x204 = var_x194 / pow((0.0031250000000000002 + var_x194), 2);
        const double var_x205 = 0.001 * var_chaste_interface__second_inward_calcium_current_d_gate__d;
        ...
        const double var_x235 = 120.0 * var_x5;
        const double var_x236 = 0.012 / var_x234;
        const double var_x237 = 2.5000000000718892e-6 * var_x227;
        const double var_x238 = 1.0 - var_chaste_interface__calcium_release__ActFrac - var_chaste_interface__calcium_release__ProdFrac;
        const double var_x239 = exp(-3.2000000000000002 + 0.080000000000000002 * var_chaste_interface__membrane__V);
        const double var_x240 = NV_Ith_S(mParameters, 2) + var_chaste_interface__intracellular_calcium_concentration__Ca_i;
        ...
        const double var_x243 = var_x242 / pow(var_x240, 3);
        const double var_x244 = pow(var_x240, (-2));
        const double var_x245 = var_x244 * var_chaste_interface__intracellular_calcium_concentration__Ca_i;
        const double var_x246 = var_x245 - var_x243;
        const double var_x247 = -0.59999999999999998 * var_x239;
        const double var_x248 = 0.5 * var_x241 * var_x244;
        const double var_x249 = 1.3149909014756744e-5 * NV_Ith_S(mParameters, 4);
        const double var_x250 = 8.0543192715385049e-10 * var_x115;
        const double var_x251 = 8.054319271538504e-6 * NV_Ith_S(mParameters, 4);
        const double var_x252 = 0.002 * NV_Ith_S(mParameters, 1);
        ...
        IJth(rJacobian, 2, 0) = (0.001 - 0.001 * var_chaste_interface__fast_sodium_current_m_gate__m) * ((var_x187) ? (100.0 * var_x184 + 100.0 * var_x185) : (var_x191 - 20.0 * var_x186 * var_x189 / pow(var_x190, 2))) + 0.44800000000000001 * var_x192 * var_chaste_interface__fast_sodium_current_m_gate__m;
        IJth(rJacobian, 3, 0) = (var_x196 + var_x198) * (-var_x199 * var_x203 + 5.0000000000000004e-6 * var_x197 * var_x202 * (var_x199 - 0.625 * var_x204) / pow((var_x195 + 0.01 * var_x197), 2)) + (0.00062500000000000001 * var_x204 - 0.0025000000000000001 * var_x197) * (var_x201 - var_chaste_interface__fast_sodium_current_h_gate__h + var_x200 * var_x203);
        IJth(rJacobian, 4, 0) = (0.001 - var_x205) * ((var_x212) ? (15.0 * var_x207 + 15.0 * var_x209) : (var_x217 - 7.5 * var_x210 * var_x215 / pow(var_x216, 2))) - var_x205 * ((var_x220) ? (5.9999999999167333 * var_x218 + 6.0000000000832667 * var_x219) : (var_x223 - 1.2 * var_x210 * var_x221 / pow(var_x222, 2)));
        IJth(rJacobian, 5, 0) = -0.001 * ((var_x229) ? (3.125 * var_x226 + 3.125 * var_x227) : (var_x232 - 1.5625 * var_x228 * var_x230 / pow(var_x231, 2))) * var_chaste_interface__second_inward_calcium_current_f_Ca_gate__f_Ca + 0.0030000000000000001 * var_x233 * (var_x10 + var_x235 * var_x8) / pow(var_x234, 2);
        IJth(rJacobian, 6, 0) = 0.048000000000000001 * var_x238 * var_x239;
        ...
        IJth(rJacobian, 5, 1) = var_x236 * (var_x90 + var_x235 * var_x7 - var_x235 * var_x89);
        IJth(rJacobian, 6, 1) = var_x238 * var_x246 + (var_x243 - var_x245) * var_chaste_interface__calcium_release__ActFrac;
        IJth(rJacobian, 7, 1) = var_x246 * var_chaste_interface__calcium_release__ActFrac;
        ...
        IJth(rJacobian, 9, 1) = (8.054319271538504e-6 * var_x114 + 5.5574802973615678e-8 * var_x113 - 8.054319271538504e-6 * var_x110 - 5.3790289324163455e-7 * var_x111 + var_x250 * var_x64) * NV_Ith_S(mParameters, 4);
        IJth(rJacobian, 10, 1) = -var_x100 * var_x249;
        ...
        IJth(rJacobian, 2, 2) = -8.0 * var_x192 - 0.001 * ((var_x187) ? (var_x193 - 499999.99998562218 * (40.999999000000003 + var_chaste_interface__membrane__V) * (-var_x193 - 0.00020000000000575113 * var_x184)) : (var_x186 * var_x191));
        ...
        IJth(rJacobian, 3, 3) = -var_x196 - var_x198;
        IJth(rJacobian, 8, 3) = -var_x120 * var_x249;
        ...
        IJth(rJacobian, 4, 4) = -0.001 * ((var_x212) ? (var_x224 - 1249999.9999640554 * (18.999999599999999 + var_chaste_interface__membrane__V) * (-var_x224 - 1.2000000000345068e-5 * var_x207)) : (var_x210 * var_x217)) - 0.001 * ((var_x220) ? (-var_x225 + 500000.00000643887 * (19.000001000000001 + var_chaste_interface__membrane__V) * (var_x225 + 1.2000000000012001e-5 * var_x219)) : (var_x210 * var_x223));
        IJth(rJacobian, 8, 4) = -var_x125 * var_x249;
        IJth(rJacobian, 9, 4) = -var_x130 * var_x251;
        IJth(rJacobian, 10, 4) = -var_x128 * var_x249;
        ...
        IJth(rJacobian, 5, 5) = -0.001 * ((var_x229) ? (-var_x237 + 1249999.9999640554 * (34.000000399999998 + var_chaste_interface__membrane__V) * (var_x237 + 2.5000000000718892e-6 * var_x226)) : (var_x228 * var_x232)) + var_x236 * (-1.0 - 119.0 * var_x8);
        IJth(rJacobian, 8, 5) = -var_x137 * var_x249;
        IJth(rJacobian, 9, 5) = -var_x143 * var_x251;
        IJth(rJacobian, 10, 5) = -var_x140 * var_x249;
        IJth(rJacobian, 1, 6) = (var_x175 * var_x178 - var_x175 * var_x180) * var_chaste_interface__intracellular_calcium_concentration__Ca_rel;
        IJth(rJacobian, 6, 6) = -0.059999999999999998 + var_x247 - var_x242 * var_x244;
        IJth(rJacobian, 7, 6) = 0.059999999999999998 + var_x248;
        IJth(rJacobian, 12, 6) = (var_x180 * var_x252 - var_x178 * var_x252) * var_chaste_interface__intracellular_calcium_concentration__Ca_rel;
        IJth(rJacobian, 6, 7) = var_x247 - var_x248;
        ...
        IJth(rJacobian, 9, 9) = (-4.9999999999999998e-7 + 5.3790289324163455e-7 * var_x160 - 8.054319271538504e-6 * var_x159 - 8.054319271538504e-6 * var_x161 + var_x250 * var_x68) * NV_Ith_S(mParameters, 4);
        ...
        IJth(rJacobian, 13, 13) = -0.050000000000000003 + var_x253;
        IJth(rJacobian, 1, 14) = 0.20000000000000001 + var_x183;
        IJth(rJacobian, 14, 14) = -0.20000000000000001 + var_x253;
    }
