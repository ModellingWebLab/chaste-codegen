...
// LCOV_EXCL_STOP
        ...
        const double var_time_independent_outward_current__i_K1_converted = 100.00000000000001 * ((fabs(23.0 + var_chaste_interface__membrane__V) < 2.500000000016378e-6) ? ((_lt_0_row[2]) * NV_Ith_S(mParameters, 5)) : ((_lt_0_row[3]) * NV_Ith_S(mParameters, 5))); // uA_per_cm2
...
// LCOV_EXCL_STOP
    ...
    void Cellbeeler_reuter_model_1977FromCellMLCvodeOpt::EvaluateAnalyticJacobian(double var_chaste_interface__environment__time, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : NV_Ith_S(rY, 0));
        // Units: mV; Initial value: -84.624
        double var_chaste_interface__slow_inward_current__Cai = NV_Ith_S(rY, 1);
...
const double var_x0 = 1 / NV_Ith_S(mParameters, 1);
        ...
        const double var_x7 = NV_Ith_S(mParameters, 0) * var_chaste_interface__slow_inward_current_f_gate__f;
        ...
        const double var_x10 = var_x9 * NV_Ith_S(mParameters, 2) * var_chaste_interface__sodium_current_j_gate__j;
        const double var_x11 = exp(3.0800000000000001 + var_x1);
        const double var_x12 = NV_Ith_S(mParameters, 6) / exp(1.4000000000000001 + var_x1);
        ...
        const double var_x20 = var_x0 * (-NV_Ith_S(mParameters, 7) + var_chaste_interface__membrane__V);
        const double var_x21 = var_x20 * NV_Ith_S(mParameters, 2) * var_chaste_interface__sodium_current_h_gate__h;
        const double var_x22 = log(var_chaste_interface__slow_inward_current__Cai);
        const double var_x23 = var_x0 * (7.6990712032745758 - var_chaste_interface__membrane__V - 13.028700000000001 * var_x22);
        const double var_x24 = NV_Ith_S(mParameters, 0) * var_chaste_interface__slow_inward_current_d_gate__d;
        const double var_x25 = 0.076990712032745753 - 0.01 * var_chaste_interface__membrane__V - 0.13028700000000001 * var_x22;
        const double var_x26 = 1.0 - var_chaste_interface__sodium_current_m_gate__m;
        ...
        const double var_x30 = 2.2400000000000002 * var_x29 * var_chaste_interface__sodium_current_m_gate__m;
        ...
        const double var_x39 = 0.25 * NV_Ith_S(mParameters, 4) - 0.25 * var_chaste_interface__membrane__V;
        const double var_x40 = exp(-19.25 + var_x39);
        const double var_x41 = 0.126 * var_x40;
        const double var_x42 = exp(-1.845 + 0.082000000000000003 * NV_Ith_S(mParameters, 4) - 0.082000000000000003 * var_chaste_interface__membrane__V);
        ...
        const double var_x47 = 0.0315 * var_x40;
        const double var_x48 = 0.01 * NV_Ith_S(mParameters, 3);
        const double var_x49 = 1.0 - var_x48;
        const double var_x50 = var_x49 / var_x46;
        const double var_x51 = 0.1394 * var_x42 / pow(var_x43, 2);
        const double var_x52 = exp(-3.2000000000000002 + var_x33 + 0.10000000000000001 * NV_Ith_S(mParameters, 4));
        const double var_x53 = 1.0 + var_x52;
        const double var_x54 = 1 / var_x53;
        const double var_x55 = 0.29999999999999999 * var_x54;
        const double var_x56 = exp(-15.600000000000001 + 0.20000000000000001 * NV_Ith_S(mParameters, 4) - 0.20000000000000001 * var_chaste_interface__membrane__V);
        ...
        const double var_x61 = var_x55 + var_x60;
        const double var_x62 = 0.01375 * var_x59;
        const double var_x63 = var_x49 / var_x61;
        const double var_x64 = 0.011000000000000001 * var_x56 * var_x58 / pow(var_x57, 2);
        const double var_x65 = 0.029999999999999999 * var_x52 / pow(var_x53, 2);
        ...
        const double var_x70 = 1.0 - var_chaste_interface__slow_inward_current_d_gate__d;
        ...
        const double var_x79 = 1.0 - var_chaste_interface__slow_inward_current_f_gate__f;
        ...
        const double var_x88 = 1.0 - var_chaste_interface__time_dependent_outward_current_x1_gate__x1;
        ...
        // Matrix entries
        IJth(rJacobian, 0, 0) = mSetVoltageDerivativeToZero ? 0.0 : (((fabs(var_x16) < 2.500000000016378e-6) ? (var_x0 * (var_x15 + (var_x6 - 0.10000000000000001 / (1.0 - exp(1.0000000000065512e-7)) - 0.10000000000000001 / (1.0 - exp(-1.0000000000065512e-7))) * NV_Ith_S(mParameters, 5))) : (var_x0 * (var_x15 + (var_x6 - 0.20000000000000001 / var_x18 + 0.0080000000000000002 * var_x16 * var_x17 / pow(var_x18, 2)) * NV_Ith_S(mParameters, 5)))));
        IJth(rJacobian, 1, 0) = -0.01 * var_x8;
        IJth(rJacobian, 2, 0) = ((var_x32) ? (var_x30 + var_x26 * (-0.5 * var_x27 - 0.5 * var_x28)) : (var_x30 - var_x26 * var_x36 - 0.10000000000000001 * var_x26 * var_x31 * var_x34 / pow(var_x35, 2)));
        IJth(rJacobian, 3, 0) = var_x46 * (-var_x47 * var_x50 + 0.043598615916955019 * var_x40 * var_x49 * (var_x47 - var_x51) / pow((var_x44 + 0.074117647058823538 * var_x40), 2)) + (var_x51 - var_x47) * (var_x48 - var_chaste_interface__sodium_current_h_gate__h + var_x41 * var_x50);
        IJth(rJacobian, 4, 0) = var_x61 * (var_x63 * var_x64 - var_x62 * var_x63 + 0.61111111111111116 * var_x49 * var_x59 * (var_x62 - var_x64 - var_x65) / pow((var_x54 + 0.18333333333333335 * var_x59), 2)) + (var_x48 - var_chaste_interface__sodium_current_j_gate__j + var_x60 * var_x63) * (var_x64 + var_x65 - var_x62);
        IJth(rJacobian, 5, 0) = 0.0011864406779661019 * var_x69 * var_chaste_interface__slow_inward_current_d_gate__d - 0.00095 * var_x70 * var_x74 + 0.0068394528437724983 * var_x70 * var_x71 * var_x73 / pow(var_x72, 2) + 0.0035000000000000005 * var_x66 * var_x68 * var_chaste_interface__slow_inward_current_d_gate__d / pow(var_x67, 2);
        IJth(rJacobian, 6, 0) = 0.00012999999999999999 * var_x78 * var_chaste_interface__slow_inward_current_f_gate__f - 9.6000000000000002e-5 * var_x79 * var_x83 - 0.0012999999999999999 * var_x75 * var_x77 * var_chaste_interface__slow_inward_current_f_gate__f / pow(var_x76, 2) - 0.0017991004497751124 * var_x79 * var_x80 * var_x82 / pow(var_x81, 2);
        IJth(rJacobian, 7, 0) = 7.7984403119376114e-5 * var_x87 * var_chaste_interface__time_dependent_outward_current_x1_gate__x1 + 4.132231404958678e-5 * var_x88 * var_x92 - 5.1999999999999997e-5 * var_x84 * var_x86 * var_chaste_interface__time_dependent_outward_current_x1_gate__x1 / pow(var_x85, 2) - 2.8571428571428571e-5 * var_x88 * var_x89 * var_x91 / pow(var_x90, 2);
        IJth(rJacobian, 0, 1) = mSetVoltageDerivativeToZero ? 0.0 : (-13.028700000000001 * var_x0 * var_x19);
        IJth(rJacobian, 1, 1) = -0.070000000000000007 - 0.13028700000000001 * var_x19;
        IJth(rJacobian, 0, 2) = mSetVoltageDerivativeToZero ? 0.0 : (-3 * var_x21 * pow(var_chaste_interface__sodium_current_m_gate__m, 2) * var_chaste_interface__sodium_current_j_gate__j);
        IJth(rJacobian, 2, 2) = ((var_x32) ? (var_x37 + var_x38 + 499999.99998562218 * (46.999999000000003 + var_chaste_interface__membrane__V) * (var_x37 + 1.0000000000287557e-6 * var_x27)) : (var_x38 + var_x31 * var_x36));
        IJth(rJacobian, 0, 3) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x10 * var_x20);
        IJth(rJacobian, 3, 3) = -var_x41 - var_x45;
        IJth(rJacobian, 0, 4) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x21 * var_x9);
        IJth(rJacobian, 4, 4) = -var_x55 - var_x60;
        IJth(rJacobian, 0, 5) = mSetVoltageDerivativeToZero ? 0.0 : (var_x23 * var_x7);
        IJth(rJacobian, 1, 5) = var_x25 * var_x7;
        IJth(rJacobian, 5, 5) = -0.070000000000000007 * var_x69 - 0.095000000000000001 * var_x74;
        IJth(rJacobian, 0, 6) = mSetVoltageDerivativeToZero ? 0.0 : (var_x23 * var_x24);
        IJth(rJacobian, 1, 6) = var_x24 * var_x25;
        IJth(rJacobian, 6, 6) = -0.012 * var_x83 - 0.0064999999999999997 * var_x78;
        IJth(rJacobian, 0, 7) = mSetVoltageDerivativeToZero ? 0.0 : (-var_x0 * var_x13);
        IJth(rJacobian, 7, 7) = -0.00050000000000000001 * var_x92 - 0.0012999999999999999 * var_x87;
    }
