...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellfink_noble_giles_model_2008FromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__Environment__time, const double rCurrentGuess[17], double rJacobian[17][17])
    {
        ...
        const double var_x2 = 1.0 + 6.2499999999999997e-8 / pow(var_chaste_interface__Ca__Ca_i, 2);
        const double var_x3 = 1 / var_x2;
        const double var_x4 = -var_chaste_interface__Ca__Ca_ss + var_chaste_interface__Ca__Ca_SR;
        const double var_x5 = pow(var_chaste_interface__Ca__Ca_ss, 2);
        const double var_x6 = pow(var_chaste_interface__Ca__Ca_SR, (-2));
        ...
        const double var_x11 = var_x10 * var_chaste_interface__Irel__R_prime;
        const double var_x12 = var_x11 * var_x9;
        const double var_x13 = 0.045899999999999996 * var_x12;
        const double var_x14 = 1 / (pow((0.44444444444444442 + var_x6), 2) * pow(var_chaste_interface__Ca__Ca_SR, 3));
        const double var_x15 = var_x4 / pow((1 - 0.59999999999999998 * var_x7), 2);
        const double var_x16 = var_x11 * var_x14 * var_x15 * var_x5;
        const double var_x17 = var_x14 * var_chaste_interface__Irel__R_prime;
        const double var_x18 = var_x15 / pow((0.40000000000000002 + var_x9), 2);
        const double var_x19 = var_x17 * var_x18 * var_x8 * pow(var_chaste_interface__Ca__Ca_ss, 4);
        const double var_x20 = 1 / (1.0 + 3.0 * var_x1);
        const double var_x21 = 1.0 * var_x20;
        const double var_x22 = 1 / (pow(var_x2, 2) * pow(var_chaste_interface__Ca__Ca_i, 3));
        const double var_x23 = var_x11 * var_x4 * var_x8 * var_chaste_interface__Ca__Ca_ss;
        const double var_x24 = var_x18 * pow(var_chaste_interface__Ca__Ca_ss, 3) * var_chaste_interface__Irel__R_prime;
        const double var_x25 = var_x10 * var_x4 * var_x9;
        ...
        const double var_x29 = 1 / var_chaste_interface__Ca__Ca_i;
        const double var_x30 = 0.00050000000000000001 + var_chaste_interface__Ca__Ca_i;
        const double var_x31 = 2.2487821987979791e-6 / var_x30;
        ...
        const double var_x37 = 0.91833577176298453 * var_x12;
        ...
        const double var_x46 = -0.11249999999999999 + 0.067500000000000004 * var_x7;
        const double var_x47 = 1 / var_chaste_interface__K__K_i;
        const double var_x48 = pow(var_chaste_interface__iKs_Xs_gate__Xs, 2) * (0.0031249999999999997 * var_chaste_interface__Na__Na_i + 0.10416666666666667 * var_chaste_interface__K__K_i) / pow((0.029999999999999999 * var_chaste_interface__Na__Na_i + var_chaste_interface__K__K_i), 2);
        const double var_x49 = log(5.4000000000000004 * var_x47);
        const double var_x50 = 1 / exp(3.1258035549938383 * var_x49 - 0.10989010989010989 * var_chaste_interface__cell__V);
        const double var_x51 = 1.0 + 0.036532500000000002 * var_x50;
        const double var_x52 = 1 / exp(1.4222406175221964 * var_x49 - 0.050000000000000003 * var_chaste_interface__cell__V);
        const double var_x53 = 1.0 + 0.079111111111111104 * var_x52;
        const double var_x54 = pow(var_x53, 2);
        const double var_x55 = 1 / exp(0.15802673528024405 * var_x49 - 0.0055555555555555558 * var_chaste_interface__cell__V);
        const double var_x56 = 1 / exp(-0.059333333333333335 + 5.9260025730091517 * var_x49 - 0.20833333333333334 * var_chaste_interface__cell__V);
        const double var_x57 = pow(var_x53, 3);
        const double var_x58 = 1 / (var_x57 + 2.0875714285714286 * var_x56 + 0.012714285714285714 * var_x55);
        const double var_x59 = var_x47 * var_x52;
        ...
        const double var_x64 = 1 / (1.0 + 399.99999999999994 * var_x5);
        const double var_x65 = 1 / (2.0 + 80.0 * var_x64);
        const double var_x66 = var_chaste_interface__Ca__Ca_ss / pow((0.0025000000000000005 + var_x5), 2);
        ...
        rJacobian[0][0] = 1.0 - (mDt * (var_x21 * (-0.00036000000000000002 - var_x13 + 0.065280000000000005 * var_x19 - 0.0097919999999999986 * var_x16) + 0.66666666666666674 * (0.00036000000000000002 * var_chaste_interface__Ca__Ca_i + 0.0063749999999999996 * var_x3 - 0.00036000000000000002 * var_chaste_interface__Ca__Ca_SR - var_x13 * var_x4) / (pow(var_x0, 3) * pow((0.33333333333333337 + var_x1), 2))));
        rJacobian[0][1] = -(mDt * (var_x21 * (0.00036000000000000002 + 7.96875e-10 * var_x22)));
        rJacobian[0][2] = -(mDt * (var_x21 * (var_x13 + 0.097919999999999993 * var_x24 - 0.091799999999999993 * var_x23)));
        rJacobian[0][3] = -(mDt * (-0.045899999999999996 * var_x20 * var_x25));
        rJacobian[0][4] = 0.0;
        rJacobian[0][5] = 0.0;
        rJacobian[0][6] = 0.0;
        rJacobian[0][7] = 0.0;
        ...
        rJacobian[1][1] = 1.0 - (mDt * (1.0 * var_x28 * (-0.0038240087783467445 - var_x31 - 0.0086391412367010315 * var_x34 - 5.3144431236283834e-11 * var_x22 - 2.298124085635426e-7 * var_x29 + 2.2487821987979791e-6 * var_chaste_interface__Ca__Ca_i / pow(var_x30, 2)) + 0.00040000000000000002 * (0.0038 * var_chaste_interface__Ca__Ca_ss + 2.40087783467447e-5 * var_chaste_interface__Ca__Ca_SR + 2.298124085635426e-7 * log(2.0 * var_x29) - 0.0038240087783467445 * var_chaste_interface__Ca__Ca_i - 1.7205545223759661e-8 * var_chaste_interface__cell__V - 0.00042515544989027063 * var_x3 - var_x31 * var_chaste_interface__Ca__Ca_i + 1.2593500345045236e-9 * var_x33 * (2.0 * var_x35 * pow(var_chaste_interface__Na__Na_i, 3) - 6860000.0 * var_x32 * var_chaste_interface__Ca__Ca_i)) / (pow(var_x26, 3) * pow(var_x27, 2))));
        ...
        rJacobian[1][15] = 0.0;
        rJacobian[1][16] = 0.0;
        rJacobian[2][0] = -(mDt * (var_x41 * (var_x37 + 0.1959116313094367 * var_x16 - 1.3060775420629114 * var_x19)));
        rJacobian[2][1] = -(mDt * (1.1399999999999999 * var_x40));
        rJacobian[2][2] = 1.0 - (mDt * (var_x41 * (-1.1399999999999999 - var_x37 + 1.8366715435259691 * var_x23 - 1.9591163130943672 * var_x24 - 0.00078728921810821686 * var_x42 * var_x44) + 0.00020000000000000001 * (1.1399999999999999 * var_chaste_interface__Ca__Ca_i - 1.1399999999999999 * var_chaste_interface__Ca__Ca_ss + var_x37 * var_x4 - var_x44 * var_x45) / (pow(var_x38, 3) * pow(var_x39, 2))));
        ...
        rJacobian[2][15] = 0.0;
        rJacobian[2][16] = 0.0;
        rJacobian[3][0] = -(mDt * (0.059999999999999998 * var_x17 * var_chaste_interface__Ca__Ca_ss));
        rJacobian[3][1] = 0.0;
        ...
        rJacobian[4][2] = 0.0;
        rJacobian[4][3] = 0.0;
        rJacobian[4][4] = 1.0 - (mDt * (-0.00073043079046142181 * var_x48 + (-26.713760659695652 * var_x49 + var_chaste_interface__cell__V) * (6.576305692690868e-7 * var_x47 * var_x50 / pow(var_x51, 2) - 9.8566502268897146e-6 * var_x53 * var_x58 * var_x59 - 1.0050926558006126e-5 * var_x54 * (-12.370953657054676 * var_x47 * var_x56 - 0.33754510655860126 * var_x54 * var_x59 - 0.0020091970628488172 * var_x47 * var_x55) / pow((var_x56 + 0.0060904673920481765 * var_x55 + 0.47902552521727226 * var_x57), 2)) + 26.713760659695652 * var_x47 * (-5.7589183215629007e-6 / var_x51 - 4.3801480314950888e-5 * var_x54 * var_x58) - 4.6583596330447822e-5 * var_x47 * var_chaste_interface__iKr_Markov__Or4 - 1.8885766345635723e-5 * var_x47 / (1.0 + exp(4.1806020066889626 - 0.16722408026755853 * var_chaste_interface__cell__V)) - 0.00038819663608706523 * var_x47 * var_chaste_interface__ito_r_gate__r * var_chaste_interface__ito_s_gate__s));
        rJacobian[4][5] = -(mDt * (0.00015902686554868019 * var_x61 - 9.9391790967925116e-8 * var_x62 - 2.1912923713842655e-5 * var_x48));
        rJacobian[4][6] = 0.0;
        rJacobian[4][7] = 0.0;
        rJacobian[4][8] = 0.0;
        rJacobian[4][9] = 0.0;
        ...
        rJacobian[4][14] = 0.0;
        rJacobian[4][15] = 0.0;
        rJacobian[4][16] = -(mDt * (4.6583596330447822e-5 * var_x49 - 1.7438052591648303e-6 * var_chaste_interface__cell__V));
        rJacobian[5][0] = 0.0;
