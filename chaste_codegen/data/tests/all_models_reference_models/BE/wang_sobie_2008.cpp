...
#include "CardiacNewtonSolver.hpp"
    ...
    boost::shared_ptr<RegularStimulus> Cellwang_sobie_2008FromCellMLBackwardEulerNoLut::UseCellMLDefaultStimulus()
    {
    ...
    double Cellwang_sobie_2008FromCellMLBackwardEulerNoLut::GetIntracellularCalciumConcentration()
    {
        return mStateVariables[1];
    }
    Cellwang_sobie_2008FromCellMLBackwardEulerNoLut::Cellwang_sobie_2008FromCellMLBackwardEulerNoLut(boost::shared_ptr<AbstractIvpOdeSolver> /* unused; should be empty */, boost::shared_ptr<AbstractStimulusFunction> pIntracellularStimulus)
                ...
                pIntracellularStimulus)
    {
        ...
        //
        this->mpSystemInfo = OdeSystemInformation<Cellwang_sobie_2008FromCellMLBackwardEulerNoLut>::Instance();
        Init();
    ...
    Cellwang_sobie_2008FromCellMLBackwardEulerNoLut::~Cellwang_sobie_2008FromCellMLBackwardEulerNoLut()
    {
    }

    
    double Cellwang_sobie_2008FromCellMLBackwardEulerNoLut::GetIIonic(const std::vector<double>* pStateVariables)
    {
    ...
    void Cellwang_sobie_2008FromCellMLBackwardEulerNoLut::ComputeResidual(double var_chaste_interface__environment__time, const double rCurrentGuess[23], double rResidual[23])
    {
    ...
    void Cellwang_sobie_2008FromCellMLBackwardEulerNoLut::ComputeJacobian(double var_chaste_interface__environment__time, const double rCurrentGuess[23], double rJacobian[23][23])
    {
        std::vector<double>& rY = rGetStateVariables();
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
        // Units: millivolt; Initial value: -80.6475
        ...
        const double var_x80 = var_x78 * var_x79 * mParameters[3] / ((1500.0 + mParameters[3]) * pow((3.2860264730588296e-7 + var_x79), 2) * (1.0 + 0.1245 * exp(-0.0038949423064193493 * var_chaste_interface__membrane__V) + 0.036499999999999998 * (-0.14285714285714285 + 0.14285714285714285 * exp(1.4858841010401188e-5 * mParameters[4])) * exp(-0.038949423064193495 * var_chaste_interface__membrane__V)));
        const double var_x81 = -0.022348 * exp(0.01176 * var_chaste_interface__membrane__V);
        const double var_x82 = 0.013733 * exp(0.038198000000000003 * var_chaste_interface__membrane__V);
        const double var_x83 = 6.8899999999999994e-5 * exp(-0.041779999999999998 * var_chaste_interface__membrane__V);
        const double var_x84 = 0.0064970000000000002 * exp(-0.16339999999999999 - 0.032680000000000001 * var_chaste_interface__membrane__V);
        const double var_x85 = 0.090820999999999999 * exp(0.11695499999999999 + 0.023390999999999999 * var_chaste_interface__membrane__V);
        ...
        const double var_x90 = exp(-0.025317124991725771 * var_chaste_interface__membrane__V);
        ...
        rJacobian[13][4] = 0.0;
        rJacobian[13][5] = 0.0;
        rJacobian[13][6] = -(mDt * (-0.019019000000000001 * var_x67 * exp(-0.038580246913580245 * pow((-1 + 0.20000000000000001 * var_chaste_interface__membrane__V), 2)) / mParameters[5]));
        rJacobian[13][7] = 0.0;
        rJacobian[13][8] = 0.0;
        rJacobian[13][9] = 0.0;
        rJacobian[13][10] = 0.0;
        ...
        rJacobian[15][13] = 0.0;
        rJacobian[15][14] = 0.0;
        rJacobian[15][15] = 1.0 - (mDt * (-0.023761000000000001 + var_x81 - 0.047002000000000002 * exp(-0.063100000000000003 * var_chaste_interface__membrane__V)));
        ...
        rJacobian[22][20] = 0.0;
        rJacobian[22][21] = 0.0;
        rJacobian[22][22] = 1.0 - (mDt * ((-9.1035704253876838e-8 * var_x80 - 0.004670010223515703 * var_x78 - 571.5059919399248 * var_x92 * pow(var_chaste_interface__sodium_concentration__Nai, 2) * mParameters[1] * exp(0.013632298072467722 * var_chaste_interface__membrane__V) - 1.7961577782752707 * var_x78 * pow(var_chaste_interface__fast_sodium_current_m_gate__m, 3) * mParameters[7] * var_chaste_interface__fast_sodium_current_h_gate__h * var_chaste_interface__fast_sodium_current_j_gate__j) * mParameters[6]));
    }

    void Cellwang_sobie_2008FromCellMLBackwardEulerNoLut::UpdateTransmembranePotential(double var_chaste_interface__environment__time)
    {
        // Time units: millisecond
        std::vector<double>& rY = rGetStateVariables();
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
    ...
    void Cellwang_sobie_2008FromCellMLBackwardEulerNoLut::ComputeOneStepExceptVoltage(double var_chaste_interface__environment__time)
    {
        ...
        const double var_T_type_calcium_current_b_gate__b_infinity = 1 / (1.0 + exp(-7.8688524590163942 - 0.16393442622950821 * var_chaste_interface__membrane__V));
        const double var_T_type_calcium_current_b_gate__tau_b = 0.10000000000000001 + 5.4000000000000004 / (1.0 + exp(15.151515151515152 + 0.15151515151515152 * var_chaste_interface__membrane__V));
        const double var_T_type_calcium_current_g_gate__g_infinity = 1 / (1.0 + exp(10.0 + 0.15151515151515152 * var_chaste_interface__membrane__V));
        const double var_T_type_calcium_current_g_gate__tau_g = 8.0 + 32.0 / (1.0 + exp(13.0 + 0.20000000000000001 * var_chaste_interface__membrane__V));
        ...
        const double var_ultra_rapidly_activating_delayed_rectifier_potassium_current__ass = 1 / (1.0 + exp(-2.9220779220779218 - 0.12987012987012986 * var_chaste_interface__membrane__V));
        const double var_ultra_rapidly_activating_delayed_rectifier_potassium_current__iss = 1 / (1.0 + exp(7.9298245614035094 + 0.17543859649122806 * var_chaste_interface__membrane__V));
        ...
        CardiacNewtonSolver<23,Cellwang_sobie_2008FromCellMLBackwardEulerNoLut>* _p_solver = CardiacNewtonSolver<23,Cellwang_sobie_2008FromCellMLBackwardEulerNoLut>::Instance();
    ...
    std::vector<double> Cellwang_sobie_2008FromCellMLBackwardEulerNoLut::ComputeDerivedQuantities(double var_chaste_interface__environment__time, const std::vector<double> & rY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
...
template<>
void OdeSystemInformation<Cellwang_sobie_2008FromCellMLBackwardEulerNoLut>::Initialise(void)
{
