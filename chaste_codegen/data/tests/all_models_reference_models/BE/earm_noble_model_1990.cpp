...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellearm_noble_model_1990FromCellMLBackwardEulerNoLut::ComputeJacobian(double var_chaste_interface__environment__time_converted, const double rCurrentGuess[10], double rJacobian[10][10])
    {
        ...
        const double var_x26 = var_x23 * var_chaste_interface__intracellular_calcium_concentration__Ca_i - var_x25 * mParameters[5];
        const double var_x27 = 1 / (1.0 - var_x25);
        const double var_x28 = (-50.0 + var_chaste_interface__membrane__V) * mParameters[8] * var_chaste_interface__L_type_calcium_current_d_gate__d;
        const double var_x29 = var_x28 * (1.0 - var_x4);
        const double var_x30 = 6.3008289073592378e-5 * var_x27 * var_x29;
        const double var_x31 = 0.00040816326530612252 * mParameters[1];
        ...
        const double var_x42 = pow(mParameters[7], 3.0);
        const double var_x43 = var_x42 * var_chaste_interface__intracellular_calcium_concentration__Ca_i;
        const double var_x44 = pow(var_chaste_interface__intracellular_sodium_concentration__Na_i, 3.0) * mParameters[5];
        const double var_x45 = 1.0 + 0.0001 * var_x43 + 0.0001 * var_x44;
        ...
        const double var_x50 = (var_x44 * var_x48 - var_x43 * var_x49) * mParameters[14];
        const double var_x51 = var_x46 * var_x50 / pow((0.0068999999999999999 + var_chaste_interface__intracellular_calcium_concentration__Ca_i), 2);
        ...
        const double var_x54 = var_x50 / pow(var_x45, 2);
        const double var_x55 = var_x53 * var_x54;
        const double var_x56 = var_x46 * mParameters[14];
        const double var_x57 = var_x49 * var_x53 * var_x56;
        const double var_x58 = var_x28 * var_x8;
        const double var_x59 = var_x33 * var_x35 * mParameters[1];
        const double var_x60 = var_x52 * pow(var_chaste_interface__intracellular_sodium_concentration__Na_i, 2.0) * mParameters[5];
        const double var_x61 = var_x48 * var_x56 * var_x60;
        const double var_x62 = var_x54 * var_x60;
        const double var_x63 = 0.002 * mParameters[1];
        const double var_x64 = exp(1.8716945411372736 - 0.037433890822745473 * var_chaste_interface__membrane__V);
        const double var_x65 = 1 / (1.0 - var_x64);
        const double var_x66 = var_x29 * var_x65;
        const double var_x67 = 6.3008289073592373e-8 * var_x66;
        const double var_x68 = exp(1.8716945411372736);
        const double var_x69 = (var_x68 * var_chaste_interface__intracellular_potassium_concentration__K_i - var_x64 * mParameters[6]) * mParameters[4];
        ...
        const double var_x72 = var_x71 * mParameters[6];
        const double var_x73 = log(var_x72);
        const double var_x74 = exp(-0.74867781645490938 + var_x24 - 2.0 * var_x73);
        const double var_x75 = 1.0 + var_x74;
        const double var_x76 = var_x72 * mParameters[13] / (10.0 + mParameters[6]);
        const double var_x77 = var_x6 * var_x68;
        const double var_x78 = mParameters[6] / (1.0 + mParameters[6]);
        const double var_x79 = var_x78 / (40.0 + var_chaste_interface__intracellular_sodium_concentration__Na_i);
        const double var_x80 = var_x78 * var_chaste_interface__intracellular_sodium_concentration__Na_i / pow((1 + 0.025000000000000001 * var_chaste_interface__intracellular_sodium_concentration__Na_i), 2);
        const double var_x81 = 3.1504144536796185e-7 * var_x68 * var_chaste_interface__intracellular_sodium_concentration__Na_i - 3.1504144536796185e-7 * var_x64 * mParameters[7];
        const double var_x82 = pow(var_chaste_interface__fast_sodium_current_m_gate__m, 3) * mParameters[10] * var_chaste_interface__fast_sodium_current_h_gate__h / (var_chaste_interface__intracellular_sodium_concentration__Na_i + 0.12 * var_chaste_interface__intracellular_potassium_concentration__K_i);
        ...
        rJacobian[5][1] = -(mDt * ((var_x31 * var_x34 - var_x31 * var_x36) * var_chaste_interface__intracellular_calcium_concentration__Ca_rel));
        rJacobian[5][2] = 0.0;
        rJacobian[5][3] = -(mDt * (0.050000000000000003 + var_x19));
        rJacobian[5][4] = -(mDt * (0.20000000000000001 + var_x19));
        rJacobian[5][5] = 1.0 - (mDt * (-17.0 + var_x21 + var_x22 - 2.8102681769358871e-7 / var_chaste_interface__intracellular_calcium_concentration__Ca_i - 5.806999820916578e-6 * var_x51 - 8.4159417694443161e-8 * var_x55 - 0.00084159417694443163 * var_x57 + (0.0030000000000000001 * var_x40 - 5.5199999999999998e-8 * var_x41 - 0.0030000000000000001 * var_x38) * mParameters[3] - var_x23 * var_x30 * var_x6 - 6.3008289073592378e-5 * var_x26 * var_x27 * var_x58));
        rJacobian[5][6] = -(mDt * (0.00020408163265306126 * var_x59 + 0.00020408163265306126 * mParameters[0]));
        rJacobian[5][7] = -(mDt * ((5.5199999999999998e-8 * var_x38 + 7.1999999999999999e-7 * var_x40 - 1.3248e-11 * var_x41) * mParameters[3]));
        rJacobian[5][8] = 0.0;
        rJacobian[5][9] = -(mDt * (0.0025247825308332946 * var_x61 - 2.5247825308332947e-7 * var_x62));
        rJacobian[6][0] = 0.0;
        rJacobian[6][1] = -(mDt * ((var_x36 * var_x63 - var_x34 * var_x63) * var_chaste_interface__intracellular_calcium_concentration__Ca_rel));
        rJacobian[6][2] = 0.0;
        rJacobian[6][3] = 0.0;
        rJacobian[6][4] = 0.0;
        rJacobian[6][5] = 0.0;
        rJacobian[6][6] = 1.0 - (mDt * (-0.0050000000000000001 - 0.001 * var_x59 - 0.001 * mParameters[0]));
        ...
        rJacobian[7][3] = 0.0;
        rJacobian[7][4] = 0.0;
        rJacobian[7][5] = -(mDt * ((2.7047999999999999e-6 * var_x41 + 0.14699999999999999 * var_x38 - 0.14699999999999999 * var_x40) * mParameters[3]));
        rJacobian[7][6] = -(mDt * (0.050000000000000003));
        rJacobian[7][7] = 1.0 - (mDt * (-0.050000000000000003 + (6.4915199999999989e-10 * var_x41 - 2.7047999999999999e-6 * var_x38 - 3.5279999999999994e-5 * var_x40) * mParameters[3]));
        rJacobian[7][8] = 0.0;
        rJacobian[7][9] = 0.0;
        rJacobian[8][0] = -(mDt * (var_x67 * var_x69));
        rJacobian[8][1] = 0.0;
        rJacobian[8][2] = 0.0;
        rJacobian[8][3] = 0.0;
        rJacobian[8][4] = 0.0;
        rJacobian[8][5] = -(mDt * (-6.3008289073592373e-8 * var_x69 * var_x70));
        rJacobian[8][6] = 0.0;
        rJacobian[8][7] = 0.0;
        rJacobian[8][8] = 1.0 - (mDt * ((-3.8219647206328062e-5 * var_x71 - var_x67 * var_x77 - 0.022482145415487095 * var_x76 / var_x75 + 0.0016831883538888633 * var_x74 * var_x76 * (-26.713760659695652 * var_x73 + var_chaste_interface__membrane__V) / pow(var_x75, 2) - 0.022482145415487095 * var_x71 * mParameters[15] * var_chaste_interface__transient_outward_current_r_gate__r * var_chaste_interface__transient_outward_current_s_gate__s) * mParameters[4]));
        rJacobian[8][9] = -(mDt * ((0.00023564636954444087 * var_x79 - 1.4727898096527556e-7 * var_x80) * mParameters[4]));
        rJacobian[9][0] = -(mDt * (var_x66 * var_x81 * mParameters[4]));
        rJacobian[9][1] = 0.0;
        rJacobian[9][2] = 0.0;
        rJacobian[9][3] = 0.0;
        rJacobian[9][4] = 0.0;
        rJacobian[9][5] = -(mDt * ((2.5247825308332947e-7 * var_x55 + 1.7420999462749734e-5 * var_x51 + 0.0025247825308332946 * var_x57 - var_x70 * var_x81) * mParameters[4]));
        rJacobian[9][6] = 0.0;
        rJacobian[9][7] = 0.0;
        rJacobian[9][8] = -(mDt * (-0.0026978574498584514 * var_x82 * mParameters[4]));
        rJacobian[9][9] = 1.0 - (mDt * ((2.2091847144791331e-7 * var_x80 + 7.5743475924998847e-7 * var_x62 - 2.6978574498584515e-6 / var_chaste_interface__intracellular_sodium_concentration__Na_i - 0.022482145415487095 * var_x82 - 0.00035346955431666128 * var_x79 - 0.0075743475924998846 * var_x61 - 3.1504144536796185e-7 * var_x66 * var_x77) * mParameters[4]));
    }
