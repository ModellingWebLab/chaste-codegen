...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellnoble_SAN_model_1989FromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__environment__time_converted, const double rCurrentGuess[6], double rJacobian[6][6])
    {
        ...
        const double var_x6 = var_x3 * pow(var_chaste_interface__intracellular_calcium_concentration__Cai, 1.0);
        const double var_x7 = pow(var_chaste_interface__intracellular_calcium_concentration__Cai, 3.0) / pow(var_x2, 2);
        ...
        const double var_x10 = exp(3.7433890822745473);
        ...
        const double var_x13 = (-50.0 + var_chaste_interface__membrane__V) * var_chaste_interface__second_inward_current_d_gate__d * var_chaste_interface__second_inward_current_f_gate__f;
        const double var_x14 = 1.1761547293737243e-5 * var_x13 / (1.0 - var_x12);
        ...
        const double var_x19 = 1 / (1.0 + 0.00020000000000000001 * var_x18 + 274.40000000000003 * var_chaste_interface__intracellular_calcium_concentration__Cai);
        const double var_x20 = var_x17 * var_x19;
        const double var_x21 = var_x16 * var_x20;
        const double var_x22 = exp(var_x15);
        const double var_x23 = 2.0 * var_x18 * var_x22 - 2744000.0 * var_x16 * var_chaste_interface__intracellular_calcium_concentration__Cai;
        const double var_x24 = var_x17 * var_x23 / pow((0.0036443148688046646 + 7.2886297376093296e-7 * var_x18 + var_chaste_interface__intracellular_calcium_concentration__Cai), 2);
        const double var_x25 = var_x19 * var_x23 / pow((0.0068999999999999999 + var_chaste_interface__intracellular_calcium_concentration__Cai), 2);
        const double var_x26 = pow(var_chaste_interface__intracellular_sodium_concentration__Nai, 2.0);
        const double var_x27 = var_x20 * var_x22 * var_x26;
        const double var_x28 = var_x24 * var_x26;
        ...
        const double var_x32 = exp(1.8716945411372736 - 0.037433890822745473 * var_chaste_interface__membrane__V);
        const double var_x33 = 5.8807736468686214e-8 * var_x13 / (1.0 - var_x32);
        const double var_x34 = -var_x31 * var_x33 * var_chaste_interface__second_inward_current_f2_gate__f2;
        const double var_x35 = log(3.0 * var_x29);
        const double var_x36 = exp(0.74867781645490938 + var_x11 - 2.0 * var_x35);
        ...
        rJacobian[0][2] = -(mDt * (var_x5 * var_x7 - var_x5 * var_x6));
        rJacobian[0][3] = 0.0;
        rJacobian[0][4] = 0.0;
        rJacobian[0][5] = 0.0;
        ...
        rJacobian[2][2] = 1.0 - (mDt * (-0.20000000000000001 + 0.040000000000000001 * var_chaste_interface__intracellular_calcium_concentration__Ca_up - 9.5418840923404938e-11 * var_x24 - 1.8066221665073799e-10 * var_x25 - 8.7430565504672044e-7 / var_chaste_interface__intracellular_calcium_concentration__Cai - 0.071845959781105084 * var_x21 + var_x6 * var_x9 - var_x7 * var_x9 - var_x10 * var_x14 * var_chaste_interface__second_inward_current_f2_gate__f2));
        rJacobian[2][3] = 0.0;
        rJacobian[2][4] = -(mDt * (1.5709757969629391e-7 * var_x27 - 2.0864178044476298e-16 * var_x28));
        rJacobian[2][5] = -(mDt * (-var_x14 * (-2.0 * var_x12 + var_x10 * var_chaste_interface__intracellular_calcium_concentration__Cai)));
        rJacobian[3][0] = 0.0;
        rJacobian[3][1] = 0.0;
        rJacobian[3][2] = 0.0;
        rJacobian[3][3] = 1.0 - (mDt * (var_x34 - 7.4808371283949473e-6 * var_chaste_interface__time_dependent_potassium_current_x_gate__x - var_x29 * var_x30 - 6.0528853041696024e-5 * var_x29 / var_x37 + 4.5316609527777084e-6 * var_x29 * var_x36 * (-26.713760659695652 * var_x35 + var_chaste_interface__membrane__V) / pow(var_x37, 2)));
        rJacobian[3][4] = -(mDt * (0.00088367388579165321 * var_x38 - 5.5229617861978321e-7 * var_x39));
        rJacobian[3][5] = -(mDt * (-var_x33 * (-3.0 * var_x32 + var_x31 * var_chaste_interface__intracellular_potassium_concentration__Ki)));
        rJacobian[4][0] = 0.0;
        rJacobian[4][1] = 0.0;
        rJacobian[4][2] = -(mDt * (0.21553787934331523 * var_x21 + 5.4198664995221396e-10 * var_x25 + 2.8625652277021485e-10 * var_x24));
        rJacobian[4][3] = -(mDt * (-0.0073630525045414608 * var_x40));
        rJacobian[4][4] = 1.0 - (mDt * (var_x34 + 6.2592534133428895e-16 * var_x28 + 8.2844426792967482e-7 * var_x39 - 0.061358770871178837 * var_x40 - 0.0013255108286874798 * var_x38 - 2.4480558341308171e-5 * var_x41 - 4.7129273908888172e-7 * var_x27 - var_x30 * var_x41));
        rJacobian[4][5] = -(mDt * (-var_x33 * (-140.0 * var_x32 + var_x31 * var_chaste_interface__intracellular_sodium_concentration__Nai)));
        rJacobian[5][0] = 0.0;
        rJacobian[5][1] = 0.0;
