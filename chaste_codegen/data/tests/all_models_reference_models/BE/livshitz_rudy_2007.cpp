...
#include "CardiacNewtonSolver.hpp"
    ...
    void Celllivshitz_rudy_2007FromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__Environment__time, const double rCurrentGuess[7], double rJacobian[7][7])
    {
        ...
        const double var_x26 = exp(0.074871767015605231 * var_chaste_interface__cell__V);
        const double var_x27 = 1 / (-1.0 + var_x26);
        const double var_x28 = var_chaste_interface__ICaL__d * var_chaste_interface__ICaL__f * var_chaste_interface__cell__V;
        const double var_x29 = var_x28 / (-67.26666666666668 + 555.55555555555566 * var_chaste_interface__Ca__Ca_T + 1111.1111111111113 * var_x13);
        const double var_x30 = var_x27 * var_x29;
        const double var_x31 = var_x19 * var_x26 * var_x30;
        const double var_x32 = exp(-0.031820500981632228 * var_chaste_interface__cell__V);
        const double var_x33 = 914666.66666666663 * var_chaste_interface__Ca__Ca_T;
        const double var_x34 = pow(var_chaste_interface__Na__Na_i, 3);
        const double var_x35 = 0.037435883507802616 * var_chaste_interface__cell__V;
        const double var_x36 = exp(var_x35);
        const double var_x37 = 1.8 * var_x34 * var_x36;
        const double var_x38 = 1829333.3333333333 * var_x13;
        const double var_x39 = 1.0 + 0.0001 * var_x32 * (-112394.24000000001 + var_x33 + var_x37 + var_x38);
        const double var_x40 = var_x32 / var_x39;
        const double var_x41 = var_x40 * (-914666.66666666663 - 1829333.3333333333 * var_x15 - 135506.17283950618 * var_x17);
        const double var_x42 = pow((-0.061440000000000002 + var_x21), (-2));
        const double var_x43 = var_x23 * var_x42 * (-0.022755555555555557 + 0.37037037037037035 * var_x13 + 0.18518518518518517 * var_chaste_interface__Ca__Ca_T);
        const double var_x44 = -0.61380000000000001 + 1.0 * var_x22 * var_x26;
        const double var_x45 = var_x28 * (-555.55555555555566 - 1111.1111111111113 * var_x15 - 82.304526748971199 * var_x17) / pow((-0.060540000000000004 + var_x13 + 0.5 * var_chaste_interface__Ca__Ca_T), 2);
        const double var_x46 = var_x27 * var_x44 * var_x45;
        const double var_x47 = pow(var_x32, 2) * (112394.24000000001 + var_x37 - var_x33 - var_x38) / pow(var_x39, 2);
        const double var_x48 = var_x47 * (91.466666666666669 + 182.93333333333334 * var_x15 + 13.550617283950618 * var_x17);
        ...
        const double var_x52 = var_x47 * var_x50;
        ...
        const double var_x59 = var_x30 * var_x44;
        ...
        const double var_x63 = 0.75 * var_x36;
        const double var_x64 = 1 / (-1.0 + var_x36);
        ...
        const double var_x71 = var_x29 * var_x36 * var_x64;
        const double var_x72 = log(4.5 * var_x70);
        ...
        const double var_x79 = exp(-36.698642499999998 + 0.061749999999999999 * var_chaste_interface__cell__V - 1.6494869150645177 * var_x72);
        const double var_x80 = exp(0.43983232 + 0.080320000000000003 * var_chaste_interface__cell__V - 2.1455350448256207 * var_x72);
        const double var_x81 = 1.0 * var_x79 + 0.49124000000000001 * var_x80;
        const double var_x82 = var_x78 * var_x81;
        const double var_x83 = var_x70 / (var_x82 + 1.02 * var_x75);
        const double var_x84 = var_x69 * (-2.663848166806566e-5 - 1.5983089000839395e-5 / var_x67) * (0.14151877954204523 * var_chaste_interface__K__K_i + 0.0025940392290056888 * var_chaste_interface__Na__Na_i) / pow(var_x68, 2);
        const double var_x85 = -26.712338705498265 * var_x72 + var_chaste_interface__cell__V;
        const double var_x86 = var_x73 / pow(var_x74, 2);
        const double var_x87 = 1 / (var_x34 * pow((0.01 + 1 / var_x49), 2) * (1.0 + 0.1245 * exp(-0.003743588350780262 * var_chaste_interface__cell__V) + 0.036499999999999998 * (-0.14285714285714285 + 0.14285714285714285 * exp(2.0802377414561666)) * exp(-var_x35)));
        ...
        rJacobian[2][2] = 1.0 - (mDt * (1.5380185720592182e-8 * var_x41 + 5.0183373527362646e-6 * var_x43 - 0.0015981617647058825 * var_x25 - 0.00023999003823076885 * var_x31 - 1.9439193096692272e-10 * var_x46 - 1.5380185720592182e-8 * var_x48 - 0.00071029411764705886 * var_x20 - 3.5374427157362017e-5 * var_x19 / (-0.040460000000000003 + var_x14) - 7.9592461104064537e-5 * var_x24 / pow((-0.060690000000000008 + var_x21), 2) + 8.319524789019007e-5 * var_x43 * pow(var_chaste_interface__ICaT__b, 2) * var_chaste_interface__ICaT__g));
        rJacobian[2][3] = 0.0;
        ...
        rJacobian[4][1] = 0.0;
        rJacobian[4][2] = -(mDt * (var_x62 * (-var_x31 * var_x58 - 3.7522371676265542e-6 * var_x46 * var_x57)));
        rJacobian[4][3] = 0.0;
        ...
        rJacobian[5][0] = 0.0;
        rJacobian[5][1] = 0.0;
        rJacobian[5][2] = -(mDt * (-3.4738558033903784e-14 * var_x65 * (-3.375 + var_x63 * var_chaste_interface__K__K_i) + 3.2633849539327485e-11 * var_x24 * var_x42 * var_x66 * var_x69 * (-26.712338705498265 * log(7.0662000000000003 / var_x68) + var_chaste_interface__cell__V) / pow(var_x67, 2)));
        rJacobian[5][3] = 0.0;
        rJacobian[5][4] = 0.0;
        rJacobian[5][5] = 1.0 - (mDt * (188.75472776079184 * var_x84 - 3.2165331512873877e-8 * var_x71 - 9.0713633255081308e-6 * var_x70 / (1.0 + exp(1.2521739130434781 - 0.16722408026755853 * var_chaste_interface__cell__V)) - 0.0011476363512500001 * var_x75 * var_x83 + 0.000273711269773125 * var_x83 * var_x85 * var_x86 - 3.9214659113300655e-5 * var_x70 * var_chaste_interface__IKr__xr / (1.0 + exp(0.4017857142857143 + 0.044642857142857144 * var_chaste_interface__cell__V)) - 4.129448361660385e-5 * var_x75 * var_x85 * (-var_x78 * (1.053972635420138 * var_x70 * var_x80 + 1.6494869150645177 * var_x70 * var_x79) + 6.4983106368865622 * var_x70 * var_x86 - 13.738155796237757 * var_x70 * var_x76 * var_x81 / pow(var_x77, 2)) / pow((var_x75 + 0.98039215686274506 * var_x82), 2)));
        rJacobian[5][6] = -(mDt * (3.459874159855314 * var_x84 + 4.1526501445598893e-6 * var_x87));
        rJacobian[6][0] = 0.0;
        rJacobian[6][1] = 0.0;
        rJacobian[6][2] = -(mDt * (4.6140557161776547e-8 * var_x48 - 4.6140557161776547e-8 * var_x41 - 1.214949568543267e-13 * var_x65 * (-105.0 + var_x63 * var_chaste_interface__Na__Na_i)));
        rJacobian[6][3] = 0.0;
        rJacobian[6][4] = 0.0;
        rJacobian[6][5] = 0.0;
        rJacobian[6][6] = 1.0 - (mDt * (2.4915900867359339e-11 * var_x52 - 2.4915900867359336e-7 * var_x51 - 6.2289752168398339e-6 * var_x87 - 6.5734516851508199e-6 * var_x88 - 1.1249533042067289e-7 * var_x71 - 0.02629380674060328 * var_x88 * pow(var_chaste_interface__INa__m, 3) * var_chaste_interface__INa__H * var_chaste_interface__INa__J));
    }
