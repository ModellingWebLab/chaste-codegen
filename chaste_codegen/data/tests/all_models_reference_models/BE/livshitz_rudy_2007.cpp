...
#include "CardiacNewtonSolver.hpp"
    ...
    void Celllivshitz_rudy_2007FromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__Environment__time, const double rCurrentGuess[7], double rJacobian[7][7])
    {
        ...
        const double var_x23 = var_x22 * (-0.33333333333333331 - var_x16 - var_x18);
        const double var_x24 = var_x23 / pow((-0.060060000000000002 + var_x21), 2);
        const double var_x25 = var_x23 / pow((-0.061440000000000002 + var_x21), 2);
        ...
        const double var_x36 = pow(var_x26, 2) * (112394.24000000001 + var_x31 - var_x27 - var_x32) / pow(var_x33, 2);
        const double var_x37 = var_x36 * (91.466666666666669 + 182.93333333333334 * var_x15 + 13.550617283950618 * var_x17);
        const double var_x38 = exp(0.074871767015605231 * var_chaste_interface__cell__V);
        const double var_x39 = 1 / (-1.0 + var_x38);
        const double var_x40 = mParameters[0] * var_chaste_interface__ICaL__d * var_chaste_interface__ICaL__f * var_chaste_interface__cell__V;
        const double var_x41 = var_x40 / (-67.26666666666668 + 555.55555555555566 * var_chaste_interface__Ca__Ca_T + 1111.1111111111113 * var_x13);
        const double var_x42 = var_x39 * var_x41;
        const double var_x43 = var_x19 * var_x38 * var_x42;
        const double var_x44 = -0.61380000000000001 + 1.0 * var_x22 * var_x38;
        const double var_x45 = var_x40 * (-555.55555555555566 - 1111.1111111111113 * var_x15 - 82.304526748971199 * var_x17) / pow((-0.060540000000000004 + var_x13 + 0.5 * var_chaste_interface__Ca__Ca_T), 2);
        const double var_x46 = var_x39 * var_x44 * var_x45;
        const double var_x47 = pow(var_chaste_interface__Na__Na_i, 2);
        const double var_x48 = var_x30 * var_x47;
        const double var_x49 = var_x34 * var_x48;
        const double var_x50 = var_x36 * var_x48;
        const double var_x51 = -5.4000000000000004 + 0.5 * var_chaste_interface__Ca__Ca_JSR_T + 5.4000000000000004 * var_x1;
        const double var_x52 = 1 / var_x51;
        const double var_x53 = pow(var_x52, 9.0);
        const double var_x54 = 1.0 + 1.0 * var_x53;
        const double var_x55 = 1 / var_x54;
        const double var_x56 = 4.6323915649710559 * var_x55;
        const double var_x57 = var_x42 * var_x44;
        const double var_x58 = (-0.5 - 5.4000000000000004 * var_x2) / pow((-1 + var_x0 + var_x1), 2);
        const double var_x59 = 0.0025894736842105265 * var_x52;
        const double var_x60 = 0.21052631578947367 + var_x59;
        const double var_x61 = pow(1 / var_x22, 1.3999999999999999);
        const double var_x62 = 1.0 + 6.4818210260626455e-7 * var_x61;
        const double var_x63 = 1 / (0.018329999999999999 * var_chaste_interface__Na__Na_i + var_chaste_interface__K__K_i);
        const double var_x64 = log(10);
        const double var_x65 = 26.712338705498265 * var_x64;
        ...
        const double var_x71 = log10(4.5 * var_x70);
        const double var_x72 = var_x64 * var_x71;
        const double var_x73 = exp(-14.1227775 + 0.23849999999999999 * var_chaste_interface__cell__V - 6.3708927812613361 * var_x72);
        const double var_x74 = 1.0 + var_x73;
        const double var_x75 = 1 / var_x74;
        const double var_x76 = exp(-2.4444678999999998 + 13.738155796237757 * var_x72 - 0.51429999999999998 * var_chaste_interface__cell__V);
        const double var_x77 = 1.0 + var_x76;
        const double var_x78 = 1 / var_x77;
        const double var_x79 = exp(0.43983232 + 0.080320000000000003 * var_chaste_interface__cell__V - 2.1455350448256207 * var_x72);
        const double var_x80 = exp(-36.698642499999998 + 0.061749999999999999 * var_chaste_interface__cell__V - 1.6494869150645177 * var_x72);
        const double var_x81 = var_x80 + 0.49124000000000001 * var_x79;
        const double var_x82 = var_x78 * var_x81;
        const double var_x83 = var_x70 / (var_x82 + 1.02 * var_x75);
        const double var_x84 = var_x63 * var_x66 * (-2.663848166806566e-5 - 1.5983089000839395e-5 / var_x62);
        const double var_x85 = -var_x65 * var_x71 + var_chaste_interface__cell__V;
        const double var_x86 = var_x73 / pow(var_x74, 2);
        const double var_x87 = var_x30 * var_x41 * var_x68;
        const double var_x88 = 1 / (var_x28 * pow((0.01 + 1 / var_x47), 2) * (1.0 + 0.1245 * exp(-0.003743588350780262 * var_chaste_interface__cell__V) + 0.036499999999999998 * (-0.14285714285714285 + 0.14285714285714285 * exp(2.0802377414561666)) * exp(-var_x29)));
        ...
        rJacobian[1][2] = -(mDt * (0.0087500000000000008 * var_x20 + 0.0196875 * var_x24));
        rJacobian[1][3] = 0.0;
        rJacobian[1][4] = 0.0;
        rJacobian[1][5] = 0.0;
        rJacobian[1][6] = 0.0;
        ...
        rJacobian[2][2] = 1.0 - (mDt * (2.7879651959645917e-6 * var_x25 + 1.5380185720592182e-8 * var_x35 - 0.0015981617647058825 * var_x24 - 0.00023999003823076885 * var_x43 - 1.9439193096692272e-10 * var_x46 - 1.5380185720592182e-8 * var_x37 - 0.00071029411764705886 * var_x20 - 3.5374427157362017e-5 * var_x19 / (-0.040460000000000003 + var_x14) - 7.9592461104064537e-5 * var_x23 / pow((-0.060690000000000008 + var_x21), 2) + 4.6219582161216707e-5 * var_x25 * pow(var_chaste_interface__ICaT__b, 2) * var_chaste_interface__ICaT__g));
        rJacobian[2][3] = 0.0;
        rJacobian[2][4] = -(mDt * (0.0070588235294117632));
        rJacobian[2][5] = 0.0;
        rJacobian[2][6] = -(mDt * (8.3053002891197791e-8 * var_x49 - 8.3053002891197795e-12 * var_x50));
        rJacobian[3][0] = 0.0;
        rJacobian[3][1] = 0.0;
        rJacobian[3][2] = 0.0;
        rJacobian[3][3] = 1.0;
        ...
        rJacobian[3][5] = 0.0;
        rJacobian[3][6] = 0.0;
        rJacobian[4][0] = -(mDt * (8.8802252544942608e-5 * var_x58 * (-var_chaste_interface__Irel__Rel - var_x56 * var_x57) + 1.4297504830157579 * var_x51 * var_x53 * var_x57 * var_x58 * var_x60 / pow(var_x54, 2)));
        rJacobian[4][1] = 0.0;
        rJacobian[4][2] = -(mDt * (var_x60 * (-var_x43 * var_x56 - 3.7522371676265542e-6 * var_x46 * var_x55)));
        rJacobian[4][3] = 0.0;
        rJacobian[4][4] = 1.0 - (mDt * (-0.21052631578947367 - var_x59));
        rJacobian[4][5] = 0.0;
        rJacobian[4][6] = 0.0;
        rJacobian[5][0] = 0.0;
        rJacobian[5][1] = 0.0;
        rJacobian[5][2] = -(mDt * (-3.4738558033903784e-14 * var_x69 * (-3.375 + var_x67 * var_chaste_interface__K__K_i) + 3.2633849539327485e-11 * var_x25 * var_x61 * var_x66 * (-var_x65 * log10(7.0662000000000003 * var_x63) + var_chaste_interface__cell__V) / pow(var_x62, 2)));
        rJacobian[5][3] = 0.0;
        rJacobian[5][4] = 0.0;
        rJacobian[5][5] = 1.0 - (mDt * (26.712338705498265 * var_x84 - 3.2165331512873877e-8 * var_x87 - 9.0713633255081308e-6 * var_x70 / (1.0 + exp(1.2521739130434781 - 0.16722408026755853 * var_chaste_interface__cell__V)) - 0.0011476363512500001 * var_x75 * var_x83 + 0.000273711269773125 * var_x83 * var_x85 * var_x86 - 0.0015001782369281045 * var_x70 * mParameters[2] * var_chaste_interface__IKr__xr / (1.0 + exp(0.4017857142857143 + 0.044642857142857144 * var_chaste_interface__cell__V)) - 4.129448361660385e-5 * var_x75 * var_x85 * (-var_x78 * (1.053972635420138 * var_x70 * var_x79 + 1.6494869150645177 * var_x70 * var_x80) + 6.4983106368865622 * var_x70 * var_x86 - 13.738155796237757 * var_x70 * var_x76 * var_x81 / pow(var_x77, 2)) / pow((var_x75 + 0.98039215686274506 * var_x82), 2)));
        rJacobian[5][6] = -(mDt * (4.1526501445598893e-6 * var_x88 + 0.48963716847178318 * var_x84));
        rJacobian[6][0] = 0.0;
        rJacobian[6][1] = 0.0;
        rJacobian[6][2] = -(mDt * (4.6140557161776547e-8 * var_x37 - 4.6140557161776547e-8 * var_x35 - 1.214949568543267e-13 * var_x69 * (-105.0 + var_x67 * var_chaste_interface__Na__Na_i)));
        rJacobian[6][3] = 0.0;
        rJacobian[6][4] = 0.0;
        rJacobian[6][5] = 0.0;
        rJacobian[6][6] = 1.0 - (mDt * (2.4915900867359339e-11 * var_x50 - 2.4915900867359336e-7 * var_x49 - 6.2289752168398339e-6 * var_x88 - 6.5734516851508199e-6 * var_x89 - 1.1249533042067289e-7 * var_x87 - 0.001643362921287705 * var_x89 * pow(var_chaste_interface__INa__m, 3) * mParameters[1] * var_chaste_interface__INa__H * var_chaste_interface__INa__J));
    }
