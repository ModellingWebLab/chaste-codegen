...
#include "CardiacNewtonSolver.hpp"
    ...
    boost::shared_ptr<RegularStimulus> Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut::UseCellMLDefaultStimulus()
    {
    ...
    double Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut::GetIntracellularCalciumConcentration()
    {
        return mStateVariables[1];
    }
    Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut::Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut(boost::shared_ptr<AbstractIvpOdeSolver> /* unused; should be empty */, boost::shared_ptr<AbstractStimulusFunction> pIntracellularStimulus)
                ...
                pIntracellularStimulus)
    {
        ...
        //
        this->mpSystemInfo = OdeSystemInformation<Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut>::Instance();
        Init();
    ...
    Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut::~Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut()
    {
    }

    
    
    void Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut::VerifyStateVariables()
    {
    ...
    double Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut::GetIIonic(const std::vector<double>* pStateVariables)
    {
    ...
    void Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut::ComputeResidual(double var_chaste_interface__environment__time, const double rCurrentGuess[7], double rResidual[7])
    {
        std::vector<double>& rY = rGetStateVariables();
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
        // Units: millivolt; Initial value: -85.423
    ...
    void Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut::ComputeJacobian(double var_chaste_interface__environment__time, const double rCurrentGuess[7], double rJacobian[7][7])
    {
        std::vector<double>& rY = rGetStateVariables();
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
        // Units: millivolt; Initial value: -85.423
        ...
        const double var_x35 = 1 / var_chaste_interface__calcium_dynamics__Ca_i;
        const double var_x36 = 0.00050000000000000001 + var_chaste_interface__calcium_dynamics__Ca_i;
        const double var_x37 = mParameters[10] / var_x36;
        const double var_x38 = exp(-0.024332029034784559 * var_chaste_interface__membrane__V);
        const double var_x39 = pow(mParameters[6], 3);
        const double var_x40 = var_x38 * var_x39;
        const double var_x41 = mParameters[19] / ((1.0 + 0.10000000000000001 * var_x38) * (669921.875 + var_x39) * (1.3799999999999999 + mParameters[4]));
        const double var_x42 = var_x40 * var_x41;
        const double var_x43 = mParameters[4] * exp(0.013101861787960915 * var_chaste_interface__membrane__V);
        const double var_x44 = var_x41 * var_x43 * pow(var_chaste_interface__sodium_dynamics__Na_i, 2);
        const double var_x45 = 0.00025000000000000001 + var_chaste_interface__calcium_dynamics__Ca_ss;
        const double var_x46 = 1.0 + 0.0001 / pow(var_x45, 2);
        const double var_x47 = 1 / var_x46;
        const double var_x48 = exp(9.9999999999858739e-8);
        const double var_x49 = 1 / (-1.0 + var_x48);
        const double var_x50 = -mParameters[4];
        const double var_x51 = 0.25 * var_chaste_interface__calcium_dynamics__Ca_ss;
        const double var_x52 = exp(-9.9999999999858739e-8);
        const double var_x53 = 1 / (-1.0 + var_x52);
        const double var_x54 = 1.3356880329828957e-6 * var_x53 * (var_x50 + var_x51 * var_x52);
        const double var_x55 = -15.0 + var_chaste_interface__membrane__V;
        const double var_x56 = fabs(var_x55) < 1.3356880329828957e-6;
        const double var_x57 = exp(-1.1230167246823641 + 0.074867781645490947 * var_chaste_interface__membrane__V);
        const double var_x58 = var_x55 / (-1.0 + var_x57);
        const double var_x59 = 1369.1986401882032 * mParameters[7] * var_chaste_interface__L_type_Ca_current_d_gate__d * var_chaste_interface__L_type_Ca_current_f2_gate__f2 * var_chaste_interface__L_type_Ca_current_f_gate__f * mParameters[11];
        const double var_x60 = var_x59 * ((var_x56) ? (-var_x54 + 374338.90822798351 * (-14.999998664311967 + var_chaste_interface__membrane__V) * (var_x54 + 1.3356880329828957e-6 * var_x49 * (var_x50 + var_x48 * var_x51))) : (var_x58 * (var_x50 + var_x51 * var_x57)));
        const double var_x61 = 3.0010972933430868 * var_x7;
        const double var_x62 = var_x11 * var_x61;
        const double var_x63 = var_x47;
        const double var_x64 = 3.3392200824572393e-7 * var_x52 * var_x53;
        const double var_x65 = -0.11249999999999999 + 0.067500000000000004 * var_x5;
        const double var_x66 = 1 / var_chaste_interface__potassium_dynamics__K_i;
        const double var_x67 = 0.016878114612481093 * var_x66;
        const double var_x68 = pow(var_chaste_interface__slow_time_dependent_potassium_current_Xs_gate__Xs, 2) * mParameters[18] / (0.029999999999999999 * var_chaste_interface__sodium_dynamics__Na_i + var_chaste_interface__potassium_dynamics__K_i);
        const double var_x69 = log(var_x66 * mParameters[5]);
        const double var_x70 = exp(-12.0 + 0.059999999999999998 * var_chaste_interface__membrane__V - 1.6028256395817388 * var_x69);
        const double var_x71 = 1.0 + var_x70;
        const double var_x72 = 1 / var_x71;
        const double var_x73 = exp(13.356880329847826 * var_x69 - 0.5 * var_chaste_interface__membrane__V);
        const double var_x74 = 1.0 + var_x73;
        const double var_x75 = 1 / var_x74;
        const double var_x76 = exp(0.02 + 0.00020000000000000001 * var_chaste_interface__membrane__V - 0.0053427521319391299 * var_x69);
        const double var_x77 = exp(-1.0 + 0.10000000000000001 * var_chaste_interface__membrane__V - 2.6713760659695653 * var_x69);
        const double var_x78 = var_x77 + 3.0 * var_x76;
        const double var_x79 = 0.10000000000000001 * var_x72 + var_x75 * var_x78;
        const double var_x80 = 1 / var_x79;
        const double var_x81 = sqrt(mParameters[5]);
        const double var_x82 = var_x66 * var_x81;
        const double var_x83 = var_x72 * mParameters[15];
        const double var_x84 = var_x66 * var_x70 / pow(var_x71, 2);
        const double var_x85 = var_x81 * (-26.713760659695652 * var_x69 + var_chaste_interface__membrane__V);
        const double var_x86 = mParameters[11] * mParameters[3];
        const double var_x87 = mParameters[5] * mParameters[20] / ((1.0 + mParameters[5]) * (1.0 + 0.035299999999999998 * exp(-0.037433890822745473 * var_chaste_interface__membrane__V) + 0.1245 * exp(-0.0037433890822745472 * var_chaste_interface__membrane__V)));
        const double var_x88 = var_x87 / (40.0 + var_chaste_interface__sodium_dynamics__Na_i);
        const double var_x89 = var_x87 * var_chaste_interface__sodium_dynamics__Na_i / pow((1 + 0.025000000000000001 * var_chaste_interface__sodium_dynamics__Na_i), 2);
        const double var_x90 = 0.016878114612481093 / var_chaste_interface__sodium_dynamics__Na_i;
        ...
        rJacobian[2][2] = 1.0 - (mDt * (var_x33 * (-0.0038 - var_x34 - 8.3363813703974642e-9 * var_x26 + (-0.0015795337492435057 * var_x42 - 0.00031590674984870116 * var_x37 - 0.0042195286531202733 * var_x35 * mParameters[8] + 0.00031590674984870116 * var_chaste_interface__calcium_dynamics__Ca_i * mParameters[10] / pow(var_x36, 2)) * mParameters[11]) + 0.00040000000000000002 * (0.0038 * var_chaste_interface__calcium_dynamics__Ca_ss + 0.066691050963179718 * var_x25 - 0.0038 * var_chaste_interface__calcium_dynamics__Ca_i - 0.066691050963179718 * var_x24 - 0.00031590674984870116 * (var_x37 * var_chaste_interface__calcium_dynamics__Ca_i + (-13.356880329847826 * log(var_x35 * mParameters[4]) + var_chaste_interface__membrane__V) * mParameters[8] - 2.0 * var_x41 * (var_x43 * pow(var_chaste_interface__sodium_dynamics__Na_i, 3) - 2.5 * var_x40 * var_chaste_interface__calcium_dynamics__Ca_i)) * mParameters[11]) / (pow(var_x31, 3) * pow(var_x32, 2))));
        rJacobian[2][3] = -(mDt * (0.0038 * var_x33));
        rJacobian[2][4] = 0.0;
        rJacobian[2][5] = 0.0;
        rJacobian[2][6] = -(mDt * (0.0018954404990922068 * var_x33 * var_x44 * mParameters[11]));
        rJacobian[3][0] = -(mDt * (-var_x47 * var_x60));
        rJacobian[3][1] = -(mDt * (var_x63 * (var_x62 + 0.64023408924652514 * var_x16 - 4.2682272616435011 * var_x18)));
        rJacobian[3][2] = -(mDt * (1.1399999999999999 * var_x47));
        rJacobian[3][3] = 1.0 - (mDt * (var_x63 * (-1.1399999999999999 - var_x62 + 6.0021945866861737 * var_x30 - 6.4023408924652525 * var_x27 - var_x59 * ((var_x56) ? (-var_x64 + (-5615083.1234197523 + 374338.90822798351 * var_chaste_interface__membrane__V) * (var_x64 + 3.3392200824572393e-7 * var_x48 * var_x49)) : (0.25 * var_x57 * var_x58)) * var_chaste_interface__L_type_Ca_current_fCass_gate__fCass) + 0.00020000000000000001 * (1.1399999999999999 * var_chaste_interface__calcium_dynamics__Ca_i - 1.1399999999999999 * var_chaste_interface__calcium_dynamics__Ca_ss + var_x14 * var_x62 - var_x60 * var_chaste_interface__L_type_Ca_current_fCass_gate__fCass) / (pow(var_x45, 3) * pow(var_x46, 2))));
        rJacobian[3][4] = -(mDt * (var_x29 * var_x47 * var_x61));
        rJacobian[3][5] = 0.0;
        rJacobian[3][6] = 0.0;
        rJacobian[4][0] = 0.0;
        rJacobian[4][1] = -(mDt * (0.059999999999999998 * var_x13 * var_x28));
        rJacobian[4][2] = 0.0;
        rJacobian[4][3] = -(mDt * (var_x65 * var_chaste_interface__calcium_dynamics__R_prime));
        rJacobian[4][4] = 1.0 - (mDt * (-0.0050000000000000001 + var_x65 * var_chaste_interface__calcium_dynamics__Ca_ss));
        rJacobian[4][5] = 0.0;
        rJacobian[4][6] = 0.0;
        rJacobian[5][0] = 0.0;
        rJacobian[5][1] = 0.0;
        ...
        rJacobian[5][3] = 0.0;
        rJacobian[5][4] = 0.0;
        rJacobian[5][5] = 1.0 - (mDt * (var_x86 * (-0.016878114612481093 * var_x68 - var_x67 * mParameters[16] / (1.0 + exp(4.1806020066889626 - 0.16722408026755853 * var_chaste_interface__membrane__V)) - 0.00072631840899465921 * var_x80 * var_x82 * var_x83 - var_x67 * mParameters[21] * var_chaste_interface__transient_outward_current_r_gate__r * var_chaste_interface__transient_outward_current_s_gate__s + 4.357910453967955e-5 * var_x80 * var_x84 * var_x85 * mParameters[15] - 2.7188924024856265e-5 * var_x83 * var_x85 * (0.16028256395817389 * var_x84 - var_x75 * (2.6713760659695653 * var_x66 * var_x77 + 0.01602825639581739 * var_x66 * var_x76) - 13.356880329847826 * var_x66 * var_x73 * var_x78 / pow(var_x74, 2)) / pow(var_x79, 2) - 0.0072631840899465916 * var_x82 * mParameters[17] * var_chaste_interface__rapid_time_dependent_potassium_current_Xr1_gate__Xr1 * var_chaste_interface__rapid_time_dependent_potassium_current_Xr2_gate__Xr2)));
        rJacobian[5][6] = -(mDt * (var_x86 * (0.0012636269993948047 * var_x88 - 0.00050634343837443281 * var_x68 - 7.8976687462175284e-7 * var_x89)));
        rJacobian[6][0] = 0.0;
        rJacobian[6][1] = 0.0;
        rJacobian[6][2] = -(mDt * (0.0047386012477305169 * var_x42 * var_x86));
        rJacobian[6][3] = 0.0;
        rJacobian[6][4] = 0.0;
        rJacobian[6][5] = 0.0;
        rJacobian[6][6] = 1.0 - (mDt * (var_x86 * (1.1846503119326294e-6 * var_x89 - 0.0018954404990922068 * var_x88 - 0.0056863214972766203 * var_x44 - var_x90 * mParameters[9] - var_x90 * pow(var_chaste_interface__fast_sodium_current_m_gate__m, 3) * mParameters[12] * var_chaste_interface__fast_sodium_current_h_gate__h * var_chaste_interface__fast_sodium_current_j_gate__j)));
    }

    void Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut::UpdateTransmembranePotential(double var_chaste_interface__environment__time)
    {
        // Time units: millisecond
        std::vector<double>& rY = rGetStateVariables();
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
    ...
    void Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut::ComputeOneStepExceptVoltage(double var_chaste_interface__environment__time)
    {
        // Time units: millisecond
        std::vector<double>& rY = rGetStateVariables();
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
        ...
        const double var_fast_sodium_current_h_gate__alpha_h = ((var_chaste_interface__membrane__V < -40.0 + mParameters[14]) ? (0.057000000000000002 * exp(-11.764705882352942 + 0.14705882352941177 * mParameters[14] - 0.14705882352941177 * var_chaste_interface__membrane__V)) : (0));
        const double var_fast_sodium_current_h_gate__beta_h = ((var_chaste_interface__membrane__V < -40.0 + mParameters[14]) ? (310000.0 * exp(0.34849999999999998 * var_chaste_interface__membrane__V - 0.34849999999999998 * mParameters[14]) + 2.7000000000000002 * exp(0.079000000000000001 * var_chaste_interface__membrane__V - 0.079000000000000001 * mParameters[14])) : (5.9230769230769234 / (1.0 + exp(-0.96036036036036043 + 0.0900900900900901 * mParameters[14] - 0.0900900900900901 * var_chaste_interface__membrane__V))));
        const double var_fast_sodium_current_h_gate__h_inf = 0.01 * mParameters[13] + (1.0 - 0.01 * mParameters[13]) / pow((1.0 + exp(9.6298788694481825 + 0.13458950201884254 * var_chaste_interface__membrane__V - 0.13458950201884254 * mParameters[14])), 2);
        ...
        const double var_fast_sodium_current_j_gate__j_inf = 0.01 * mParameters[13] + (1.0 - 0.01 * mParameters[13]) / pow((1.0 + exp(9.6298788694481825 + 0.13458950201884254 * var_chaste_interface__membrane__V - 0.13458950201884254 * mParameters[14])), 2);
        ...
        CardiacNewtonSolver<7,Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut>* _p_solver = CardiacNewtonSolver<7,Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut>::Instance();
    ...
    std::vector<double> Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut::ComputeDerivedQuantities(double var_chaste_interface__environment__time, const std::vector<double> & rY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
...
template<>
void OdeSystemInformation<Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut>::Initialise(void)
{
...
CHASTE_CLASS_EXPORT(Cellten_tusscher_model_2006_MFromCellMLBackwardEulerNoLut)
