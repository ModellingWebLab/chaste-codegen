...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellten_tusscher_model_2006_epiFromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__environment__time, const double rCurrentGuess[7], double rJacobian[7][7])
    {
        ...
        const double var_x51 = log(5.4000000000000004 * var_x49);
        const double var_x52 = exp(-12.0 + 0.059999999999999998 * var_chaste_interface__membrane__V - 1.6028256395817388 * var_x51);
        const double var_x53 = 1.0 + var_x52;
        const double var_x54 = 1 / var_x53;
        const double var_x55 = exp(13.356880329847826 * var_x51 - 0.5 * var_chaste_interface__membrane__V);
        const double var_x56 = 1.0 + var_x55;
        const double var_x57 = 1 / var_x56;
        const double var_x58 = exp(-1.0 + 0.10000000000000001 * var_chaste_interface__membrane__V - 2.6713760659695653 * var_x51);
        const double var_x59 = exp(0.02 + 0.00020000000000000001 * var_chaste_interface__membrane__V - 0.0053427521319391299 * var_x51);
        const double var_x60 = var_x58 + 3.0 * var_x59;
        const double var_x61 = 0.10000000000000001 * var_x54 + var_x57 * var_x60;
        const double var_x62 = var_x49 / var_x61;
        const double var_x63 = -26.713760659695652 * var_x51 + var_chaste_interface__membrane__V;
        const double var_x64 = var_x52 / pow(var_x53, 2);
        ...
        rJacobian[5][3] = 0.0;
        rJacobian[5][4] = 0.0;
        rJacobian[5][5] = 1.0 - (mDt * (-0.011750408368292439 * var_x50 - 0.0016876848753885161 * var_x54 * var_x62 - 4.5587787568311439e-5 * var_x49 / (1.0 + exp(4.1806020066889626 - 0.16722408026755853 * var_chaste_interface__membrane__V)) + 0.00010126109252331096 * var_x62 * var_x63 * var_x64 - 0.00091800065377284671 * var_x49 * var_chaste_interface__transient_outward_current_r_gate__r * var_chaste_interface__transient_outward_current_s_gate__s - 0.00047773503410627734 * var_x49 * var_chaste_interface__rapid_time_dependent_potassium_current_Xr1_gate__Xr1 * var_chaste_interface__rapid_time_dependent_potassium_current_Xr2_gate__Xr2 - 6.3176611368492499e-5 * var_x54 * var_x63 * (-var_x57 * (2.6713760659695653 * var_x49 * var_x58 + 0.01602825639581739 * var_x49 * var_x59) + 0.16028256395817389 * var_x49 * var_x64 - 13.356880329847826 * var_x49 * var_x55 * var_x60 / pow(var_x56, 2)) / pow(var_x61, 2)));
        rJacobian[5][6] = -(mDt * (0.00053729341037579627 * var_x66 - 0.00035251225104877316 * var_x50 - 3.3580838148487267e-7 * var_x67));
        rJacobian[6][0] = 0.0;
        rJacobian[6][1] = 0.0;
