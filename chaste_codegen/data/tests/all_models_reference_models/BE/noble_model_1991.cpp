...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellnoble_model_1991FromCellMLBackwardEulerNoLut::ComputeJacobian(double var_chaste_interface__environment__time_converted, const double rCurrentGuess[9], double rJacobian[9][9])
    {
        ...
        const double var_x22 = var_x20 * var_chaste_interface__intracellular_calcium_concentration__Ca_up;
        const double var_x23 = 0.018716945411372737 * var_chaste_interface__membrane__V;
        const double var_x24 = mParameters[5] * exp(var_x23);
        const double var_x25 = pow(mParameters[7], 3.0) * exp(-var_x23);
        const double var_x26 = (var_x24 * pow(var_chaste_interface__intracellular_sodium_concentration__Na_i, 3.0) - var_x25 * var_chaste_interface__intracellular_calcium_concentration__Ca_i) * mParameters[15] / pow((0.0068999999999999999 + var_chaste_interface__intracellular_calcium_concentration__Ca_i), 2);
        const double var_x27 = mParameters[15] / (1.0 + 144.92753623188406 * var_chaste_interface__intracellular_calcium_concentration__Ca_i);
        const double var_x28 = var_x25 * var_x27;
        const double var_x29 = 0.074867781645490947 * var_chaste_interface__membrane__V;
        const double var_x30 = (-50.0 + var_chaste_interface__membrane__V) * mParameters[8] * var_chaste_interface__L_type_Ca_channel_d_gate__d * var_chaste_interface__L_type_Ca_channel_f_gate__f;
        const double var_x31 = var_x14 * var_x16 * mParameters[1];
        const double var_x32 = var_x24 * var_x27 * pow(var_chaste_interface__intracellular_sodium_concentration__Na_i, 2.0);
        const double var_x33 = 0.002 * mParameters[1];
        const double var_x34 = 1 / var_chaste_interface__intracellular_potassium_concentration__K_i;
        const double var_x35 = var_x34 * mParameters[6];
        const double var_x36 = log(var_x35);
        const double var_x37 = exp(-0.74867781645490938 + var_x29 - 2.0 * var_x36);
        const double var_x38 = 1.0 + var_x37;
        const double var_x39 = var_x35 * mParameters[14] / (10.0 + mParameters[6]);
        const double var_x40 = var_x30 * exp(1.8716945411372736) / (1.0 - exp(1.8716945411372736 - 0.037433890822745473 * var_chaste_interface__membrane__V));
        const double var_x41 = mParameters[6] / (1.0 + mParameters[6]);
        const double var_x42 = var_x41 / (40.0 + var_chaste_interface__intracellular_sodium_concentration__Na_i);
        const double var_x43 = var_x41 * var_chaste_interface__intracellular_sodium_concentration__Na_i / pow((1 + 0.025000000000000001 * var_chaste_interface__intracellular_sodium_concentration__Na_i), 2);
        const double var_x44 = pow(var_chaste_interface__fast_sodium_current_m_gate__m, 3) * mParameters[11] * var_chaste_interface__fast_sodium_current_h_gate__h / (var_chaste_interface__intracellular_sodium_concentration__Na_i + 0.12 * var_chaste_interface__intracellular_potassium_concentration__K_i);
        ...
        rJacobian[4][4] = 1.0 - (mDt * (-7.0 + var_x10 + var_x11 - 1.0549054718227806e-6 / var_chaste_interface__intracellular_calcium_concentration__Ca_i - 4.3596094751625959e-6 * var_x26 - 0.00063182746016849224 * var_x28 + (0.00040000000000000002 * var_x21 - 0.00040000000000000002 * var_x19 - 7.1999999999999991e-9 * var_x22) * mParameters[3] - 4.7303520325519803e-5 * var_x30 * exp(3.7433890822745473) / (1.0 - exp(3.7433890822745473 - var_x29))));
        rJacobian[4][5] = -(mDt * (0.00020408163265306126 * var_x31 + 0.00020408163265306126 * mParameters[0]));
        rJacobian[4][6] = -(mDt * ((9.5999999999999999e-8 * var_x21 + 7.1999999999999991e-9 * var_x19 - 1.7279999999999999e-12 * var_x22) * mParameters[3]));
        rJacobian[4][7] = 0.0;
        rJacobian[4][8] = -(mDt * (0.0018954823805054767 * var_x32));
        rJacobian[5][0] = -(mDt * ((var_x17 * var_x33 - var_x15 * var_x33) * var_chaste_interface__intracellular_calcium_concentration__Ca_rel));
        rJacobian[5][1] = 0.0;
        rJacobian[5][2] = 0.0;
        rJacobian[5][3] = 0.0;
        rJacobian[5][4] = 0.0;
        rJacobian[5][5] = 1.0 - (mDt * (-0.0050000000000000001 - 0.001 * var_x31 - 0.001 * mParameters[0]));
        ...
        rJacobian[6][2] = 0.0;
        rJacobian[6][3] = 0.0;
        rJacobian[6][4] = -(mDt * ((3.5279999999999993e-7 * var_x22 + 0.019599999999999999 * var_x19 - 0.019599999999999999 * var_x21) * mParameters[3]));
        rJacobian[6][5] = -(mDt * (0.050000000000000003));
        rJacobian[6][6] = 1.0 - (mDt * (-0.050000000000000003 + (8.4671999999999982e-11 * var_x22 - 4.7039999999999993e-6 * var_x21 - 3.5279999999999993e-7 * var_x19) * mParameters[3]));
        rJacobian[6][7] = 0.0;
        rJacobian[6][8] = 0.0;
        rJacobian[7][0] = 0.0;
        rJacobian[7][1] = 0.0;
        ...
        rJacobian[7][5] = 0.0;
        rJacobian[7][6] = 0.0;
        rJacobian[7][7] = 1.0 - (mDt * ((-1.0127092529498693e-5 * var_x34 - 4.7303520325519801e-8 * var_x40 - 4.5130532869178019e-6 * mParameters[10] * var_chaste_interface__time_dependent_potassium_current_x_gate__x - 0.016878487549164489 * var_x39 / var_x38 + 0.0012636549203369845 * var_x37 * var_x39 * (-26.713760659695652 * var_x36 + var_chaste_interface__membrane__V) / pow(var_x38, 2) - 0.016878487549164489 * var_x34 * mParameters[16] * var_chaste_interface__transient_outward_current_r_gate__r * var_chaste_interface__transient_outward_current_s_gate__s) * mParameters[4]));
        rJacobian[7][8] = -(mDt * ((0.00088455844423588909 * var_x42 - 5.5284902764743069e-7 * var_x43) * mParameters[4]));
        rJacobian[8][0] = 0.0;
        rJacobian[8][1] = 0.0;
        rJacobian[8][2] = 0.0;
        rJacobian[8][3] = 0.0;
        rJacobian[8][4] = -(mDt * ((1.3078828425487789e-5 * var_x26 + 0.0018954823805054767 * var_x28) * mParameters[4]));
        rJacobian[8][5] = 0.0;
        rJacobian[8][6] = 0.0;
        rJacobian[8][7] = -(mDt * (-0.0020254185058997385 * var_x44 * mParameters[4]));
        rJacobian[8][8] = 1.0 - (mDt * ((8.2927354147114598e-7 * var_x43 - 1.0127092529498693e-5 / var_chaste_interface__intracellular_sodium_concentration__Na_i - 0.0013268376663538336 * var_x42 - 0.016878487549164489 * var_x44 - 0.0056864471415164304 * var_x32 - 2.3651760162759901e-7 * var_x40) * mParameters[4]));
    }
