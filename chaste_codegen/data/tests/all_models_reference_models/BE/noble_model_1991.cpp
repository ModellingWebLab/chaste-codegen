...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellnoble_model_1991FromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__environment__time_converted, const double rCurrentGuess[9], double rJacobian[9][9])
    {
        ...
        const double var_x5 = 1.0 - var_chaste_interface__calcium_release__ActFrac - var_chaste_interface__calcium_release__ProdFrac;
        const double var_x6 = 1.0 * var_x1 * var_chaste_interface__intracellular_calcium_concentration__Ca_i;
        const double var_x7 = 1.0 * var_x2 / pow(var_x0, 3);
        ...
        const double var_x23 = 1 / (1.0 + 144.92753623188406 * var_chaste_interface__intracellular_calcium_concentration__Ca_i);
        const double var_x24 = 0.018716945411372737 * var_chaste_interface__membrane__V;
        ...
        const double var_x28 = (-50.0 + var_chaste_interface__membrane__V) * var_chaste_interface__L_type_Ca_channel_d_gate__d * var_chaste_interface__L_type_Ca_channel_f_gate__f;
        const double var_x29 = exp(var_x24);
        const double var_x30 = (2.0 * var_x29 * pow(var_chaste_interface__intracellular_sodium_concentration__Na_i, 3.0) - 2744000.0 * var_x25 * var_chaste_interface__intracellular_calcium_concentration__Ca_i) / pow((0.0068999999999999999 + var_chaste_interface__intracellular_calcium_concentration__Ca_i), 2);
        const double var_x31 = var_x14 * var_x16;
        const double var_x32 = var_x23 * var_x29 * pow(var_chaste_interface__intracellular_sodium_concentration__Na_i, 2.0);
        ...
        const double var_x35 = var_x28 * exp(1.8716945411372736) / (1.0 - exp(1.8716945411372736 - 0.037433890822745473 * var_chaste_interface__membrane__V));
        const double var_x36 = log(4.0 * var_x34);
        const double var_x37 = exp(-0.74867781645490938 + var_x27 - 2.0 * var_x36);
        ...
        rJacobian[0][2] = 0.0;
        rJacobian[0][3] = 0.0;
        rJacobian[0][4] = -(mDt * (var_x5 * var_x6 + (var_x7 - var_x6) * var_chaste_interface__calcium_release__ActFrac - var_x5 * var_x7));
        rJacobian[0][5] = 0.0;
        rJacobian[0][6] = 0.0;
        rJacobian[0][7] = 0.0;
        rJacobian[0][8] = 0.0;
        ...
        rJacobian[1][2] = 0.0;
        rJacobian[1][3] = 0.0;
        rJacobian[1][4] = -(mDt * ((var_x6 - var_x7) * var_chaste_interface__calcium_release__ActFrac));
        rJacobian[1][5] = 0.0;
        rJacobian[1][6] = 0.0;
        rJacobian[1][7] = 0.0;
        rJacobian[1][8] = 0.0;
        ...
        rJacobian[4][4] = 1.0 - (mDt * (-7.0000000000000009 + var_x10 + var_x11 + 0.00040000000000000002 * var_x21 - 0.86686727535117136 * var_x26 - 7.2e-9 * var_x22 - 1.0549054718227806e-6 / var_chaste_interface__intracellular_calcium_concentration__Ca_i - 2.1798047375812981e-9 * var_x30 - 0.00040000000000000002 * var_x19 - 1.1825880081379951e-5 * var_x28 * exp(3.7433890822745473) / (1.0 - exp(3.7433890822745473 - var_x27))));
        rJacobian[4][5] = -(mDt * (0.051020408163265314 * var_x31));
        rJacobian[4][6] = -(mDt * (7.2e-9 * var_x19 + 9.5999999999999999e-8 * var_x21 - 1.7279999999999999e-12 * var_x22));
        rJacobian[4][7] = 0.0;
        ...
        rJacobian[6][2] = 0.0;
        rJacobian[6][3] = 0.0;
        rJacobian[6][4] = -(mDt * (0.019599999999999999 * var_x19 + 3.5279999999999993e-7 * var_x22 - 0.019599999999999999 * var_x21));
        rJacobian[6][5] = -(mDt * (0.050000000000000003));
        rJacobian[6][6] = 1.0 - (mDt * (-0.050000000000000003 + 8.4671999999999982e-11 * var_x22 - 3.5279999999999993e-7 * var_x19 - 4.7040000000000002e-6 * var_x21));
        rJacobian[6][7] = 0.0;
        rJacobian[6][8] = 0.0;
        rJacobian[7][0] = 0.0;
        rJacobian[7][1] = 0.0;
        ...
        rJacobian[7][5] = 0.0;
        rJacobian[7][6] = 0.0;
        rJacobian[7][7] = 1.0 - (mDt * (-1.0127092529498693e-5 * var_x34 - 4.5130532869178019e-6 * var_chaste_interface__time_dependent_potassium_current_x_gate__x - 1.1825880081379952e-8 * var_x35 - 0.004822425014046997 * var_x34 / var_x38 - 8.4392437745822447e-5 * var_x34 * var_chaste_interface__transient_outward_current_r_gate__r * var_chaste_interface__transient_outward_current_s_gate__s + 0.00036104426295342415 * var_x34 * var_x37 * (-26.713760659695652 * var_x36 + var_chaste_interface__membrane__V) / pow(var_x38, 2)));
        rJacobian[7][8] = -(mDt * (0.00070764675538871125 * var_x39 - 4.4227922211794452e-7 * var_x40));
        rJacobian[8][0] = 0.0;
        rJacobian[8][1] = 0.0;
        rJacobian[8][2] = 0.0;
        rJacobian[8][3] = 0.0;
        rJacobian[8][4] = -(mDt * (6.5394142127438948e-9 * var_x30 + 2.6006018260535142 * var_x26));
        rJacobian[8][5] = 0.0;
        rJacobian[8][6] = 0.0;
        rJacobian[8][7] = -(mDt * (-0.71132697927198818 * var_x41));
        rJacobian[8][8] = 1.0 - (mDt * (6.634188331769168e-7 * var_x40 - 1.0127092529498693e-5 / var_chaste_interface__intracellular_sodium_concentration__Na_i - 5.9129400406899753e-8 * var_x35 - 0.0010614701330830668 * var_x39 - 5.9277248272665686 * var_x41 - 5.6864471415164302e-6 * var_x32));
    }
