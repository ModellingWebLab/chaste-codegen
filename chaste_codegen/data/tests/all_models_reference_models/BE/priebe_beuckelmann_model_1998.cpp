...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellpriebe_beuckelmann_model_1998FromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__environment__time, const double rCurrentGuess[12], double rJacobian[12][12])
    {
        ...
        const double var_x23 = 0.00050000000000000001 + var_chaste_interface__Ionic_concentrations__Cai;
        const double var_x24 = 0.0023800000000000002 + var_chaste_interface__Ionic_concentrations__Cai;
        const double var_x25 = 1.0 + 0.00011900000000000002 / pow(var_x24, 2) + 3.5000000000000004e-5 / pow(var_x23, 2);
        ...
        const double var_x48 = 1.0 * (0.00023800000000000004 / pow(var_x24, 3) + 7.0000000000000007e-5 / pow(var_x23, 3)) / pow(var_x25, 2);
        ...
        const double var_x53 = var_chaste_interface__IKr_Xr_gate__Xr / (1.0 + exp(1.1304347826086956 + 0.043478260869565216 * var_chaste_interface__cell__V));
        const double var_x54 = 1.0 * var_chaste_interface__Ionic_concentrations__Ki;
        const double var_x55 = var_x54 + 0.042999999999999997 * var_chaste_interface__Ionic_concentrations__Nai;
        const double var_x56 = var_chaste_interface__Ito_r_gate__r * var_chaste_interface__Ito_t_gate__t;
        const double var_x57 = var_x56 * (0.10066438494060802 * var_chaste_interface__Ionic_concentrations__Ki + 0.004328568552446144 * var_chaste_interface__Ionic_concentrations__Nai) / pow(var_x55, 2);
        const double var_x58 = pow(var_chaste_interface__IKs_Xs_gate__Xs, 2);
        const double var_x59 = var_x54 + 0.018329999999999999 * var_chaste_interface__Ionic_concentrations__Nai;
        const double var_x60 = var_x58 * (0.15315014533948793 * var_chaste_interface__Ionic_concentrations__Ki + 0.0028072421640728137 * var_chaste_interface__Ionic_concentrations__Nai) / pow(var_x59, 2);
        const double var_x61 = log(4.0 * var_x52);
        const double var_x62 = exp(-12.0 + 0.059999999999999998 * var_chaste_interface__cell__V - 1.6035454492691736 * var_x61);
        const double var_x63 = 1.0 + var_x62;
        const double var_x64 = 1 / var_x63;
        const double var_x65 = exp(13.36287874390978 * var_x61 - 0.5 * var_chaste_interface__cell__V);
        const double var_x66 = 1.0 + var_x65;
        const double var_x67 = 1 / var_x66;
        const double var_x68 = exp(0.02 + 0.00020000000000000001 * var_chaste_interface__cell__V - 0.0053451514975639122 * var_x61);
        const double var_x69 = exp(-1.0 + 0.10000000000000001 * var_chaste_interface__cell__V - 2.6725757487819561 * var_x61);
        const double var_x70 = 1.0 * var_x69 + 3.0 * var_x68;
        const double var_x71 = 0.10000000000000001 * var_x64 + var_x67 * var_x70;
        const double var_x72 = 1 / var_x71;
        const double var_x73 = var_x64 * var_x72;
        const double var_x74 = -26.725757487819561 * var_x61 + var_chaste_interface__cell__V;
        const double var_x75 = var_x52 * var_x62 / pow(var_x63, 2);
        const double var_x76 = 0.25 * var_x74;
        ...
        rJacobian[3][1] = 0.0;
        rJacobian[3][2] = 0.0;
        rJacobian[3][3] = 1.0 - (mDt * (-var_x77 * (3.4901380509403466 * var_x60 + 79.648102465199855 * var_x57 + 6.681439371954891 * var_x52 * var_x73 + 0.40088636231729341 * var_x52 * var_x53 - 0.40088636231729341 * var_x72 * var_x74 * var_x75 + var_x64 * var_x76 * (0.16035454492691736 * var_x75 - var_x67 * (2.6725757487819561 * var_x52 * var_x69 + 0.016035454492691736 * var_x52 * var_x68) - 13.36287874390978 * var_x52 * var_x65 * var_x70 / pow(var_x66, 2)) / pow(var_x71, 2))));
        rJacobian[3][4] = -(mDt * (-var_x77 * (3.4248684060035934 * var_x57 + 0.063974230473736557 * var_x60 - 0.089693693633866756 * var_x81)));
        rJacobian[3][5] = 0.0;
        rJacobian[3][6] = 0.0;
        rJacobian[3][7] = 0.0;
        rJacobian[3][8] = 0.0;
        ...
        rJacobian[5][3] = 0.0;
        rJacobian[5][4] = 0.0;
        rJacobian[5][5] = 1.0 - (mDt * (((var_x46 + 0.0018500000000000001 * var_chaste_interface__cell__V + 8.9708471082494196e-5 * var_x45 - 0.026725757487819562 * var_x83 + var_x73 * var_x76 + 1.0 * GetIntracellularAreaStimulus(var_chaste_interface__environment__time) / HeartConfig::Instance()->GetCapacitance() + 16.0 * var_x82 * (-26.725757487819561 * var_x83 + var_chaste_interface__cell__V) + 0.014999999999999999 * var_x53 * var_x74 + 0.29999999999999999 * var_x56 * (-26.725757487819561 * log(9.9339999999999993 / var_x55) + var_chaste_interface__cell__V) + 0.02 * var_x58 * (-26.725757487819561 * log(6.5295399999999999 / var_x59) + var_chaste_interface__cell__V) + 0.94545454545454544 * var_x80 / (1.0 + 31.622776601683793 * var_x79) < -150.0) ? (-100.5) : (-0.5))));
        rJacobian[5][6] = 0.0;
        rJacobian[5][7] = 0.0;
        rJacobian[5][8] = 0.0;
        rJacobian[5][9] = 0.0;
