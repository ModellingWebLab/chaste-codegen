...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellluo_rudy_1994FromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__environment__time, const double rCurrentGuess[5], double rJacobian[5][5])
    {
        ...
        const double var_x27 = 1.0 + 1.7279999999999996e-9 / pow(var_chaste_interface__ionic_concentrations__Cai, 3);
        const double var_x28 = 1.0025441254350579e-16 * var_x25 * var_x26 / (pow(var_x27, 2) * pow(var_chaste_interface__ionic_concentrations__Cai, 4));
        const double var_x29 = exp(var_x16);
        ...
        const double var_x40 = var_x17 * log10(var_x33 * mParameters[1]);
        ...
        const double var_x55 = var_x12 * var_x29 * var_x31;
        ...
        const double var_x62 = var_x60 * var_x61 * mParameters[1] / ((1.5 + mParameters[1]) * pow((0.031622776601683791 + var_x61), 2) * (1.0 + 0.1245 * exp(-0.0037573302761014088 * var_chaste_interface__membrane__V) + 0.036499999999999998 * (-0.14285714285714285 + 0.14285714285714285 * exp(0.01485884101040119 * mParameters[2])) * exp(-var_x16)));
        ...
        rJacobian[3][0] = 0.0;
        rJacobian[3][1] = 0.0;
        rJacobian[3][2] = -(mDt * (-var_x23 * var_x28 + 1.5356429857854459e-14 * var_x32 * (var_x15 + var_x30 * var_chaste_interface__ionic_concentrations__Ki)));
        ...
        rJacobian[4][0] = 0.0;
        rJacobian[4][1] = 0.0;
        rJacobian[4][2] = -(mDt * (0.18221961715658433 * var_x7 - var_x28 * var_x66 + 5.3707721005449532e-14 * var_x32 * (var_x64 + var_x30 * var_chaste_interface__ionic_concentrations__Nai)));
        rJacobian[4][3] = -(mDt * (var_x68 - var_x63 * var_chaste_interface__ionic_concentrations__Nai));
        rJacobian[4][4] = 1.0 - (mDt * (var_x68 - 5.594554271400994e-8 * var_x55 - 0.54665885146975302 * var_x14 - 1.139681818874567e-6 * var_x60 - 6.4825765265103909e-6 * var_x62 - var_x37 * (var_x21 + var_x18 * var_x65) - 0.080828497792522477 * var_x60 * pow(var_chaste_interface__fast_sodium_current_m_gate__m, 3) * mParameters[4] * var_chaste_interface__fast_sodium_current_h_gate__h * var_chaste_interface__fast_sodium_current_j_gate__j));
    }
