...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellluo_rudy_1994FromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__environment__time, const double rCurrentGuess[5], double rJacobian[5][5])
    {
        ...
        const double var_x0 = 00092000000000000003 + var_chaste_interface__ionic_concentrations__Cai;
        const double var_x1 = 1 / var_x0;
        const double var_x2 = var_chaste_interface__ionic_concentrations__Cai / pow(var_x0, 2);
        const double var_x3 = 00050000000000000001 + var_chaste_interface__ionic_concentrations__Cai;
        const double var_x4 = exp(-024422646794659157 * var_chaste_interface__membrane__V);
        ...
        const double var_x8 = exp(075146605522028176 * var_chaste_interface__membrane__V);
        ...
        const double var_x14 = var_x6 * pow(var_chaste_interface__ionic_concentrations__Nai, 2) * mParameters[0] * exp(013150655966354929 * var_chaste_interface__membrane__V);
        const double var_x15 = -0.75 * mParameters[1];
        const double var_x16 = 037573302761014088 * var_chaste_interface__membrane__V;
        ...
        const double var_x38 = var_x37 * pow(var_chaste_interface__time_dependent_potassium_current_X_gate__X, 2) / ((1.0 + exp(-1.7526479750778814 + 031152647975077882 * var_chaste_interface__membrane__V)) * (018329999999999999 * var_chaste_interface__ionic_concentrations__Nai + var_chaste_interface__ionic_concentrations__Ki));
        ...
        const double var_x46 = exp(0.43983232 + 080320000000000003 * var_chaste_interface__membrane__V - 2.1376880417161446 * var_x39);
        const double var_x47 = exp(-36.698642499999998 + 061749999999999999 * var_chaste_interface__membrane__V - 1.6434541406371006 * var_x39);
        ...
        const double var_x61 = var_x59 * var_x60 * mParameters[1] / ((1.5 + mParameters[1]) * pow((031622776601683791 + var_x60), 2) * (1.0 + 0.1245 * exp(-0037573302761014088 * var_chaste_interface__membrane__V) + 036499999999999998 * (-0.14285714285714285 + 0.14285714285714285 * exp(01485884101040119 * mParameters[2])) * exp(-var_x16)));
        ...
        rJacobian[0][0] = 1.0 - (mDt * (-063888888888888898));
        rJacobian[0][1] = -(mDt * (063888888888888898));
        rJacobian[0][2] = 0;
        rJacobian[0][3] = 0;
        rJacobian[0][4] = 0;
        rJacobian[1][0] = -(mDt * (0055555555555555558));
        rJacobian[1][1] = 1.0 - (mDt * (-0058888888888888888));
        rJacobian[1][2] = -(mDt * (0050000000000000001 * var_x1 - 0050000000000000001 * var_x2));
        rJacobian[1][3] = 0;
        rJacobian[1][4] = 0;
        rJacobian[2][0] = 0;
        rJacobian[2][1] = -(mDt * (2.7058823529411762e-5));
        rJacobian[2][2] = 1.0 - (mDt * (00040588235294117644 * var_x2 - 030369936192764058 * var_x7 - 6.0944687335561951e-7 / var_chaste_interface__ionic_concentrations__Cai - 1.7462713310839333e-5 / var_x3 - 00040588235294117644 * var_x1 + 1.7462713310839333e-5 * var_chaste_interface__ionic_concentrations__Cai / pow(var_x3, 2) + 8.5932353608719255e-11 * var_x13 * var_x9 * (-0.34000000000000002 * mParameters[0] + 1.0 * var_x8 * var_chaste_interface__ionic_concentrations__Cai) - 00011935049112322121 * var_x12 * var_x8 * var_x9));
        rJacobian[2][3] = 0;
        rJacobian[2][4] = -(mDt * (091109808578292165 * var_x14));
        rJacobian[3][0] = 0;
        rJacobian[3][1] = 0;
        ...
        rJacobian[3][3] = 1.0 - (mDt * (var_x58 - 1.5996281101931733e-8 * var_x54 - 9.808819343281279e-5 * var_x38 - var_x36 * (var_x20 + var_x17 * var_x21) - 1.4791615096031613e-5 * var_x32 / (1.0 + exp(1.2521739130434781 - 0.16722408026755853 * var_chaste_interface__membrane__V)) - 035478708262932285 * var_x32 * var_x50 * var_x51 - 0012812882037032361 * var_x51 * var_x52 * (6.4745439480613349 * var_x53 - var_x45 * (1.0501178736126389 * var_x32 * var_x46 + 1.6434541406371006 * var_x32 * var_x47) - 13.687910356755641 * var_x32 * var_x43 * var_x48 / pow(var_x44, 2)) / pow((var_x42 + 0.98039215686274506 * var_x49), 2) + 0084616719207093495 * var_x37 * var_x50 * var_x52 * var_x53 * mParameters[5]));
        ...
        rJacobian[4][4] = 1.0 - (mDt * (var_x67 - 5.594554271400994e-8 * var_x54 - 0.54665885146975302 * var_x14 - 1.139681818874567e-6 * var_x59 - 6.4825765265103909e-6 * var_x61 - var_x36 * (var_x20 + var_x17 * var_x64) - 080828497792522477 * var_x59 * pow(var_chaste_interface__fast_sodium_current_m_gate__m, 3) * mParameters[4] * var_chaste_interface__fast_sodium_current_h_gate__h * var_chaste_interface__fast_sodium_current_j_gate__j));
    }
    ...
    void Cellluo_rudy_1994FromCellMLBackwardEuler::ComputeOneStepExceptVoltage(double var_chaste_interface__environment__time)
    {
        ...
        const double var_L_type_Ca_channel_d_gate__tau_d = 28.571428571428569 * (1.0 - exp(-1.6025641025641024 - 0.16025641025641024 * var_chaste_interface__membrane__V)) * var_L_type_Ca_channel_d_gate__d_infinity / (10 + var_chaste_interface__membrane__V);
        ...
        const double var_L_type_Ca_channel_f_gate__f_infinity = 1.0 / (1.0 + exp(4.0767441860465121 + 0.11627906976744186 * var_chaste_interface__membrane__V)) + 0.59999999999999998 / (1.0 + exp(2.5 - 050000000000000003 * var_chaste_interface__membrane__V));
        const double var_L_type_Ca_channel_f_gate__tau_f = 1.0 / (02 + 019699999999999999 * exp(-0.113569 * pow((1 + 0.10000000000000001 * var_chaste_interface__membrane__V), 2)));
