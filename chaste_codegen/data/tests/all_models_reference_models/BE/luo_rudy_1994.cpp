...
#include "CardiacNewtonSolver.hpp"
    ...
    boost::shared_ptr<RegularStimulus> Cellluo_rudy_1994FromCellMLBackwardEulerNoLut::UseCellMLDefaultStimulus()
    {
    ...
    double Cellluo_rudy_1994FromCellMLBackwardEulerNoLut::GetIntracellularCalciumConcentration()
    {
        return mStateVariables[1];
    }
    Cellluo_rudy_1994FromCellMLBackwardEulerNoLut::Cellluo_rudy_1994FromCellMLBackwardEulerNoLut(boost::shared_ptr<AbstractIvpOdeSolver> /* unused; should be empty */, boost::shared_ptr<AbstractStimulusFunction> pIntracellularStimulus)
                ...
                pIntracellularStimulus)
    {
        ...
        //
        this->mpSystemInfo = OdeSystemInformation<Cellluo_rudy_1994FromCellMLBackwardEulerNoLut>::Instance();
        Init();
    ...
    Cellluo_rudy_1994FromCellMLBackwardEulerNoLut::~Cellluo_rudy_1994FromCellMLBackwardEulerNoLut()
    {
    }

    
    double Cellluo_rudy_1994FromCellMLBackwardEulerNoLut::GetIIonic(const std::vector<double>* pStateVariables)
    {
    ...
    void Cellluo_rudy_1994FromCellMLBackwardEulerNoLut::ComputeResidual(double var_chaste_interface__environment__time, const double rCurrentGuess[5], double rResidual[5])
    {
    ...
    void Cellluo_rudy_1994FromCellMLBackwardEulerNoLut::ComputeJacobian(double var_chaste_interface__environment__time, const double rCurrentGuess[5], double rJacobian[5][5])
    {
        std::vector<double>& rY = rGetStateVariables();
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
        // Units: mV; Initial value: -84.624
        ...
        const double var_x12 = var_x11 < 1.3307320976818627e-6;
        const double var_x13 = exp(0.075146605522028176 * var_chaste_interface__membrane__V);
        const double var_x14 = 0.078597788527232842 * var_chaste_interface__membrane__V / (-1.0 + var_x13);
        const double var_x15 = pow(var_chaste_interface__ionic_concentrations__Cai, 2);
        const double var_x16 = var_chaste_interface__L_type_Ca_channel_d_gate__d * var_chaste_interface__L_type_Ca_channel_f_gate__f;
        const double var_x17 = var_x16 / (1.0 + 2777777.7777777785 * var_x15);
        const double var_x18 = -0.34000000000000002 * mParameters[0];
        const double var_x19 = var_chaste_interface__ionic_concentrations__Cai;
        const double var_x20 = var_x9 * (var_x18 + var_x19 * var_x7);
        const double var_x21 = var_x16 * var_chaste_interface__ionic_concentrations__Cai / pow((3.5999999999999994e-7 + var_x15), 2);
        const double var_x22 = exp(-0.024422646794659157 * var_chaste_interface__membrane__V);
        const double var_x23 = pow(mParameters[2], 3);
        const double var_x24 = 1 / ((1.0 + 0.10000000000000001 * var_x22) * (669921.875 + var_x23) * (1.3799999999999999 + mParameters[0]));
        const double var_x25 = var_x22 * var_x23 * var_x24;
        const double var_x26 = var_x24 * pow(var_chaste_interface__ionic_concentrations__Nai, 2) * mParameters[0] * exp(0.013150655966354929 * var_chaste_interface__membrane__V);
        const double var_x27 = exp(9.9999999999999995e-8);
        const double var_x28 = 1 / (-1.0 + var_x27);
        const double var_x29 = -0.75 * mParameters[1];
        const double var_x30 = 0.75 * var_chaste_interface__ionic_concentrations__Ki;
        const double var_x31 = var_x28 * (var_x29 + var_x27 * var_x30);
        const double var_x32 = exp(-9.9999999999999995e-8);
        const double var_x33 = 1 / (-1.0 + var_x32);
        const double var_x34 = var_x33 * (var_x29 + var_x30 * var_x32);
        const double var_x35 = 1.6947874999999998e-11 * var_x34;
        const double var_x36 = 1 / (var_chaste_interface__ionic_concentrations__Ki + var_chaste_interface__ionic_concentrations__Nai);
        const double var_x37 = log(var_x36 * (mParameters[1] + mParameters[2]));
        const double var_x38 = -26.614641953637257 * var_x37 + var_chaste_interface__membrane__V;
        const double var_x39 = 2.6614641953637254e-6 + var_x38;
        const double var_x40 = 187866.51380507043 * var_x39;
        const double var_x41 = fabs(var_x38) < 2.6614641953637254e-6;
        const double var_x42 = 0.037573302761014088 * var_chaste_interface__membrane__V;
        const double var_x43 = exp(var_x42 - var_x37);
        const double var_x44 = -1.0 + var_x43;
        const double var_x45 = 1 / var_x44;
        const double var_x46 = 0.75 * var_x43;
        const double var_x47 = var_x46 * var_chaste_interface__ionic_concentrations__Ki;
        const double var_x48 = var_x29 + var_x47;
        const double var_x49 = var_x45 * var_x48;
        const double var_x50 = 6.3678763853082156e-6 * var_x38;
        const double var_x51 = 1.0 + 1.7279999999999996e-9 / pow(var_chaste_interface__ionic_concentrations__Cai, 3);
        const double var_x52 = 1.5743774922328884e-11 / (pow(var_x51, 2) * pow(var_chaste_interface__ionic_concentrations__Cai, 4));
        const double var_x53 = 1.8691085000000002e-11 * var_x8 * (var_x29 + var_x30 * var_x7);
        const double var_x54 = 0.5 + 187866.51380507043 * var_chaste_interface__membrane__V;
        const double var_x55 = var_x11 < 2.6614641953637254e-6;
        const double var_x56 = exp(var_x42);
        const double var_x57 = var_chaste_interface__membrane__V / (-1.0 + var_x56);
        const double var_x58 = 2.1866354058790117e-9 * var_x21;
        const double var_x59 = 26.614641953637257 * var_x36;
        const double var_x60 = var_x59 * (3.1839381926541078e-6 * var_x31 + 3.1839381926541078e-6 * var_x34);
        const double var_x61 = var_x32 * var_x33;
        const double var_x62 = -1.2710906249999999e-11 * var_x61 + var_x39 * (2.387953644490581e-6 * var_x61 + 2.387953644490581e-6 * var_x27 * var_x28);
        const double var_x63 = var_x45 * var_x50;
        const double var_x64 = 0.00016947874999999999 * var_x36;
        const double var_x65 = var_x36 * var_x43;
        const double var_x66 = var_x50 * var_x65 / pow(var_x44, 2);
        const double var_x67 = var_x49 * var_x64 - var_x48 * var_x66;
        const double var_x68 = 0.0030369936192764057 / var_x51;
        const double var_x69 = 1 / var_chaste_interface__ionic_concentrations__Ki;
        const double var_x70 = 0.5 + 187866.51380507043 * var_chaste_interface__membrane__V;
        const double var_x71 = var_x4 * var_x5;
        const double var_x72 = var_x7 * var_x8;
        const double var_x73 = 1.4018313750000001e-11 * var_x72;
        const double var_x74 = var_x56 * var_x57;
        const double var_x75 = 0.0030369936192764057 * var_x17;
        const double var_x76 = sqrt(mParameters[1]);
        const double var_x77 = var_x76 * pow(var_chaste_interface__time_dependent_potassium_current_X_gate__X, 2) / ((1.0 + exp(-1.7526479750778814 + 0.031152647975077882 * var_chaste_interface__membrane__V)) * (0.018329999999999999 * var_chaste_interface__ionic_concentrations__Nai + var_chaste_interface__ionic_concentrations__Ki));
        const double var_x78 = log(var_x69 * mParameters[1]);
        const double var_x79 = exp(-14.1227775 + 0.23849999999999999 * var_chaste_interface__membrane__V - 6.3475921059424856 * var_x78);
        const double var_x80 = 1.0 + var_x79;
        const double var_x81 = 1 / var_x80;
        const double var_x82 = exp(-2.4444678999999998 + 13.687910356755641 * var_x78 - 0.51429999999999998 * var_chaste_interface__membrane__V);
        const double var_x83 = 1.0 + var_x82;
        const double var_x84 = 1 / var_x83;
        const double var_x85 = exp(0.43983232 + 0.080320000000000003 * var_chaste_interface__membrane__V - 2.1376880417161446 * var_x78);
        const double var_x86 = exp(-36.698642499999998 + 0.061749999999999999 * var_chaste_interface__membrane__V - 1.6434541406371006 * var_x78);
        const double var_x87 = var_x86 + 0.49124000000000001 * var_x85;
        const double var_x88 = var_x84 * var_x87;
        const double var_x89 = 1 / (var_x88 + 1.02 * var_x81);
        const double var_x90 = var_x76 * var_x81 * mParameters[5];
        const double var_x91 = -26.614641953637257 * var_x78 + var_chaste_interface__membrane__V;
        const double var_x92 = var_x69 * var_x79 / pow(var_x80, 2);
        const double var_x93 = 4.7759072889811619e-6 * var_x38 * var_x45 * var_x65;
        const double var_x94 = 1 / var_chaste_interface__ionic_concentrations__Nai;
        const double var_x95 = pow(var_x94, 1.5);
        const double var_x96 = var_x94 * var_x95 * mParameters[1] / ((1.5 + mParameters[1]) * pow((0.031622776601683791 + var_x95), 2) * (1.0 + 0.1245 * exp(-0.0037573302761014088 * var_chaste_interface__membrane__V) + 0.036499999999999998 * (-0.14285714285714285 + 0.14285714285714285 * exp(0.01485884101040119 * mParameters[2])) * exp(-var_x42)));
        const double var_x97 = -0.75 * mParameters[2];
        const double var_x98 = 0.75 * var_chaste_interface__ionic_concentrations__Nai;
        const double var_x99 = var_x28 * (var_x97 + var_x27 * var_x98);
        const double var_x100 = var_x33 * (var_x97 + var_x32 * var_x98);
        const double var_x101 = 1.6947874999999998e-11 * var_x100;
        const double var_x102 = var_x46 * var_chaste_interface__ionic_concentrations__Nai;
        const double var_x103 = var_x102 + var_x97;
        const double var_x104 = 6.5370374999999993e-11 * var_x8 * (var_x97 + var_x7 * var_x98);
        const double var_x105 = var_x59 * (3.1839381926541078e-6 * var_x100 + 3.1839381926541078e-6 * var_x99);
        const double var_x106 = -var_x103 * var_x66 + var_x103 * var_x45 * var_x64;
        const double var_x107 = 4.9027781250000001e-11 * var_x72;
        ...
        rJacobian[2][2] = 1.0 - (mDt * (0.00040588235294117644 * var_x2 - 0.030369936192764058 * var_x25 - 6.0944687335561951e-7 / var_chaste_interface__ionic_concentrations__Cai - 1.7462713310839333e-5 / var_x3 - 0.00040588235294117644 * var_x1 + 1.7462713310839333e-5 * var_chaste_interface__ionic_concentrations__Cai / pow(var_x3, 2) + 1.0933177029395058e-9 * var_x21 * ((var_x12) ? (-var_x20 + 375733.02761014085 * (1.3307320976818627e-6 + var_chaste_interface__membrane__V) * (var_x20 + var_x6 * (var_x18 + var_x19 * var_x4))) : (var_x14 * (var_x18 + var_x13 * var_x19))) - 0.0015184968096382029 * var_x17 * ((var_x12) ? (-var_x10 + (0.5 + 375733.02761014085 * var_chaste_interface__membrane__V) * (var_x10 + var_x4 * var_x6)) : (var_x13 * var_x14))));
        rJacobian[2][3] = 0.0;
        rJacobian[2][4] = -(mDt * (0.091109808578292165 * var_x26));
        rJacobian[3][0] = 0.0;
        rJacobian[3][1] = 0.0;
        rJacobian[3][2] = -(mDt * (var_x58 * ((var_x55) ? (-var_x53 + var_x54 * (var_x53 + 1.8691085000000002e-11 * var_x5 * (var_x29 + var_x30 * var_x4))) : (7.0228579563684903e-6 * var_x57 * (var_x29 + var_x30 * var_x56))) - var_x52 * ((var_x41) ? (-var_x35 + var_x40 * (var_x35 + 1.6947874999999998e-11 * var_x31)) : (var_x49 * var_x50))));
        rJacobian[3][3] = 1.0 - (mDt * (-9.808819343281279e-5 * var_x77 - var_x68 * ((var_x41) ? (var_x60 + var_x62) : (var_x67 + var_x63 * (var_x46 + var_x36 * var_x47))) - var_x75 * ((var_x55) ? (-var_x73 + var_x70 * (var_x73 + 1.4018313750000001e-11 * var_x71)) : (5.2671434672763675e-6 * var_x74)) - 1.4791615096031613e-5 * var_x69 / (1.0 + exp(1.2521739130434781 - 0.16722408026755853 * var_chaste_interface__membrane__V)) - 0.035478708262932285 * var_x69 * var_x89 * var_x90 - 0.0012812882037032361 * var_x90 * var_x91 * (6.4745439480613349 * var_x92 - var_x84 * (1.0501178736126389 * var_x69 * var_x85 + 1.6434541406371006 * var_x69 * var_x86) - 13.687910356755641 * var_x69 * var_x82 * var_x87 / pow(var_x83, 2)) / pow((var_x81 + 0.98039215686274506 * var_x88), 2) + 0.0084616719207093495 * var_x76 * var_x89 * var_x91 * var_x92 * mParameters[5]));
        rJacobian[3][4] = -(mDt * (4.3217176843402606e-6 * var_x96 - 1.7979565856234583e-6 * var_x77 - var_x68 * ((var_x41) ? (var_x60) : (var_x67 + var_x93 * var_chaste_interface__ionic_concentrations__Ki))));
        rJacobian[4][0] = 0.0;
        rJacobian[4][1] = 0.0;
        rJacobian[4][2] = -(mDt * (0.18221961715658433 * var_x25 + var_x58 * ((var_x55) ? (-var_x104 + var_x54 * (var_x104 + 6.5370374999999993e-11 * var_x5 * (var_x97 + var_x4 * var_x98))) : (2.4561808914760263e-5 * var_x57 * (var_x97 + var_x56 * var_x98))) - var_x52 * ((var_x41) ? (-var_x101 + var_x40 * (var_x101 + 1.6947874999999998e-11 * var_x99)) : (var_x103 * var_x63))));
        rJacobian[4][3] = -(mDt * (-var_x68 * ((var_x41) ? (var_x105) : (var_x106 + var_x93 * var_chaste_interface__ionic_concentrations__Nai))));
        rJacobian[4][4] = 1.0 - (mDt * (-0.54665885146975302 * var_x26 - 1.139681818874567e-6 * var_x94 - 6.4825765265103909e-6 * var_x96 - var_x68 * ((var_x41) ? (var_x105 + var_x62) : (var_x106 + var_x63 * (var_x46 + var_x102 * var_x36))) - var_x75 * ((var_x55) ? (-var_x107 + var_x70 * (var_x107 + 4.9027781250000001e-11 * var_x71)) : (1.8421356686070195e-5 * var_x74)) - 0.080828497792522477 * var_x94 * pow(var_chaste_interface__fast_sodium_current_m_gate__m, 3) * mParameters[4] * var_chaste_interface__fast_sodium_current_h_gate__h * var_chaste_interface__fast_sodium_current_j_gate__j));
    }

    void Cellluo_rudy_1994FromCellMLBackwardEulerNoLut::UpdateTransmembranePotential(double var_chaste_interface__environment__time)
    {
        // Time units: millisecond
        std::vector<double>& rY = rGetStateVariables();
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
    ...
    void Cellluo_rudy_1994FromCellMLBackwardEulerNoLut::ComputeOneStepExceptVoltage(double var_chaste_interface__environment__time)
    {
        // Time units: millisecond
        std::vector<double>& rY = rGetStateVariables();
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
        ...
        CardiacNewtonSolver<5,Cellluo_rudy_1994FromCellMLBackwardEulerNoLut>* _p_solver = CardiacNewtonSolver<5,Cellluo_rudy_1994FromCellMLBackwardEulerNoLut>::Instance();
    ...
    std::vector<double> Cellluo_rudy_1994FromCellMLBackwardEulerNoLut::ComputeDerivedQuantities(double var_chaste_interface__environment__time, const std::vector<double> & rY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__membrane__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
...
template<>
void OdeSystemInformation<Cellluo_rudy_1994FromCellMLBackwardEulerNoLut>::Initialise(void)
{
