...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellkurata_model_2002FromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__environment__time, const double rCurrentGuess[13], double rJacobian[13][13])
    {
        ...
        const double var_x93 = pow(var_chaste_interface__slowly_activating_delayed_rectifier_potassium_current_n_gate__n, 2) / (0.12 * var_chaste_interface__intracellular_ion_concentrations__Nai + var_chaste_interface__intracellular_ion_concentrations__Ki);
        ...
        const double var_x96 = pow(var_chaste_interface__hyperpolarisation_activated_current_y_gate__y, 2);
        const double var_x97 = 1 / var_chaste_interface__intracellular_ion_concentrations__Nai;
        const double var_x98 = pow(var_x97, 1.3);
        const double var_x99 = exp(-4.0 - 0.033333333333333333 * var_chaste_interface__membrane__V + 0.89088499421326284 * log(10) * log10(140.0 * var_x97));
        ...
        rJacobian[11][9] = 0.0;
        rJacobian[11][10] = 0.0;
        rJacobian[11][11] = 1.0 - (mDt * ((-1.406291416773793e-8 - 4.4324501714929775e-6 * var_x93 - 3.4227414451683221e-6 * var_x95 - 3.0804673006514896e-5 * var_x95 * var_chaste_interface__AP_sensitive_currents_q_gate__q - 3.9577587173161957e-5 * var_x94 * var_x96 + 0.0001711370722584161 * var_x94 * (-0.040569866382531065 * var_chaste_interface__rapidly_activating_delayed_rectifier_potassium_current_pa_gate__paF - 0.027046577588354045 * var_chaste_interface__rapidly_activating_delayed_rectifier_potassium_current_pa_gate__paS) * var_chaste_interface__rapidly_activating_delayed_rectifier_potassium_current_pi_gate__piy) * mParameters[0]));
        rJacobian[11][12] = -(mDt * ((3.4286956523403795e-5 * var_x101 + 1.6191392426334868e-6 * var_x102 - 5.3189402057915725e-7 * var_x93) * mParameters[0]));
        rJacobian[12][0] = 0.0;
        rJacobian[12][1] = 0.0;
        rJacobian[12][2] = 0.0;
        rJacobian[12][3] = 0.0;
        ...
        rJacobian[12][10] = 0.0;
        rJacobian[12][11] = 0.0;
        rJacobian[12][12] = 1.0 - (mDt * ((-2.4287088639502301e-6 * var_x102 - 5.1430434785105685e-5 * var_x101 - 9.2414019019544699e-7 * var_x97 - var_x103 * var_x89 - var_x104 * var_x90 - 2.4598814923744084e-5 * var_x96 * var_x97) * mParameters[0]));
    }
