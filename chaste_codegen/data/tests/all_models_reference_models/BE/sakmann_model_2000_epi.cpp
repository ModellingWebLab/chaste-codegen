...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellsakmann_model_2000_epiFromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__environment__time_converted, const double rCurrentGuess[12], double rJacobian[12][12])
    {
        ...
        const double var_x33 = 5.2033872358071781e-5 * var_x32 / (1.0 - var_x31);
        ...
        const double var_x43 = var_x41 * var_chaste_interface__intracellular_calcium_concentration__Ca_up;
        const double var_x44 = 1 / (1.0 + 144.92753623188406 * var_chaste_interface__intracellular_calcium_concentration__Ca_i);
        ...
        const double var_x49 = (2.0 * var_x48 * pow(var_chaste_interface__intracellular_sodium_concentration__Na_i, 3.0) - 2744000.0 * var_x46 * var_chaste_interface__intracellular_calcium_concentration__Ca_i) / pow((0.0068999999999999999 + var_chaste_interface__intracellular_calcium_concentration__Ca_i), 2);
        const double var_x50 = var_x35 * var_x37;
        const double var_x51 = var_x44 * var_x48 * pow(var_chaste_interface__intracellular_sodium_concentration__Na_i, 2.0);
        ...
        const double var_x54 = var_x32 / (1.0 - var_x53);
        ...
        const double var_x57 = pow(var_chaste_interface__slow_delayed_rectifier_potassium_current_xs_gate__xs, 2) * (0.12195121951219512 * var_chaste_interface__intracellular_potassium_concentration__K_i + 0.0036585365853658534 * var_chaste_interface__intracellular_sodium_concentration__Na_i) / pow((0.029999999999999999 * var_chaste_interface__intracellular_sodium_concentration__Na_i + var_chaste_interface__intracellular_potassium_concentration__K_i), 2);
        const double var_x58 = var_x52 * var_chaste_interface__L_type_Ca_channel_f2ds_gate__f2ds;
        const double var_x59 = log(4.0 * var_x56);
        const double var_x60 = exp(-0.56150836234118207 + 0.056150836234118207 * var_chaste_interface__membrane__V - 1.5 * var_x59);
        ...
        rJacobian[7][7] = 1.0 - (mDt * (-7.0000000000000009 + var_x28 + var_x29 + 0.00040000000000000002 * var_x42 - 0.20784009793819685 * var_x47 - 7.2e-9 * var_x43 - 1.0549054718227806e-6 / var_chaste_interface__intracellular_calcium_concentration__Ca_i - 5.2262998388249204e-10 * var_x49 - 0.00040000000000000002 * var_x40));
        rJacobian[7][8] = -(mDt * (1.0204081632653063e-5 + 0.051020408163265314 * var_x50));
        rJacobian[7][9] = -(mDt * (7.2e-9 * var_x40 + 9.5999999999999999e-8 * var_x42 - 1.7279999999999999e-12 * var_x43));
        rJacobian[7][10] = 0.0;
        ...
        rJacobian[9][5] = 0.0;
        rJacobian[9][6] = 0.0;
        rJacobian[9][7] = -(mDt * (0.019599999999999999 * var_x40 + 3.5279999999999993e-7 * var_x43 - 0.019599999999999999 * var_x42));
        rJacobian[9][8] = -(mDt * (0.050000000000000003));
        rJacobian[9][9] = 1.0 - (mDt * (-0.050000000000000003 + 8.4671999999999982e-11 * var_x43 - 3.5279999999999993e-7 * var_x40 - 4.7040000000000002e-6 * var_x42));
        rJacobian[9][10] = 0.0;
        rJacobian[9][11] = 0.0;
        rJacobian[10][0] = 0.0;
        rJacobian[10][1] = -(mDt * (-var_x55 * (-4.0 * var_x53 + var_x52 * var_chaste_interface__intracellular_potassium_concentration__K_i)));
        rJacobian[10][2] = 0.0;
        rJacobian[10][3] = 0.0;
        rJacobian[10][4] = 0.0;
        rJacobian[10][5] = 0.0;
        ...
        rJacobian[10][8] = 0.0;
        rJacobian[10][9] = 0.0;
        rJacobian[10][10] = 1.0 - (mDt * (-0.00044546582021107476 * var_x57 - var_x55 * var_x58 - 0.00096448500280939937 * var_x56 / var_x61 - 8.4392437745822447e-5 * var_x56 * var_chaste_interface__transient_outward_current_r_gate__r * var_chaste_interface__transient_outward_current_s_gate__s - 1.3534516283556781e-5 * var_x56 * var_chaste_interface__rapid_delayed_rectifier_potassium_current_xr_gate__xr / (1.0 + exp(0.4017857142857143 + 0.044642857142857144 * var_chaste_interface__membrane__V)) + 5.4156639443013623e-5 * var_x56 * var_x60 * (-26.713760659695652 * var_x59 + var_chaste_interface__membrane__V) / pow(var_x61, 2)));
        rJacobian[10][11] = -(mDt * (0.00070764675538871125 * var_x62 - 1.3363974606332242e-5 * var_x57 - 4.4227922211794452e-7 * var_x63));
        rJacobian[11][0] = 0.0;
        rJacobian[11][1] = -(mDt * (-var_x64 * (-140.0 * var_x53 + var_x52 * var_chaste_interface__intracellular_sodium_concentration__Na_i)));
        rJacobian[11][2] = 0.0;
        rJacobian[11][3] = 0.0;
        rJacobian[11][4] = 0.0;
        rJacobian[11][5] = 0.0;
        rJacobian[11][6] = 0.0;
        rJacobian[11][7] = -(mDt * (0.62352029381459051 * var_x47 + 1.5678899516474762e-9 * var_x49));
        rJacobian[11][8] = 0.0;
        rJacobian[11][9] = 0.0;
        rJacobian[11][10] = -(mDt * (-0.74036952188011318 * var_x65));
        rJacobian[11][11] = 1.0 - (mDt * (6.634188331769168e-7 * var_x63 - 0.0010614701330830668 * var_x62 - 6.1697460156676094 * var_x65 - 1.0127092529498693e-5 / var_chaste_interface__intracellular_sodium_concentration__Na_i - 1.3633825666499794e-6 * var_x51 - var_x58 * var_x64));
    }
