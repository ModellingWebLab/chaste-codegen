...
#include "CardiacNewtonSolver.hpp"
    ...
    boost::shared_ptr<RegularStimulus> Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut::UseCellMLDefaultStimulus()
    {
    ...
    double Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut::GetIntracellularCalciumConcentration()
    {
        return mStateVariables[1];
    }
    Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut::Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut(boost::shared_ptr<AbstractIvpOdeSolver> /* unused; should be empty */, boost::shared_ptr<AbstractStimulusFunction> pIntracellularStimulus)
                ...
                pIntracellularStimulus)
    {
        ...
        //
        this->mpSystemInfo = OdeSystemInformation<Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut>::Instance();
        Init();
    ...
    Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut::~Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut()
    {
    }

    
    double Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut::GetIIonic(const std::vector<double>* pStateVariables)
    {
    ...
    void Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut::ComputeResidual(double var_chaste_interface__Time__time, const double rCurrentGuess[6], double rResidual[6])
    {
        ...
        double var_chaste_interface__membrane__Ca_i_bulk = rCurrentGuess[0];
        double var_chaste_interface__membrane__Ca_i_peripheral = rCurrentGuess[1];
        double var_chaste_interface__membrane__Ca_sr = rCurrentGuess[2];
        double var_chaste_interface__membrane__K_i = rCurrentGuess[3];
        double var_chaste_interface__membrane__Na_i = rCurrentGuess[4];
        double var_chaste_interface__y_ca_Ltype__y_ca_Ltype = rCurrentGuess[5];
        ...
        rResidual[0] = rCurrentGuess[0] - rY[1] - mDt*d_dt_chaste_interface_var_membrane__Ca_i_bulk;
        rResidual[1] = rCurrentGuess[1] - rY[2] - mDt*d_dt_chaste_interface_var_membrane__Ca_i_peripheral;
        rResidual[2] = rCurrentGuess[2] - rY[3] - mDt*d_dt_chaste_interface_var_membrane__Ca_sr;
        rResidual[4] = rCurrentGuess[4] - rY[4] - mDt*d_dt_chaste_interface_var_membrane__Na_i;
        rResidual[3] = rCurrentGuess[3] - rY[5] - mDt*d_dt_chaste_interface_var_membrane__K_i;
        rResidual[5] = rCurrentGuess[5] - rY[10] - mDt*d_dt_chaste_interface_var_y_ca_Ltype__y_ca_Ltype;
    }

    void Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut::ComputeJacobian(double var_chaste_interface__Time__time, const double rCurrentGuess[6], double rJacobian[6][6])
    {
        std::vector<double>& rY = rGetStateVariables();
        double var_chaste_interface__membrane__Vm = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
        // Units: voltage_units; Initial value: -88.744
        ...
        const double var_x0 = 6.5103007883492169e-7 * mParameters[0];
        const double var_x1 = pow(var_chaste_interface__membrane__Ca_i_bulk, 0.60000000000000009);
        const double var_x2 = pow(var_chaste_interface__membrane__Ca_i_bulk, 1.6000000000000001);
        const double var_x3 = 594982.00195257401 * var_x2;
        const double var_x4 = pow(var_chaste_interface__membrane__Ca_sr, 1.6000000000000001);
        const double var_x5 = 0.42784033515676106 * var_x4;
        const double var_x6 = 1.0 + var_x5;
        const double var_x7 = mParameters[3] / (var_x3 + var_x6);
        const double var_x8 = var_x1 * var_x7;
        const double var_x9 = 1.6807231087970119e-6 + 7.1908113817342693e-7 * var_x4;
        const double var_x10 = -var_x5;
        const double var_x11 = (var_x10 + var_x3) * mParameters[3] / pow((var_x2 + var_x9), 2);
        const double var_x12 = var_x1 * var_x11;
        const double var_x13 = pow(var_chaste_interface__membrane__Ca_sr, 0.60000000000000009);
        const double var_x14 = var_x13 * var_x7;
        const double var_x15 = var_x11 * var_x13;
        const double var_x16 = 1 / var_chaste_interface__membrane__Ca_i_peripheral;
        const double var_x17 = pow(var_chaste_interface__membrane__Ca_i_peripheral, 0.60000000000000009);
        const double var_x18 = pow(var_chaste_interface__membrane__Ca_i_peripheral, 1.6000000000000001);
        const double var_x19 = 594982.00195257401 * var_x18;
        const double var_x20 = mParameters[3] / (var_x19 + var_x6);
        const double var_x21 = var_x17 * var_x20;
        const double var_x22 = pow(var_x16, 1.5);
        const double var_x23 = (var_x10 + var_x19) * mParameters[3] / pow((var_x18 + var_x9), 2);
        const double var_x24 = var_x17 * var_x23;
        const double var_x25 = 0.018716945411372737 * var_chaste_interface__membrane__Vm;
        const double var_x26 = exp(var_x25);
        const double var_x27 = pow(var_chaste_interface__membrane__Na_i, 3.0) * mParameters[4];
        const double var_x28 = exp(-var_x25);
        const double var_x29 = pow(mParameters[6], 3.0);
        const double var_x30 = var_x29 * var_chaste_interface__membrane__Ca_i_peripheral;
        const double var_x31 = var_x26 * var_x27 - var_x28 * var_x30;
        const double var_x32 = 1.0 + 0.001 * var_x27 + 0.001 * var_x30;
        const double var_x33 = mParameters[14] / var_x32;
        const double var_x34 = var_x31 * var_x33 / pow((0.0068999999999999999 + var_chaste_interface__membrane__Ca_i_peripheral), 2);
        const double var_x35 = pow(mParameters[2], 2);
        const double var_x36 = pow(var_chaste_interface__membrane__Ca_i_peripheral, 2);
        const double var_x37 = 1.0 + var_x35 / var_x36;
        const double var_x38 = var_x35 * mParameters[1] * var_chaste_interface__membrane__Ca_sr / (pow(var_x37, 2) * pow(var_chaste_interface__membrane__Ca_i_peripheral, 3));
        const double var_x39 = 1 / (1.0 + 144.92753623188406 * var_chaste_interface__membrane__Ca_i_peripheral);
        const double var_x40 = var_x29 * var_x39;
        const double var_x41 = var_x28 * var_x33 * var_x40;
        const double var_x42 = var_x31 * mParameters[14] / pow(var_x32, 2);
        const double var_x43 = var_x40 * var_x42;
        const double var_x44 = mParameters[7] * var_chaste_interface__x_Ltype__x_Ltype * var_chaste_interface__y_Ltype__y_Ltype;
        const double var_x45 = mParameters[1] / var_x37;
        const double var_x46 = var_x13 * var_x20;
        const double var_x47 = var_x13 * var_x23;
        const double var_x48 = var_x39 * pow(var_chaste_interface__membrane__Na_i, 2.0) * mParameters[4];
        const double var_x49 = var_x26 * var_x33 * var_x48;
        const double var_x50 = var_x42 * var_x48;
        const double var_x51 = 6.5103007883492173e-6 * mParameters[0];
        const double var_x52 = 1 / var_chaste_interface__membrane__K_i;
        const double var_x53 = 2.6087192562388333e-5 * var_x52;
        const double var_x54 = 1 / var_chaste_interface__membrane__Na_i;
        const double var_x55 = 1 / ((1.0 + 2.536253862184366 * pow((1 / mParameters[5]), 1.45)) * (1.0 + exp(0.0080000000000000002 * var_chaste_interface__membrane__Vm)) * (1.0 + exp(-1.7777777777777777 - 0.022222222222222223 * var_chaste_interface__membrane__Vm)) * pow((0.031269543464665414 + var_x54), 2) * pow(var_chaste_interface__membrane__Na_i, 2));
        const double var_x56 = 1 / (1.0 + 99999999.999999985 * var_x36);
        const double var_x57 = 1 / (2.0 + 80.0 * var_x56);
        const double var_x58 = var_chaste_interface__membrane__Ca_i_peripheral / pow((1.0e-8 + var_x36), 2);
        
        
        rJacobian[0][0] = 1.0 - (mDt * (-0.0032551503941746084 - var_x0 + 1.7507220768316695e-12 * var_x12 - 0.61976188741847005 * var_x8));
        rJacobian[0][1] = -(mDt * (0.0032551503941746084));
        rJacobian[0][2] = -(mDt * (var_x0 + 4.4565908340138476e-7 * var_x14 + 1.2589112236334627e-18 * var_x15));
        rJacobian[0][3] = 0.0;
        rJacobian[0][4] = 0.0;
        rJacobian[0][5] = 0.0;
        rJacobian[1][0] = -(mDt * (0.0097654511825238238));
        rJacobian[1][1] = 1.0 - (mDt * (-0.0097654511825238238 + 5.2521662304950081e-12 * var_x24 + 3.9061804730095297e-6 * var_x38 - 0.0020000000000003904 * var_x41 - 2.0000000000003904e-6 * var_x43 - 1.3800000000002694e-5 * var_x34 - 1.8592856622554101 * var_x21 - 2.608719256238833e-9 * var_x16 - 2.34784733061495e-5 * var_x16 * var_chaste_interface__x_Ttype__x_Ttype * var_chaste_interface__y_Ttype__y_Ttype - 1.4648176773785738e-11 * var_x16 * var_x22 / pow((1.0 + 1.0000000000000002e-6 * var_x22), 2) - 2.608719256238833e-5 * var_x16 * var_x44 * var_chaste_interface__y_ca_Ltype__y_ca_Ltype));
        rJacobian[1][2] = -(mDt * (1.9530902365047649e-6 * var_x45 + 3.776733670900388e-18 * var_x47 + 1.3369772502041541e-6 * var_x46));
        rJacobian[1][3] = 0.0;
        rJacobian[1][4] = -(mDt * (0.0060000000000011711 * var_x49 - 6.0000000000011716e-6 * var_x50));
        rJacobian[1][5] = -(mDt * (var_x44 * (2.608719256238833e-5 * log(var_x16 * mParameters[4]) - 1.9530902365047649e-6 * var_chaste_interface__membrane__Vm)));
        rJacobian[2][0] = -(mDt * (var_x51 + 6.1976188741847009 * var_x8 - 1.7507220768316693e-11 * var_x12));
        rJacobian[2][1] = -(mDt * (6.1976188741847009 * var_x21 - 1.7507220768316693e-11 * var_x24 - 1.3020601576698435e-5 * var_x38));
        rJacobian[2][2] = 1.0 - (mDt * (-var_x51 - 4.4565908340138475e-6 * var_x14 - 4.4565908340138475e-6 * var_x46 - 6.5103007883492173e-6 * var_x45 - 1.2589112236334627e-17 * var_x15 - 1.2589112236334627e-17 * var_x47));
        rJacobian[2][3] = 0.0;
        rJacobian[2][4] = 0.0;
        rJacobian[2][5] = 0.0;
        rJacobian[3][0] = 0.0;
        rJacobian[3][1] = 0.0;
        rJacobian[3][2] = 0.0;
        rJacobian[3][3] = 1.0 - (mDt * (-2.6087192562388331e-7 * var_x52 - 7.8261577687164992e-5 * var_x52 / (1.0 + exp(0.29411764705882354 - 0.058823529411764705 * var_chaste_interface__membrane__Vm)) - 4.9229056822851045e-6 * var_x52 * var_chaste_interface__y_gate_f_k__y_gate_f_k - var_x53 * mParameters[13] * var_chaste_interface__x_ks__x_ks * var_chaste_interface__y_ks__y_ks - var_x53 * mParameters[15] * var_chaste_interface__x_to_fast__x_to_fast * var_chaste_interface__y_to_fast__y_to_fast - 6.7688101937298463e-6 * var_x52 * pow(mParameters[5], 0.80000000000000004) * mParameters[11] / (1.0 + exp(9.1999999999999993 + 0.10000000000000001 * var_chaste_interface__membrane__Vm)) - 4.8309615856274683e-6 * var_x52 * mParameters[5] * mParameters[12] * var_chaste_interface__y_kr__y_kr / (1.0 + exp(1.4732142857142858 + 0.044642857142857144 * var_chaste_interface__membrane__Vm))));
        rJacobian[3][4] = -(mDt * (2.7006144546041496e-5 * var_x55));
        rJacobian[3][5] = 0.0;
        rJacobian[4][0] = 0.0;
        rJacobian[4][1] = -(mDt * (1.0350000000002022e-5 * var_x34 + 0.001500000000000293 * var_x41 + 1.5000000000002929e-6 * var_x43));
        rJacobian[4][2] = 0.0;
        rJacobian[4][3] = 0.0;
        rJacobian[4][4] = 1.0 - (mDt * (4.5000000000008793e-6 * var_x50 - 2.6087192562388331e-7 * var_x54 - 0.0045000000000008792 * var_x49 - 4.0509216819062244e-5 * var_x55 - 1.1814973773137653e-6 * var_x54 * var_chaste_interface__y_gate_f_na__y_gate_f_na - 5.2174385124776666e-5 * var_x54 * var_chaste_interface__x_na_late__x_na_late * var_chaste_interface__y_na_late__y_na_late - 2.6087192562388333e-5 * var_x54 * mParameters[9] * var_chaste_interface__x_na_fast__x_na_fast * var_chaste_interface__y_na_fast__y_na_fast));
        rJacobian[4][5] = 0.0;
        rJacobian[5][0] = 0.0;
        rJacobian[5][1] = -(mDt * (-1.2e-8 * var_x57 * var_x58 + 2.5000000000000002e-10 * var_x58 * (0.40000000000000002 - var_chaste_interface__y_ca_Ltype__y_ca_Ltype + 0.59999999999999998 * var_x56) / pow((0.025000000000000001 + var_x56), 2)));
        rJacobian[5][2] = 0.0;
        rJacobian[5][3] = 0.0;
        rJacobian[5][4] = 0.0;
        rJacobian[5][5] = 1.0 - (mDt * (-var_x57));
    }

    void Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut::UpdateTransmembranePotential(double var_chaste_interface__Time__time)
    {
        // Time units: millisecond
        std::vector<double>& rY = rGetStateVariables();
        double var_chaste_interface__membrane__Vm = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
    ...
    void Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut::ComputeOneStepExceptVoltage(double var_chaste_interface__Time__time)
    {
        ...
        const double var_x_Ltype__tau_x_Ltype = 0.69999999999999996;
        ...
        const double var_x_Ttype__x_inf_Ttype = 1 / (1.0 + exp(-8.6909090909090896 - 0.18181818181818182 * var_chaste_interface__membrane__Vm));
        const double var_x_ks__tau_x_ks = 1 / (0.00013100000000000001 * (30.0 + var_chaste_interface__membrane__Vm) / (-1.0 + exp(2.0609999999999999 + 0.068699999999999997 * var_chaste_interface__membrane__Vm)) + 7.1899999999999999e-5 * (30.0 + var_chaste_interface__membrane__Vm) / (1.0 - exp(-4.4399999999999995 - 0.14799999999999999 * var_chaste_interface__membrane__Vm)));
        const double var_x_ks__x_inf_ks = 1 / (1.0 + exp(0.089820359281437126 - 0.059880239520958084 * var_chaste_interface__membrane__Vm));
        ...
        const double var_x_na_late__tau_x_na_late = 15.0;
        const double var_x_na_late__x_inf_na_late = 1 / (1.0 + exp(-6.0 - 0.20000000000000001 * var_chaste_interface__membrane__Vm));
        const double var_x_to_fast__tau_x_to_fast = 5.0;
        const double var_x_to_fast__x_inf_to_fast = 1 / (1.0 + exp(0.77777777777777779 - 0.1111111111111111 * var_chaste_interface__membrane__Vm));
        const double var_y_Ltype__tau_y_Ltype = 25.100000000000001 / (0.040000000000000001 + 0.69999999999999996 * exp(-0.16483600000000001 * pow((1 + 0.068965517241379309 * var_chaste_interface__membrane__Vm), 2)));
        const double var_y_Ltype__y_inf_Ltype = 1 / (1.0 + exp(5.5956678700361007 + 0.18050541516245489 * var_chaste_interface__membrane__Vm));
        const double var_y_Ttype__tau_y_Ttype = 1.4227099999999999 * exp(-0.051189999999999999 * var_chaste_interface__membrane__Vm);
        const double var_y_Ttype__y_inf_Ttype = 1 / (1.0 + exp(17.545219638242894 + 0.25839793281653745 * var_chaste_interface__membrane__Vm));
        const double var_y_gate_f_k__tau_y_f_gate = 6000.0 / (exp(3.6000000000000001 + 0.11 * var_chaste_interface__membrane__Vm) + exp(-2.8999999999999999 - 0.040000000000000001 * var_chaste_interface__membrane__Vm));
        const double var_y_gate_f_k__y_inf_f_gate = 1 / (1.0 + exp(10.9 + 0.10000000000000001 * var_chaste_interface__membrane__Vm));
        ...
        const double var_y_gate_f_na__y_inf_f_gate = 1 / (1.0 + exp(10.9 + 0.10000000000000001 * var_chaste_interface__membrane__Vm));
        const double var_y_kr__y_inf_kr = 1 / (1.0 + exp(-6.666666666666667 - 0.13333333333333333 * var_chaste_interface__membrane__Vm));
        ...
        const double var_y_na_fast__tau_y_na_fast = 2.0;
        const double var_y_na_fast__y_inf_na_fast = 1 / (1.0 + exp(17.424242424242426 + 0.25252525252525254 * var_chaste_interface__membrane__Vm - 0.25252525252525254 * mParameters[10]));
        const double var_y_na_late__tau_y_na_late = 120.0 + exp(4.0 + 0.040000000000000001 * var_chaste_interface__membrane__Vm);
        const double var_y_na_late__y_inf_na_late = 0.10000000000000001 + 0.90000000000000002 / (1.0 + exp(12.0 + 0.15873015873015872 * var_chaste_interface__membrane__Vm));
        const double var_y_to_fast__tau_y_to_fast = 350.0;
        const double var_y_to_fast__y_inf_to_fast = 1 / (1.0 + exp(3.4375 + 0.125 * var_chaste_interface__membrane__Vm));
        
        
        rY[8] = (var_chaste_interface__x_Ltype__x_Ltype + ((var_x_Ltype__x_inf_Ltype / var_x_Ltype__tau_x_Ltype) * mDt)) / (1.0 - ((-1 / var_x_Ltype__tau_x_Ltype) * mDt));
        rY[9] = (var_chaste_interface__y_Ltype__y_Ltype + ((var_y_Ltype__y_inf_Ltype / var_y_Ltype__tau_y_Ltype) * mDt)) / (1.0 - ((-1 / var_y_Ltype__tau_y_Ltype) * mDt));
        rY[14] = (var_chaste_interface__y_na_fast__y_na_fast + ((var_y_na_fast__y_inf_na_fast / var_y_na_fast__tau_y_na_fast) * mDt)) / (1.0 - ((-1 / var_y_na_fast__tau_y_na_fast) * mDt));
        rY[13] = (var_chaste_interface__x_na_fast__x_na_fast + ((var_x_na_fast__x_inf_na_fast / var_x_na_fast__tau_x_na_fast) * mDt)) / (1.0 - ((-1 / var_x_na_fast__tau_x_na_fast) * mDt));
        rY[6] = (var_chaste_interface__x_Ttype__x_Ttype + ((var_x_Ttype__x_inf_Ttype / var_x_Ttype__tau_x_Ttype) * mDt)) / (1.0 - ((-1 / var_x_Ttype__tau_x_Ttype) * mDt));
        rY[18] = (var_chaste_interface__x_ks__x_ks + ((var_x_ks__x_inf_ks / var_x_ks__tau_x_ks) * mDt)) / (1.0 - ((-1 / var_x_ks__tau_x_ks) * mDt));
        rY[15] = (var_chaste_interface__x_na_late__x_na_late + ((var_x_na_late__x_inf_na_late / var_x_na_late__tau_x_na_late) * mDt)) / (1.0 - ((-1 / var_x_na_late__tau_x_na_late) * mDt));
        rY[11] = (var_chaste_interface__x_to_fast__x_to_fast + ((var_x_to_fast__x_inf_to_fast / var_x_to_fast__tau_x_to_fast) * mDt)) / (1.0 - ((-1 / var_x_to_fast__tau_x_to_fast) * mDt));
        rY[7] = (var_chaste_interface__y_Ttype__y_Ttype + ((var_y_Ttype__y_inf_Ttype / var_y_Ttype__tau_y_Ttype) * mDt)) / (1.0 - ((-1 / var_y_Ttype__tau_y_Ttype) * mDt));
        rY[20] = (var_chaste_interface__y_gate_f_k__y_gate_f_k + ((var_y_gate_f_k__y_inf_f_gate / var_y_gate_f_k__tau_y_f_gate) * mDt)) / (1.0 - ((-1 / var_y_gate_f_k__tau_y_f_gate) * mDt));
        rY[21] = (var_chaste_interface__y_gate_f_na__y_gate_f_na + ((var_y_gate_f_na__y_inf_f_gate / var_y_gate_f_na__tau_y_f_gate) * mDt)) / (1.0 - ((-1 / var_y_gate_f_na__tau_y_f_gate) * mDt));
        rY[17] = (var_chaste_interface__y_kr__y_kr + ((var_y_kr__y_inf_kr / var_y_kr__tau_y_kr) * mDt)) / (1.0 - ((-1 / var_y_kr__tau_y_kr) * mDt));
        rY[19] = (var_chaste_interface__y_ks__y_ks + ((var_y_ks__y_inf_ks / var_y_ks__tau_y_ks) * mDt)) / (1.0 - ((-1 / var_y_ks__tau_y_ks) * mDt));
        rY[16] = (var_chaste_interface__y_na_late__y_na_late + ((var_y_na_late__y_inf_na_late / var_y_na_late__tau_y_na_late) * mDt)) / (1.0 - ((-1 / var_y_na_late__tau_y_na_late) * mDt));
        rY[12] = (var_chaste_interface__y_to_fast__y_to_fast + ((var_y_to_fast__y_inf_to_fast / var_y_to_fast__tau_y_to_fast) * mDt)) / (1.0 - ((-1 / var_y_to_fast__tau_y_to_fast) * mDt));
        
        double _guess[6] = {rY[1],rY[2],rY[3],rY[5],rY[4],rY[10]};
        CardiacNewtonSolver<6,Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut>* _p_solver = CardiacNewtonSolver<6,Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut>::Instance();
        _p_solver->Solve(*this, var_chaste_interface__Time__time, _guess);
        rY[1] = _guess[0];
        rY[2] = _guess[1];
        rY[3] = _guess[2];
        rY[5] = _guess[3];
    ...
    std::vector<double> Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut::ComputeDerivedQuantities(double var_chaste_interface__Time__time, const std::vector<double> & rY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__membrane__Vm = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
...
template<>
void OdeSystemInformation<Cellcorrias_purkinje_2011FromCellMLBackwardEulerNoLut>::Initialise(void)
{
