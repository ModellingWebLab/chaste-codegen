...
#include "CardiacNewtonSolver.hpp"
    ...
    void Cellpandit_model_2001_epiFromCellMLBackwardEuler::ComputeJacobian(double var_chaste_interface__environment__time_converted, const double rCurrentGuess[13], double rJacobian[13][13])
    {
        ...
        const double var_x40 = log(10);
        const double var_x41 = pow(var_chaste_interface__intracellular_ion_concentrations__Na_i, 2);
        const double var_x42 = var_x34 * var_x38 * var_x41;
        const double var_x43 = var_x37 * var_x41;
        const double var_x44 = 0.0023800000000000002 + var_chaste_interface__intracellular_ion_concentrations__Ca_ss;
        const double var_x45 = 1.0 + 0.00011900000000000002 / pow(var_x44, 2);
        const double var_x46 = 1 / var_x45;
        const double var_x47 = 0.001 * var_x46;
        const double var_x48 = var_x47 * (84000.0 * var_chaste_interface__intracellular_ion_concentrations__Ca_JSR - 84000.0 * var_chaste_interface__intracellular_ion_concentrations__Ca_ss);
        const double var_x49 = 84000.0 * var_chaste_interface__SR_Ca_release_channel__P_O1;
        const double var_x50 = 84000.0 * var_chaste_interface__SR_Ca_release_channel__P_O2;
        const double var_x51 = 0.10000000000000001 * var_chaste_interface__L_type_Ca_channel_Ca_inact_gate__Ca_inact;
        const double var_x52 = 1 / var_chaste_interface__intracellular_ion_concentrations__K_i;
        const double var_x53 = var_x40 * log10(5.4000000000000004 * var_x52);
        const double var_x54 = exp(-0.10977188457982838 + 0.06345195640452507 * var_chaste_interface__membrane__V - 1.613 * var_x53);
        const double var_x55 = 1.0 + var_x54;
        const double var_x56 = var_x52 / (1.0 + exp(-35.49354838709678));
        const double var_x57 = exp(4.5158823529411762 + 1.4953416461467837 * var_x53 - 0.058823529411764705 * var_chaste_interface__membrane__V);
        const double var_x58 = 0.040000000000000001 * var_chaste_interface__membrane__V;
        const double var_x59 = 1 / var_chaste_interface__intracellular_ion_concentrations__Na_i;
        const double var_x60 = pow(var_x59, 1.5);
        const double var_x61 = var_x59 * var_x60 / (pow((0.031622776601683791 + var_x60), 2) * (1.0 + 0.1245 * exp(-0.0039337852699643568 * var_chaste_interface__membrane__V) + 0.036499999999999998 * (-0.14285714285714285 + 0.14285714285714285 * exp(2.0802377414561666)) * exp(-0.039337852699643565 * var_chaste_interface__membrane__V)));
        ...
        rJacobian[9][9] = 1.0 - (mDt * (var_x27 * (-30837.453183520603 - var_x31 + 40000.0 * var_chaste_interface__intracellular_and_SR_Ca_fluxes__LTRPNCa + 200000.0 * var_chaste_interface__intracellular_and_SR_Ca_fluxes__HTRPNCa + 3.5444788081288787e-5 * var_x21 - 4.0288009715211813e-8 * var_x37 - 0.00022799730679188537 * var_x29 - 30.33500195190371 * var_x39 - 1625.0626147884036 * var_x20 + 0.0022145492519739826 * var_chaste_interface__intracellular_ion_concentrations__Ca_i / pow(var_x30, 2)) + 0.001 * (0.0029999999999999996 / pow(var_x23, 3) + 0.00023800000000000004 / pow(var_x24, 3)) * (var_x5 + 0.066000000000000003 * var_chaste_interface__intracellular_and_SR_Ca_fluxes__HTRPNCa + 37.453183520599246 * var_chaste_interface__intracellular_ion_concentrations__Ca_ss - 37.453183520599246 * var_chaste_interface__intracellular_ion_concentrations__Ca_i - 1.7937848940989261e-5 * var_chaste_interface__membrane__V - var_x22 * (0.14000000000000001 - var_chaste_interface__intracellular_and_SR_Ca_fluxes__HTRPNCa) - var_x28 * (0.070000000000000007 - var_chaste_interface__intracellular_and_SR_Ca_fluxes__LTRPNCa) - var_x31 * var_chaste_interface__intracellular_ion_concentrations__Ca_i + 0.00022799730679188537 * var_x40 * log10(1.2 * var_x29) + 1.1055029865854122e-5 * var_x36 * var_x38 - 1.0 * var_x16 * var_x17) / pow(var_x25, 2)));
        rJacobian[9][10] = -(mDt * (0.037453183520599252 * var_x26));
        rJacobian[9][11] = 0.0;
        rJacobian[9][12] = -(mDt * (var_x27 * (3.9798107517074834e-5 * var_x42 - 5.2855989422289547e-14 * var_x43)));
        rJacobian[10][0] = -(mDt * (var_x47 * (870.15176483187713 - 13.386950228182727 * var_chaste_interface__membrane__V) * (0.10000000000000001 * var_chaste_interface__L_type_Ca_channel_f_11_gate__f_11 - 0.10000000000000001 * var_chaste_interface__L_type_Ca_channel_f_12_gate__f_12) * var_chaste_interface__L_type_Ca_channel_d_gate__d));
        rJacobian[10][1] = 0.0;
        rJacobian[10][2] = 0.0;
        rJacobian[10][3] = -(mDt * (var_x48));
        rJacobian[10][4] = -(mDt * (var_x48));
        rJacobian[10][5] = 0.0;
        rJacobian[10][6] = 0.0;
        rJacobian[10][7] = -(mDt * (var_x47 * (var_x49 + var_x50)));
        rJacobian[10][8] = 0.0;
        rJacobian[10][9] = -(mDt * (29.213483146067414 * var_x46));
        rJacobian[10][10] = 1.0 - (mDt * (var_x47 * (-29213.483146067414 - var_x49 - var_x50) + 2.3800000000000004e-7 * (29213.483146067414 * var_chaste_interface__intracellular_ion_concentrations__Ca_i + 84000.0 * var_x13 - 29213.483146067414 * var_chaste_interface__intracellular_ion_concentrations__Ca_ss - 13.386950228182727 * (-65.0 + var_chaste_interface__membrane__V) * ((0.10000000000000001 - var_x51) * var_chaste_interface__L_type_Ca_channel_f_12_gate__f_12 + (0.90000000000000002 + var_x51) * var_chaste_interface__L_type_Ca_channel_f_11_gate__f_11) * var_chaste_interface__L_type_Ca_channel_d_gate__d) / (pow(var_x44, 3) * pow(var_x45, 2))));
        rJacobian[10][11] = 0.0;
        rJacobian[10][12] = 0.0;
        rJacobian[11][0] = 0.0;
        rJacobian[11][1] = 0.0;
        ...
        rJacobian[11][9] = 0.0;
        rJacobian[11][10] = 0.0;
        rJacobian[11][11] = 1.0 - (mDt * (-3.8843985601580465e-6 * var_x52 - 3.2651466157850246e-5 * var_x52 * var_chaste_interface__hyperpolarisation_activated_current_y_gate__y - 0.00067554757567966038 * var_x56 / var_x55 + 25.420807984495319 * var_x52 * (-3.4336586151856603e-5 * var_chaste_interface__Ca_independent_transient_outward_K_current_s_gate__s - 4.4180257576880957e-6 * var_chaste_interface__Ca_independent_transient_outward_K_current_s_slow_gate__s_slow) * var_chaste_interface__Ca_independent_transient_outward_K_current_r_gate__r - 0.00019703470957323427 * var_x52 * var_chaste_interface__steady_state_outward_K_current_r_ss_gate__r_ss * var_chaste_interface__steady_state_outward_K_current_s_ss_gate__s_ss + 4.286481532120841e-5 * var_x54 * var_x56 * (-1.73 - 25.420807984495319 * var_x53 + var_chaste_interface__membrane__V) / pow(var_x55, 2) - 1.6557538619599519e-7 * var_x52 * var_x57 * (10.0 + 48.0 / (exp(1.48 + var_x58) + exp(-1.48 - var_x58))) / pow((1.0 + var_x57), 2)));
        rJacobian[11][12] = -(mDt * (6.5767488669640998e-6 * var_x61));
        rJacobian[12][0] = 0.0;
        rJacobian[12][1] = 0.0;
        rJacobian[12][2] = 0.0;
        rJacobian[12][3] = 0.0;
        ...
        rJacobian[12][10] = 0.0;
        rJacobian[12][11] = 0.0;
        rJacobian[12][12] = 1.0 - (mDt * (1.5856796826686865e-16 * var_x43 - 2.2560474246135324e-6 * var_x59 - 1.193943225512245e-7 * var_x42 - 9.8651233004461501e-6 * var_x61 - 8.1628665394625614e-6 * var_x59 * var_chaste_interface__hyperpolarisation_activated_current_y_gate__y - 0.022518252522655345 * var_x59 * pow(var_chaste_interface__sodium_current_m_gate__m, 3) * var_chaste_interface__sodium_current_h_gate__h * var_chaste_interface__sodium_current_j_gate__j));
    }
