...
{% include "Shared/hpp/destructor_verify_state_variables_GetIIonic" %}
    ...
    DEVICE
    static void ComputeResidualDevice(
        ValueType {{free_variable.var_name}}, 
        const ValueType rCurrentGuess[NONLINEAR_SIZE], 
        const ValueType rY[], 
        ValueType rResidual[NONLINEAR_SIZE], 
        ValueType mDt, 
    ...
    template<typename ValueType>
    DEVICE
    static void ComputeJacobianDevice(
        ValueType {{free_variable.var_name}}, 
        const ValueType rCurrentGuess[NONLINEAR_SIZE], 
        const ValueType rY[], 
        ValueType rJacobian[NONLINEAR_SIZE][NONLINEAR_SIZE], 
        ValueType mDt, 
        const ValueType mParameters[], 
        bool mSetVoltageDerivativeToZero, 
        ValueType mFixedVoltage, 
    ...
    template<typename ValueType>
    DEVICE
    static void UpdateTransmembranePotentialDevice(
        ValueType {{free_variable.var_name}}, 
        ValueType rY[], 
        ValueType mDt, 
        const ValueType mParameters[], 
